-- =====================================================================
-- RLS GAP CLOSURE — tenant-scoped policies for the 53 NO-RLS tenant tables
-- =====================================================================
-- Author: worker W5, branch p1-w5, 2026-07-12. Corrected by worker W1,
--   branch p1-w1, 2026-07-15 (see deploy-prep/rls-tier4-missing-tables-reconciliation.md).
-- Source of truth: deploy-prep/rls-coverage-audit.md (the original 58 "GAP —
--   no RLS" tenant tables, later reconciled to 53 — see correction note below).
--   Target policy shape: platform/docs/tenant-isolation-rls-plan.md Stage 1.
--
-- ***  PREP FILE — DO NOT EXECUTE AS-IS. NOT RUN BY W1/W5. NOT IN THE APPLIED  ***
-- ***  MIGRATION SEQUENCE. The leader runs prod DDL only after Jeff           ***
-- ***  approves and the HARD PRECONDITIONS below are met.                     ***
--
-- ---------------------------------------------------------------------
-- CORRECTION (W1, 2026-07-15): the original 58-target list included 5 tables
-- that do NOT exist in prod and have no plausible rename target:
--   booking_cleaners, cleaners, cleaner_payouts, settings, member_pin_reset_codes
-- Confirmed independently by W2 (live census), leader (direct Mgmt-API read),
-- and W1 (PostgREST probe via a third credential path) — all agree these 5
-- return 404/PGRST205 "Could not find the table" in prod. Full disposition
-- per table (dead code vs. needs repointing to team_members / team_member_payouts
-- / booking_assignees vs. unapplied migration) is in
-- deploy-prep/rls-tier4-missing-tables-reconciliation.md — NOT settled here,
-- this file only removes them from the RLS target list so the guard can run.
-- They have been removed ENTIRELY below (target array + ALTER/POLICY blocks),
-- not commented out, since they have no rename target to preserve. Target
-- count updated 58 -> 53 throughout (guard self-check, RAISE messages, Tier 4
-- count, footer tally).
-- ---------------------------------------------------------------------
--
-- ---------------------------------------------------------------------
...
-- rows. Keep this array in sync with the 53 targets applied below.
-- ---------------------------------------------------------------------
DO $guard$
DECLARE
  _targets text[] := ARRAY[
    ...
    -- Tier 4 — Core client / ops (5 phantom tables removed — see correction
    -- note above: booking_cleaners, cleaners, cleaner_payouts, settings,
    -- member_pin_reset_codes do not exist in prod)
    'booking_notes','crews',
    'recurring_schedules','schedule_issues','routes','notifications',
    'tenant_settings','tenant_invites',
    'oauth_state_nonces',
...
BEGIN
  IF array_length(_targets, 1) <> 53 THEN
    RAISE EXCEPTION 'RLS gap-closure guard: expected 53 targets, found %',
  ...
  RAISE NOTICE 'RLS gap-closure guard passed: 53 targets exist, tenant_id present, zero NULLs.';
END
$guard$;
...
-- (5 phantom tables removed here — booking_cleaners, cleaners,
-- cleaner_payouts, settings, member_pin_reset_codes do not exist in prod
-- and have no rename target. See correction note at top of file and
-- deploy-prep/rls-tier4-missing-tables-reconciliation.md for full disposition.)
-- =====================================================================
...
-- =====================================================================
-- End of 53 targets (Tier 1: 8, Tier 2: 14, Tier 3: 7, Tier 4: 9, Tier 5: 15).
-- (Originally 58 targets / Tier 4: 14 — 5 phantom tables removed, see
-- correction note at top of file.)
