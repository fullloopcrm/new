...
1. **Redirect direction** — `curl -sI https://www.<domain>/sitemap.xml`. ⚠ Do **not** assume a clean `301/308` → apex here: for these three the apex→www redirect is deliberately skipped in app middleware, and www is *not* served by the app — Vercel treats the apex as primary and `307`s www→apex (per the `APEX_CANONICAL_DOMAINS` comment, `middleware.ts:171-174`). So the www URL may `307` to apex **or may fail entirely** (no www DNS/cert on the migrated build). Record the raw code; a 5xx / connection failure / cert error on www is itself a finding, because the sitemap points there (see #2).
2. **`<loc>` host matches canonical** — ⚠ known defect (all three now confirmed in code): their sitemap generators hardcode `https://www.…` `<loc>` URLs even though the site canonicalizes to the **apex**. `the-nyc-marketing-company` (`sitemap.ts:4` base `https://www.thenycmarketingcompany.com`), `consortium-nyc` (`sitemap.ts:4` `https://www.consortiumnyc.com`), and `the-nyc-interior-designer` (`_lib/siteData.ts:4` `SITE_DOMAIN = "https://www.thenycinteriordesigner.com"`, consumed by its `sitemap.ts`). This is the same www-vs-apex class as `seo-canonical-audit.md` Flag 4 — a sitemap advertising the non-canonical (and possibly unserved) host dilutes indexing. **Check:** every `<loc>` host must equal the apex canonical host. Full dedicated detection + fix: `sitemap-www-vs-apex-detection.md`.
