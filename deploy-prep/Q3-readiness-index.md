...
| 5 | **owner_phone backfill** (DATA) | p1-w1 | ⚠️ **file TBD** | — | Populate `tenants.owner_phone` for every non-nycmaid tenant. **⚠️ MUST run BEFORE the booking-owner fix deploy** (Wave C `017043f`) or non-nycmaid owners lose admin tooling (fail-closed). **This file does not exist yet — biggest DB-side gap.** |
...
- **Prereq DB writes (ORDER-CRITICAL):**
  - ⚠️ **owner_phone backfill MUST precede `017043f`** (else non-nycmaid owners lose admin tooling). **Backfill file does not exist yet.**
...
- **⚠️ Missing / open:** the owner_phone backfill file (hard blocker for this wave); the 2nd-voice-tenant DID seeding.
...
- **owner_phone backfill (§A #5):** referenced as "file TBD" in the manifest and **still absent here** — the single hardest DB-side blocker for Wave C.
...
🔒 Jeff actions plus two ⚠️ real gaps W1 flags but cannot close from this lane:
the **owner_phone backfill file** (Wave C prereq) and the **061 numbering
collision** (Wave D DB write). Everything else is prepared and waiting on a gate.
