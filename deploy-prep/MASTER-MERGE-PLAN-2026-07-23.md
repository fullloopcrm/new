...
- **NEW (TOP-PRIORITY financial-bug sweep, ~17:37-17:52):**
  ...
  - **CONFLICT E — RESOLVED as of ~18:05, closed not just documented.** W1's calendar-panel refactor (Phase 1 above) DELETES `STATUS_ACTIONS`, `updateStatus()`, and the "Complete" button entirely from `bookings/[id]/page.tsx`, moving them into a new `BookingDetailContent.tsx` — the exact code `770577c0f`'s fix edited, meaning a plain merge would have silently dropped the checkout-price fix. Per leader's 17:59 dispatch, re-applied the fix directly against `BookingDetailContent.tsx` in an isolated scratch worktree (`scratchpad/scratch-merge-test`, W1's `e376fbfca` cherry-picked on top of the existing W1+W3 BookingsAdmin.tsx merge test): widened the component's local `Booking` type with the same 4 fields (`one_time_credit_cents`/`recurring_type`/`max_hours`/`team_size`), ported the identical `computeCheckoutPricing()`-based `completed` branch into this file's `updateStatus()`, ported the RTL test (adjusted for `bookingId`/`onClose` props instead of `useParams`/`useRouter`). **Verified working post-refactor:** test passes, tsc clean, full suite 805 files / 4427 tests passing, 0 failed. **Decision: whoever merges applies W1's `e376fbfca` normally (no special handling needed for it), then applies the fix as `deploy-prep/w3-checkout-price-fix-post-refactor.patch`** (committed to this branch, see below) **to `BookingDetailContent.tsx` instead of trying to cherry-pick the now-obsolete `770577c0f` onto `page.tsx` — that original commit should be DROPPED from the merge sequence** (its target file no longer exists in the post-refactor tree) **in favor of the patch.** The patch is a real, git-apply-able diff, not just a note — `git apply deploy-prep/w3-checkout-price-fix-post-refactor.patch` from the repo root (paths are `a/platform/...`, not `a/src/...`) after `e376fbfca` lands.
