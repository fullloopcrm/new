...
export default function QuoteView({ token }: { token: string }) {
  ...
  async function payDeposit() {
    try {
      const res = await fetch(`/api/quotes/public/${token}/deposit-checkout`, { method: 'POST' })
      const data = await res.json()
      if (res.ok && data.url) { window.location.href = data.url; return }
      setErr(data.error || 'Could not start deposit checkout.')
    } catch { setErr('Could not start deposit checkout.') }
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {(accepted || quote.status === 'accepted' || quote.status === 'converted') && (
          <div className="mb-4 p-4 rounded-xl bg-green-50 border border-green-200">
            ...
            {(quote.deposit_cents ?? 0) > 0 && !quote.deposit_paid_at && (
              <div className="mt-3">
                <p className="text-sm text-green-800 mb-2">A deposit of <strong>{formatCents(quote.deposit_cents || 0)}</strong> secures your booking.</p>
                <button onClick={payDeposit} className="px-5 py-2.5 text-sm font-semibold rounded-lg text-white" style={{ backgroundColor: primary }}>Pay Deposit</button>
              </div>
            )}
            {quote.deposit_paid_at && <p className="text-sm text-green-700 mt-2">✓ Deposit paid — you&apos;re all set.</p>}
            {(quote.deposit_cents ?? 0) === 0 && <p className="text-xs text-green-700 mt-1">Thanks — you&apos;ll be contacted shortly to schedule.</p>}
          </div>
        )}
