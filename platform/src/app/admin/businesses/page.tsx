...
export default function BusinessesPage() {
  ...
  if (loading) return <p className="text-gray-400">Loading businesses...</p>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{businesses.length} total &middot; {activeCount} active &middot; {setupCount} in setup</p>
        </div>
              ...
              return (
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{b.industry?.replace(/_/g, ' ')}</p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-gray-300">{b.owner_name || '—'}</p>
                    {b.owner_email && <p className="text-xs text-gray-500">{b.owner_email}</p>}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${billingColors[b.billing_status] || 'bg-gray-700 text-gray-400'}`}>
