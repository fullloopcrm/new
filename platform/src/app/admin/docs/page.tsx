...
- **Real onboarding process (white-glove, confirmed 2026-07-28 — the actual live path, not this route):** a platform admin creates the tenant at \`/admin/businesses/new\`, walks the wizard (\`/admin/businesses/[id]/wizard\`), then calls \`POST /api/admin/businesses/[id]/provision\` and \`POST /api/admin/businesses/[id]/activate\` (domain registration, onboarding gate, etc. — idempotent, safe to re-run). Activation mints an owner PIN via \`POST /api/admin/businesses/[id]/users\`, returned ONCE to the admin, who hands it to the business owner out of band (call/email). The owner (and any team member created the same way) then logs in at their own tenant domain's \`/fullloop\` page with that PIN — a per-tenant token (\`verifyTenantAdminToken\`), not \`getOwnerUserId()\`/Clerk/admin impersonation. \`/sign-in\` and \`/sign-up\` on the shared marketing domain are intentionally NOT that login — see their own file comments.
