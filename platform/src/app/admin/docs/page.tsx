...
**Notable route behavior:**
- **\`/api/auth/*\` (login/logout/me) is very likely DEAD/orphaned code, not a live auth path.** \`auth/login\` authenticates against \`admin_users\` via legacy \`lib/nycmaid/auth\` (\`hashPassword\`/\`createSessionCookie\`) — but a comment elsewhere in the codebase (\`client-analytics/route.ts\`) states plainly that the \`admin_users\` table has been removed and this route is orphaned. Confirm the table's actual existence in Supabase before relying on or building against this route; it may currently 500 on every call. It does NOT overlap with the real live paths: dashboard users authenticate via Clerk, and \`/admin/*\` authenticates via \`admin-auth\`/admin PIN — this \`auth/login\` is neither.
