...
**Notable route behavior:**
- **Correction — a prior draft of this doc called \`/api/auth/*\` dead code. That was wrong, and the mistake is worth keeping visible: it is NOT dead.** \`auth/login\` authenticates against \`admin_users\` via legacy \`lib/nycmaid/auth\` (\`hashPassword\`/\`createSessionCookie\`), and is actively called by \`src/components/auth/SiteAdminLoginClient.tsx\`, which is wired into the login pages of the three known legacy per-tenant clones (\`nyc-mobile-salon\`, \`the-florida-maid\`, \`wash-and-fold-nyc\` — see the Global Rule "known debt" list). Those tenants' admin logins genuinely depend on this route working. It does NOT overlap with the global paths: dashboard users authenticate via Clerk, and the main \`/admin/*\` authenticates via \`admin-auth\`/admin PIN — this is a third, separate, clone-only path.
  **Unresolved contradiction, not something I could settle from source alone:** a comment in \`client-analytics/route.ts\` states plainly that the \`admin_users\` table "has been removed" — but \`auth/login\` queries that exact table directly, and has a live caller. If the table is really gone, the three clone tenants' admin logins are broken in production right now; if it still exists, that other comment is simply stale. This needs an actual Supabase check to resolve, not another source read — flagging it rather than guessing either way.
...
- \`tenants\` (POST) — creating a new tenant record requires a resolved OWNER user id (\`getOwnerUserId()\`), not just admin auth — a tenant can't be created floating without an owner attached from the start.
...
| \`dashboard\` | 16 routes: \`/api/dashboard\` (GET), \`/api/dashboard/comms-preview\` (GET), \`/api/dashboard/hr\` (GET), \`/api/dashboard/hr/[id]\` (GET,PATCH), \`/api/dashboard/hr/[id]/documents\` (POST,PATCH), \`/api/dashboard/hr/[id]/notes\` (POST), \`/api/dashboard/import/analyze\` (POST), \`/api/dashboard/import/batch/[id]\` (GET,POST), \`/api/dashboard/import/stage\` (POST), \`/api/dashboard/messages\` (GET,POST), \`/api/dashboard/onboarding\` (GET,PATCH), \`/api/dashboard/onboarding/activate\` (POST), \`/api/dashboard/onboarding/profile\` (GET,PUT,POST), \`/api/dashboard/onboarding/progress\` (GET), \`/api/dashboard/schedules/import\` (POST), \`/api/dashboard/team-messages\` (GET) |
...
- \`dashboard\` (16 routes) — despite landing in this catch-all bucket, these are real TENANT dashboard-side APIs: HR document/notes management (\`dashboard/hr/[id]/documents\`, \`/notes\`), the full onboarding flow (\`dashboard/onboarding/profile\`, \`/activate\`, \`/progress\`), and CSV import staging (\`dashboard/import/analyze\`, \`/stage\`, \`schedules/import\`) — not admin or internal tooling despite the bucket name.
...
**Platform utility links (dashboard):** Onboarding *(5-section profile wizard, autosaves on step change + explicit "Save for later"; every field routes to its real store via \`/api/dashboard/onboarding/profile\`)*, Settings, Users, "AI (Voice \\| SMS \\| Web)" → \`/dashboard/selena\` *(Selena's own conversation-stats dashboard — NOT the same as \`/dashboard/ai\` below)*, Platform Docs → \`/dashboard/docs\` *(see Tenant Knowledge Panel — separate from this doc)*.
