...
/**
 * POST /api/reviews/upload is fully unauthenticated (public review-widget
 * upload, tenant resolved from the signed x-tenant-id header). It had no
 * rate limit at all, so an anonymous caller could loop it to write unlimited
 * 100MB video objects into the shared `uploads` bucket, burning storage
...
describe('POST /api/reviews/upload — anonymous storage-abuse rate limit', () => {
