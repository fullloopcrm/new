 ...
 * booking_checklist. Downstream tool handlers WRITE to `clients` keyed off
 * this conversation's client_id, so a garbage phone from an anonymous
 * visitor could silently misattribute (and later corrupt) an unrelated
 * client's record. Same bug class fixed elsewhere this session
 * (getClientProfile, selena/agent.ts loadContext, etc.) but this route's
 * own inline lookup was never covered on this branch.
 */
...
let h: Harness
beforeEach(() => {
  h = createTenantDbHarness({ clients: [{ ...UNRELATED_CLIENT }], sms_conversations: [], sms_conversation_messages: [] })
  holder.from = h.from
})

function chat(phone: string) {
  return POST(
...
describe('POST /api/chat — new conversation phone-link match', () => {
  ...
  it('CONTROL: still links the correct client on an exact 10-digit match', async () => {
    const res = await chat('2125551234')
    expect(res.status).toBe(200)
    const ins = h.capture.inserts.find((i) => i.table === 'sms_conversations')
