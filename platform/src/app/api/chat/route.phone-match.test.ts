 ...
 * client_id, so this was a corruption vector, not just a read. Same bug
 * fixed in sibling /api/yinez. Fixed to require a full, exact 10-digit match.
 */
...
const SECRET = 'chat-phone-match-test-secret'
...
describe('POST /api/chat — new-conversation phone link must be exact', () => {
  it('a short malformed phone does NOT link the new conversation to an unrelated client', async () => {
    ...
    const convo = fake._all('sms_conversations').find((c) => c.tenant_id === TENANT)
