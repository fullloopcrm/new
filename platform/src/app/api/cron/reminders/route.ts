        ...
        for (const booking of tomorrowTeamBookings || []) {
          ...
          if (isNycMaid(tenantId)) {
            ...
            const { data: dayJobs } = await supabaseAdmin
              .from('bookings')
              .select('id, start_time, clients(name, phone, address, latitude, longitude), client_properties(address, latitude, longitude)')
            ...
            if (jobs.length && jobs[0].id === booking.id) {
              ...
              for (let i = 0; i < jobs.length; i++) {
                ...
                // Full name, phone, and address — not just first name — so a
                // cleaner still has the booking's real details in-hand from
                // this one text if the system/portal is down the next day.
                lines.push(`${t} ${j.clients?.name || 'Client'}${j.clients?.phone ? ' ' + j.clients.phone : ''}${addr ? '\n  ' + addr : ''}`)
                if (i < jobs.length - 1) {
