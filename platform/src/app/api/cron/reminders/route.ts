...
import { etHour, etToday, formatNaiveET, nowNaiveET, etDayBoundaryUTC } from '@/lib/recurring'
import type {
    ...
    try {
            ...
            if (client?.email) {
              if (isNycMaid(tenantId) && booking.client_id) {
                ...
                await notify({
                  ...
                  message: `Hi ${clientName}, your ${booking.service_type || 'appointment'} is ${label} on ${naiveToAnchoredDate(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' , timeZone: 'UTC' })} at ${naiveToAnchoredDate(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })}.`,
                  channel: 'email',
      ...
      if (etHour(now) === 20) {
        ...
        for (const booking of tomorrowTeamBookings || []) {
          ...
          let teamMsg = `${client?.name || 'Client'} - tomorrow at ${naiveToAnchoredDate(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })}`
          ...
          if (isNycMaid(tenantId)) {
            ...
            if (jobs.length && jobs[0].id === booking.id) {
              ...
              for (let i = 0; i < jobs.length; i++) {
                ...
                const t = naiveToAnchoredDate(j.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })
      ...
      for (const booking of hourBookings || []) {
        ...
        if (reminderSmsOn && booking.client_id && tenant.telnyx_api_key && tenant.telnyx_phone) {
          const smsBody = `${tenant.name}: Reminder — ${memberFirst} arrives at ${naiveToAnchoredDate(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })}. Almost time!\nReply STOP to opt out.`
          try {
            const result = await sendClientSMS(tenant, booking.client_id, smsBody)
            sent += result.sent
            if (result.sent === 0) failed++
        ...
        if (booking.team_member_id && member?.phone && tenant.telnyx_api_key && tenant.telnyx_phone) {
          const smsBody = `${tenant.name}: Job in ${hoursBefore} hour${hoursBefore === 1 ? '' : 's'} — ${client?.name || 'Client'} at ${naiveToAnchoredDate(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })}`
          try {
      ...
      if (localHour === 8 || localHour === 14) {
        ...
        if (pendingBookings && pendingBookings.length > 0) {
          const details = pendingBookings.slice(0, 5).map(b => {
            ...
            const date = naiveToAnchoredDate(b.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' , timeZone: 'UTC' })
      ...
      if (localHour === 20) {
        ...
        const todayJobsList = (todayBookings || []).map((b: any) => ({
          ...
          time: `${naiveToAnchoredDate(b.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })} – ${naiveToAnchoredDate(b.end_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'UTC' })}`,
      ...
      if (localHour === 21) {
        ...
        const todayStartInstant = todayStart
        const todayEndInstant = todayEnd
