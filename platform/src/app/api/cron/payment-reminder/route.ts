  ...
  for (const tenant of tenants || []) {
    ...
    try {
      ...
      for (const b of pending || []) {
        ...
        // sms_consent is the blanket STOP/START opt-out flag (webhooks/telnyx's STOP handler
        // sets it false tenant-wide) -- this route's client-facing nudge below sent unconditionally,
        // same consent-bypass bug class as payment-followup-daily's fixed sms_consent gate. The
        // owner-escalation SMS past this block is internal and stays ungated.
        ...
        if (minsSinceAlert < 30) {
          if (clientNudgeOn && !clientOptedOut && tenant.telnyx_api_key && tenant.telnyx_phone) {
            await sendSMS({
              to: client.phone,
