  ...
  for (const tenant of tenants || []) {
    try {
      ...
      for (const booking of bookings || []) {
        ...
        // This is another "Reply STOP to opt out" message — must itself
        // honor sms_consent, same as every other real client-SMS call site.
        if (!client?.phone || client.sms_consent === false) {
          skipped++
          continue
        }
      ...
      for (const job of doneJobs || []) {
        ...
        if (!jc?.phone || jc.sms_consent === false) { skipped++; continue }
