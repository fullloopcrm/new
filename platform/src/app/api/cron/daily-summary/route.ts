...
import { etDayBoundaryUTC, etHour } from '@/lib/recurring'

export const maxDuration = 300 // Vercel pro plan

const ADMIN_ET_HOUR = 8 // morning recap — today's jobs + yesterday's revenue
const TEAM_ET_HOUR = 20 // 8pm ET — next-day job lookahead for cleaners

// Daily summary cron — polls hourly, each section only fires at its own ET hour.
// 1. Admin summary (today's jobs, yesterday's revenue) — 8am ET
// 2. Team member 3-day lookahead (SMS + email) — 8pm ET
// 3. Recurring expiration check (30-day warning) — runs alongside the admin summary
export async function GET(request: Request) {
  const cronAuthError = verifyCronSecret(request)
  if (cronAuthError) return cronAuthError

  const now = new Date()
  const runAdmin = etHour(now) === ADMIN_ET_HOUR
  const runTeam = etHour(now) === TEAM_ET_HOUR
  ...
  if (runAdmin || runTeam) {
    ...
    for (const tenant of tenants || []) {
      ...
      // ============================================
      // TEAM MEMBER 3-DAY LOOKAHEAD — 8pm ET
      // ============================================
      if (runTeam) {
