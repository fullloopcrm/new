  ...
  try {
    ...
    if ((via === 'email' || via === 'both') && toEmail) {
      try {
        ...
        // before HTML interpolation so a malicious name/title can't inject
        // markup into the real outbound email sent to toEmail. Same class as
        // the escapeHtml already applied on the accept/decline routes' owner
        // notifications, just missed here on the customer-facing send.
        ...
        const html = emailShell({
          ...
          bodyHtml: `
            ...
            <p style="margin:0 0 14px">Your proposal <strong>${quote.quote_number}</strong>${quote.title ? ` — ${escapeHtml(quote.title)}` : ''} is ready. Total <strong>${formatCents(quote.total_cents)}</strong>.${depositLine}</p>
