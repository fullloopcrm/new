...
describe('POST /api/team-portal/update-phone', () => {
  ...
  it('cascades the phone sync to cleaner_applications by the member email', async () => {
    ...
    store.set('cleaner_applications', [
      { id: 'app-1', email: 'cleaner@example.com', phone: '+15550000000' },
    ])
    const good = makeToken('member-1', Date.now() + 60_000)
    const req = new Request('https://x.test/api/team-portal/update-phone', {
      method: 'POST',
    ...
    const res = await POST(req)
    expect(res.status).toBe(200)
    const application = store.get('cleaner_applications')?.find(a => a.id === 'app-1')
    expect(application?.phone).toBe('+15551234567')
  })

  it('does NOT cascade when the member has no email on file', async () => {
    const store = (supabaseAdmin as unknown as Fake)._store
    store.set('team_members', [
      { id: 'member-1', name: 'Test Cleaner', email: null, phone: '+15550000000' },
    ])
    store.set('cleaner_applications', [
      { id: 'app-1', email: 'cleaner@example.com', phone: '+15550000000' },
    ])
    ...
    expect(res.status).toBe(200)
    const application = store.get('cleaner_applications')?.find(a => a.id === 'app-1')
    expect(application?.phone).toBe('+15550000000')
  })
})
