...
export async function POST(request: Request) {
  ...
  const { data: booking } = await supabaseAdmin
    .from('bookings')
    .select('id, status, check_in_time, check_out_time, hourly_rate, pay_rate, team_size, max_hours, price, service_type_id, recurring_type, team_member_id, referrer_id, sales_partner_id, client_id, clients(name, address, do_not_service, sales_partner_id), team_members!bookings_team_member_id_fkey(pay_rate)')
    .eq('id', booking_id)
