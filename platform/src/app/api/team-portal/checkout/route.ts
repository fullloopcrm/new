...
export const POST = withMobileCors(async function POST(request: Request) {
  ...
  if (isNycMaid(auth.tid)) {
    ...
    // Direct cleaner payout at checkout — pays the cleaner the moment they
    // check out, independent of client payment status (Jeff, 2026-08-07).
    // Shared with the admin dashboard checkout path (PUT /api/bookings/[id])
    // so both surfaces pay the same way and can never drift out of sync.
