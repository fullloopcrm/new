...
/**
 * POST /api/team-portal/running-late sent the client "running late" SMS via a
 * raw sendSMS() call with no sms_consent check — unlike payment-processor.ts/
 * notify-team.ts, which gate SMS on `sms_consent !== false`. A client who'd
 * replied STOP still got these texts. Admin SMS is intentionally NOT gated
 * (business's own number, not consent-gated anywhere else in this codebase).
 * Mock harness mirrors route.tenantdb.test.ts's chain() approach.
 */
...
describe('POST /api/team-portal/running-late — sms_consent gate', () => {
  it('does not SMS the client when sms_consent is false', async () => {
    DB.bookings = [{ id: BOOKING_ID, tenant_id: TENANT, team_member_id: MEMBER_ID, client_id: 'client-a', start_time: new Date().toISOString(), running_late_at: null, running_late_eta: null, clients: { name: 'Client', phone: '+15551110000', sms_consent: false }, team_members: { name: 'Member' } }]
    const res = await POST(makeReq(10))
    expect(res.status).toBe(200)
    expect(sendSMS).not.toHaveBeenCalled()
  })

  it('SMS the client when consented', async () => {
    DB.bookings = [{ id: BOOKING_ID, tenant_id: TENANT, team_member_id: MEMBER_ID, client_id: 'client-a', start_time: new Date().toISOString(), running_late_at: null, running_late_eta: null, clients: { name: 'Client', phone: '+15551110000', sms_consent: true }, team_members: { name: 'Member' } }]
    const res = await POST(makeReq(10))
    expect(res.status).toBe(200)
