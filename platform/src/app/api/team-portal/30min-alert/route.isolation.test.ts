...
/**
 * tenantDb conversion probe — team-portal/30min-alert/route.ts (docs/adr/0004).
...
function req(bookingId: string, token: string): NextRequest {
  return new NextRequest('http://x/api/team-portal/30min-alert', {
    method: 'POST',
...
describe('team-portal/30min-alert POST — auth', () => {
  it('rejects a request with no bearer token', async () => {
    const res = await POST(new NextRequest('http://x/api/team-portal/30min-alert', {
      method: 'POST',
...
describe('team-portal/30min-alert POST — tenantDb isolation', () => {
...
describe('team-portal/30min-alert POST — undelivered-payment escalation isolation', () => {
