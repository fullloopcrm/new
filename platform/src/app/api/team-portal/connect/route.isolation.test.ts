...
/**
 * tenantDb conversion probe — team-portal/connect/route.ts (field-staff
 * side of platform Connect chat). Converts connect_channels/connect_messages/
 ...
 * team member with a valid token for tenant A could pass another tenant's
...
beforeEach(() => {
  store = {
    ...
    connect_channels: [
      { id: 'chan-A1', tenant_id: 'tenant-A', type: 'general' },
      { id: 'chan-B1', tenant_id: 'tenant-B', type: 'general' },
    ],
    connect_messages: [
      { id: 'msg-A1', tenant_id: 'tenant-A', channel_id: 'chan-A1', sender_type: 'team', sender_id: 'm-A', sender_name: 'A', body: 'hi A', created_at: '2026-01-01' },
      { id: 'msg-B1', tenant_id: 'tenant-B', channel_id: 'chan-B1', sender_type: 'team', sender_id: 'm-B', sender_name: 'B', body: 'hi B', created_at: '2026-01-01' },
    ],
...
function getMessages(tid: string, id: string) {
  return GET(new NextRequest('http://x/api/team-portal/connect', { headers: authHeader(tid, id) }))
}

function postMessage(tid: string, id: string, body: string, channel_id?: string) {
  return POST(new NextRequest('http://x/api/team-portal/connect', {
    method: 'POST',
...
describe('team-portal/connect GET — tenantDb isolation', () => {
  it('team member A only ever reads its own tenant\'s general channel/messages', async () => {
    const res = await getMessages('tenant-A', 'member-A')
    const body = await res.json()
    expect(body.messages.map((m: { id: string }) => m.id)).toEqual(['msg-A1'])
  })
})

describe('team-portal/connect POST — tenantDb isolation', () => {
  it('a forged cross-tenant channel_id is rejected, never inserted', async () => {
    const res = await postMessage('tenant-A', 'member-A', 'sneaky', 'chan-B1')
    expect(res.status).toBe(404)
    expect(store.connect_messages.some((m) => m.body === 'sneaky')).toBe(false)
  })

  it('a reply to the caller\'s own tenant channel is stamped with its own tenant_id', async () => {
    const res = await postMessage('tenant-A', 'member-A', 'thanks!', 'chan-A1')
