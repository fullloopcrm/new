...
/**
 * W4 isolation probe for the tenantDb() conversion of GET /api/team-portal/connect/unread.
 * The connect_channels lookup used to carry a manual .eq('tenant_id', auth.tid)
 * filter. Proves a member's unread count is computed off the CALLER tenant's
 * general channel only, not a foreign tenant's channel/messages.
 */
...
describe('GET /api/team-portal/connect/unread — tenantDb scoping', () => {
  it('counts unread only off the caller tenant\'s own channel, ignoring a foreign tenant\'s messages', async () => {
    ...
    const req = new NextRequest('https://x/api/team-portal/connect/unread', {
