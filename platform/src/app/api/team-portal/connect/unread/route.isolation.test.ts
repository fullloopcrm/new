...
/**
 * tenantDb conversion probe — team-portal/connect/unread/route.ts.
 ...
 * it trusted the channel id resolved one query earlier. Proves a team
 * member's unread badge count is built only from its own tenant's general
 * channel/messages, even when another tenant has messages that would
 * inflate the count if the scoping were dropped.
 */
...
function getUnread(tid: string, id: string) {
  ...
  return GET(new NextRequest('http://x/api/team-portal/connect/unread', { headers: header }))
}

describe('team-portal/connect/unread GET — tenantDb isolation', () => {
  it('team member A\'s unread count never includes tenant B\'s messages', async () => {
    const res = await getUnread('tenant-A', 'member-A')
    const body = await res.json()
    expect(body.unread).toBe(1)
  })

  it('a token for tenant B sees its own (larger) unread count, unaffected by tenant A', async () => {
    const res = await getUnread('tenant-B', 'member-B')
    const body = await res.json()
