...
/**
 * POST /api/ai/assistant — bookings.edit gate (broad-hunt: this route, the
 * live AI bar mounted on every dashboard page (dashboard-shell.tsx ->
 * selena-bar.tsx), only called getTenantForRequest() for base tenant auth,
 * no requirePermission check, despite giving the AI tools that mutate
 * bookings/clients and read revenue stats. Per rbac.ts 'staff' lacks
 * bookings.edit (view/create only); 'manager'/'admin'/'owner' all have it
 * and must keep working. Sibling /api/admin/ai-chat already had this gate.
 */
...
describe('POST /api/ai/assistant — bookings.edit permission', () => {
