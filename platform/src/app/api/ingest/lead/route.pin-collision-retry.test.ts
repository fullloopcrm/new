/**
 * POST /api/ingest/lead -- retry-on-conflict for clients.pin.
 *
 ...
 * route is the cross-site ingest sink for external partner sites. A
 * collision threw the raw 23505 up to the outer catch, returning a generic
 * 500 and losing an external site's lead submission outright. Verifies the
 * route instead regenerates and retries, and gives up cleanly once
...
function req(): Request {
  return new Request('https://api.example.com/api/ingest/lead', {
    method: 'POST',
    headers: new Headers({ 'content-type': 'application/json', 'x-ingest-secret': 'test-ingest-secret' }),
    body: JSON.stringify({ tenant_slug: 'acme', name: 'New Lead', email: 'lead@example.com', phone: '2125550000' }),
  })
}
...
describe('POST /api/ingest/lead — clients.pin conflict handling', () => {
