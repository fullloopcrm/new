/**
 * Job photos & videos — job-site media documentation (CompanyCam-style), tenant-scoped.
 *
 * GET  → { photos: JobPhoto[] }  — full gallery for the job, newest first
 * POST → two modes:
 *   - multipart/form-data: direct photo upload (images only — small enough
 *     to pass through the serverless function body limit)
 *   - application/json: reference-save for a video already PUT to the
 *     signed URL from POST /api/jobs/[id]/photos/signed-url (mirrors
 *     /api/team-portal/video-upload's signed-URL flow, since a video
 *     routinely exceeds the ~4.5MB Vercel body cap a multipart POST hits)
 * Both modes log a job_events row so it shows in the existing Activity
 * timeline for free.
 */
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const videoCheck = await verifySignedUpload('uploads', `${tenantId}/job-photos/${jobId}`, url, VIDEO_UPLOAD_CONFIG)
      ...
      const { data: photo, error } = await db
        ...
        .single()
      if (error || !photo) {
        console.error('POST /api/jobs/[id]/photos (video) insert', error)
      ...
      await logJobEvent({
        ...
        detail: { photo_id: photo.id, media_type: 'video', photo_type: photoType },
      })
    ...
    await logJobEvent({
      ...
      job_id: jobId,
      event_type: 'photo_added',
      detail: { photo_id: photo.id, media_type: 'photo', photo_type: photoType, uploaded_by: uploadedBy },
    })
