/**
 * Comments on a single job photo or video. Reuses crm_notes
 * (subject_type = 'job_photo') as the thread instead of a parallel
 * comments table.
 *
 * crm_notes has no tenant_id column — isolation is enforced here by confirming
 * the media item belongs to a job owned by the requesting tenant before any
 * read or write, same pattern as the parent photos route.
 *
 * GET  → { comments: [...] }
