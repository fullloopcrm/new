...
/**
 * GET /api/clients/enriched — permission gate.
 *
 ...
 * returning every client's full PII (name, email, phone, address) plus LTV/
 * health data — the same class of data /api/clients gates behind
...
function seed() {
  return {
    clients: [
      { id: 'cli-a1', tenant_id: A, name: 'Ann', email: 'ann@x.com', phone: null, address: null, status: 'active', source: null, created_at: '2020-01-01' },
    ],
    bookings: [],
    recurring_schedules: [],
    team_members: [],
...
describe('GET /api/clients/enriched — permission probe', () => {
  it('owner (has clients.view) can read the enriched client list', async () => {
