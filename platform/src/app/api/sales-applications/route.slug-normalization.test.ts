...
/**
 * Sales-applications POST — tenant_slug resolver-twin hardening.
 *
 * Same bug class as portal/auth's: this public application-form route
 * hand-rolls its own `tenants.slug` lookup instead of the shared resolver,
 * so it never inherited the resolver's `.toLowerCase()` normalization or its
 * maybeSingle()+explicit-error-check masked-error fix. A mixed-case
 * tenant_slug posted directly in the body (not via the always-lowercase
 * x-tenant-slug header) silently 404'd for a real, active tenant.
 */
...
const APPLICANT = {
  name: 'Pat', email: 'pat@example.com', phone: '5551234567',
  location: 'NYC', video_url: 'https://example.com/v.mp4',
}

describe('sales-applications POST — tenant_slug case normalization', () => {
...
describe('sales-applications POST — masked tenant-lookup DB error surfaces loud', () => {
