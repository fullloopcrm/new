...
export async function POST(request: Request) {
  ...
  // Unauthenticated + no rate limit == an anonymous caller could loop this to
  // spam waitlist inserts and admin SMS/notify sends. In-memory Map didn't
  // survive serverless cold starts (each new lambda instance reset the
  // counter), same gap fixed in track/public-upload — swap to the DB-backed
  // limiter so it actually persists across invocations.
