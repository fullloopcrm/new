...
export async function POST(request: Request) {
  ...
  // Two layers, both failClosed (auth-critical: a DB error denies, never opens).
  // Per-IP (5/10min) stops a single host guessing PINs. A wrong PIN resolves to
  // no account, so per-victim lockout isn't possible here — instead a per-tenant
  // cap (100/10min) locks out distributed PIN-spraying that rotates IPs, which
  // the per-IP bucket alone can't see. 100/10min sits far above any real login
  // volume for a tenant but far below what brute-forcing a 6-digit PIN needs;
  // bump it for unusually high-traffic tenants if false 429s appear.
