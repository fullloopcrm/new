 ...
 * The referrer-facing UI reads its dashboard from the Bearer-token-gated
 * GET /api/referrers/[code] instead (see src/app/site/referral/page.tsx),
 * but the old unauthenticated route was only ever rate-limited, never
 * actually closed. Probe: an unauthenticated caller must be rejected before
 * any referrer data is looked up or returned.
 */
