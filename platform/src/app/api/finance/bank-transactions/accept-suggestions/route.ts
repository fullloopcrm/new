  ...
  try {
    ...
    for (const t of txns) {
      ...
      try {
        ...
        // existence check on coa_id too means a suggestion that lands on an
        // already-learned pattern with a DIFFERENT coa_id never matches the
        // existing row, falls into the insert branch, and 23505s on the
        // 2-column unique index -- silently, since this call never captured
        // the write's error either. Look up by (tenant_id, pattern) only;
        // same coa_id reaffirms (increment hit_count), a different coa_id
        // corrects it (overwrite coa_id, reset hit_count to 1).
        ...
        if (pattern) {
          ...
          const { error: patternErr } = existing
            ? await supabaseAdmin
                .from('categorization_patterns')
                .update(
                  existing.coa_id === t.suggested_coa_id
                    ? { hit_count: (existing.hit_count || 0) + 1, last_used_at: new Date().toISOString() }
                    : { coa_id: t.suggested_coa_id, hit_count: 1, last_used_at: new Date().toISOString() },
                )
                .eq('id', existing.id)
            : await supabaseAdmin.from('categorization_patterns').insert({
                tenant_id: tenantId, pattern, coa_id: t.suggested_coa_id, hit_count: 1,
              })
          if (patternErr) console.error('[accept-suggestions] failed to update categorization_patterns', patternErr)
        }
        accepted++
      } catch (e) {
