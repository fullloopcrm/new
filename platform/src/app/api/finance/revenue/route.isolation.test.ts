...
// period=month resolves dateFrom to the 1st of the REAL current calendar
// month (`new Date()` in route.ts), and the seed rows are pinned to
// 2026-07-10/11. Without a pinned system time this test only passes while
// the suite happens to run in July 2026 — pin "now" inside that month so
// the fixture stays in-window regardless of when this actually runs.
beforeAll(() => {
  vi.useFakeTimers()
