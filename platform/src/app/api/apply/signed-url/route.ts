/**
 * Pre-signed upload URL for public job-application uploads (video / resume /
 * photo). Tenant resolved from host. Mirrors the response shape of
 * /api/management-applications/signed-url so the tenant career forms can PUT
...
export async function POST(request: NextRequest) {
  ...
  const rl = await rateLimitDb(`apply_signed:${tenant.id}:${ip}`, 10, 10 * 60 * 1000)
  if (!rl.allowed) {
  ...
  try {
    ...
    const path = `${tenant.id}/applications/${config.folder}/${timestamp}-${randomId}.${ext}`
    ...
    if (error || !data) {
      console.error('[apply signed-url] error:', error)
    ...
    console.error('[apply signed-url] error:', err)
    return NextResponse.json({ error: 'Failed to create upload URL' }, { status: 500 })
  }
}
