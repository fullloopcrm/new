...
/**
 * GET /api/team-members/[id]/stripe-onboard called getTenantForRequest() with
 ...
 * status (charges/payouts enabled, details submitted, account id) for any
...
vi.mock('stripe', () => {
  class MockStripe {
    accounts = {
      retrieve: async () => ({ id: 'acct_1', charges_enabled: true, payouts_enabled: true, details_submitted: true }),
    }
  }
...
const getReq = () => new Request('http://x/api/team-members/tm-1/stripe-onboard') as unknown as NextRequest
const ctx = { params: Promise.resolve({ id: 'tm-1' }) }

describe('GET /api/team-members/[id]/stripe-onboard — permission gate', () => {
