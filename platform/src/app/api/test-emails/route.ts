...
export async function POST() {
  ...
  const templates: Array<{ name: string; fn: () => { subject: string; html: string } | string }> = [
    ...
    { name: '5. Daily Ops Recap', fn: () => ({ subject: '[TEST] Daily Ops Recap', html: dailyOpsRecapEmail({ ...base, todayDate: new Date().toLocaleDateString(), tomorrowDate: new Date(Date.now() + 86400000).toLocaleDateString(), todayJobs: [], tomorrowJobs: [], todayRevenue: '$500', todayLabor: '$150', todayProfit: '$350', todayJobCount: 0, tomorrowJobCount: 0, todayPaid: 0, todayUnpaid: 0 }) }) },
