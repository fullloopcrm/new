...
export async function POST() {
  ...
  const templates: Array<{ name: string; fn: () => { subject: string; html: string } | string }> = [
    ...
    { name: '5. Daily Ops Recap', fn: () => ({ subject: '[TEST] Daily Ops Recap', html: dailyOpsRecapEmail({ ...base, todayDate: new Date().toLocaleDateString('en-US', { timeZone: 'America/New_York' }), tomorrowDate: new Date(Date.now() + 86400000).toLocaleDateString(), todayJobs: [], tomorrowJobs: [], todayRevenue: '$500', todayLabor: '$150', todayProfit: '$350', todayJobCount: 0, tomorrowJobCount: 0, todayPaid: 0, todayUnpaid: 0 }) }) },
    ...
    { name: '7. Notification Digest', fn: () => ({ subject: '[TEST] Digest', html: notificationDigestEmail({ ...base, date: new Date().toLocaleDateString('en-US', { timeZone: 'America/New_York' }), emailCount: 3, smsCount: 1, entries: [{ type: 'booking', recipient: 'test@x', time: '1h ago', channel: 'email' }] }) }) },
    { name: '8. Payment Receipt', fn: () => ({ subject: '[TEST] Payment Receipt', html: paymentReceiptEmail({ ...base, clientName: 'Test Client', amount: '$150', date: new Date().toLocaleDateString('en-US', { timeZone: 'America/New_York' }), paymentMethod: 'Stripe', serviceName: 'Standard' }) }) },
