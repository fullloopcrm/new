 ...
 * with no do_not_service check. do_not_service is a stronger kill-switch than
 * sms_consent (see route.do-not-service.test.ts in bookings/route.ts for the
 * full rationale) — this is the admin-facing update/delete path, reachable
 * even though the client-facing self-service reschedule path
 * (client/reschedule/[id]/route.ts) already blocks do_not_service clients at
 * the auth layer via protectClientAPI().
 */
...
const bookingsStore = [{
  ...
  clients: { name: 'Own Client', phone: '+15551234567', sms_consent: true as boolean | null, do_not_service: false as boolean | null },
...
beforeEach(() => {
  ...
  bookingsStore[0].clients.do_not_service = false
  bookingsStore[0].team_members.sms_consent = true
  notify.mockClear()
  sendSMS.mockClear()
})

describe('PUT /api/bookings/[id] — do_not_service gate', () => {
  it('does not SMS the client on confirm when flagged do_not_service', async () => {
    bookingsStore[0].clients.do_not_service = true
    const res = await PUT(req({ status: 'scheduled' }), params)
    expect(res.status).toBe(200)
    expect(sendSMS).not.toHaveBeenCalled()
  })

  it('SMS the client on confirm when not flagged do_not_service', async () => {
    const res = await PUT(req({ status: 'scheduled' }), params)
    expect(res.status).toBe(200)
    expect(sendSMS).toHaveBeenCalledTimes(1)
  })

  it('does not SMS the client on reschedule when flagged do_not_service', async () => {
    bookingsStore[0].clients.do_not_service = true
  ...
  it('SMS the client on reschedule when not flagged do_not_service', async () => {
    const res = await PUT(req({ start_time: '2026-08-02T10:00:00Z' }), params)
    expect(res.status).toBe(200)
    expect(sendSMS).toHaveBeenCalledTimes(1)
  })
...
describe('DELETE /api/bookings/[id] — do_not_service gate', () => {
  it('does not SMS the client on cancellation when flagged do_not_service', async () => {
    bookingsStore[0].clients.do_not_service = true
  ...
  it('SMS the client on cancellation when not flagged do_not_service', async () => {
    const res = await DELETE(new Request('https://x'), params)
    expect(res.status).toBe(200)
    expect(sendSMS).toHaveBeenCalledTimes(1)
  })
