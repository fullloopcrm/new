...
export async function POST(request: Request) {
  ...
  const jobDate = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' , timeZone: 'America/New_York' })
  const jobTime = new Date(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'America/New_York' })
  const endTime = booking.end_time ? new Date(booking.end_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' , timeZone: 'America/New_York' }) : ''
      ...
      const broadcastHtml = `
        <div style="font-family: sans-serif; max-width: 500px;">
          ...
          <div style="background: #fef2f2; padding: 20px; border: 2px solid #fecaca;">
            ...
            <p style="margin: 5px 0;"><strong>Location:</strong> ${client?.address ? escapeHtml(client.address) : 'TBD'}</p>
