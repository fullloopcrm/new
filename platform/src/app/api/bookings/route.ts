...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = db
      .from('bookings')
      .select('*, clients(name, phone, address, customer_number), team_members!bookings_team_member_id_fkey(name, phone), client_properties(*), booking_team_members(is_lead, position, team_members(name))', { count: 'exact' })
