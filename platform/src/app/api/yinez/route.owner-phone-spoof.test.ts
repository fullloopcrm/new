...
/**
 * POST /api/yinez is fully unauthenticated (public web-chat widget) — unlike
 ...
 * forwarded verbatim into askSelena, which feeds it into isOwnerOfTenant() to
 * decide whether the caller gets owner-gated tools (Stripe refunds, SMS
 * broadcasts to every client, revenue data, settings writes). Anyone who knew
 * or guessed a tenant's registered owner_phone could paste it into this
 * endpoint's request body and be granted full admin-tool access with zero
 * authentication. FIX: strip a caller-supplied phone that would pass
 * isOwnerOfTenant() (checked against the verified tenant) before it ever
 * reaches the agent.
 */

const SECRET = 'yinez-route-owner-phone-spoof-test-secret'
const TENANT = 'tenant-yinez-owner-spoof'
  ...
  function makeBuilder(table: string) {
    ...
    const builder: any = {
      ...
      then: (resolve: (v: { count: number; error: null; data: unknown }) => unknown) =>
        resolve({ count: 0, error: null, data: table === 'clients' ? [] : null }),
      insert: () => ({
...
describe('POST /api/yinez — unauthenticated owner-phone spoofing is blocked', () => {
