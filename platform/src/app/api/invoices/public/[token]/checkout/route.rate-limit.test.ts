/**
 * invoices/public/[token]/checkout/route.ts — missing rate limiting.
 *
 ...
 * public_token could be replayed to spam Stripe session creation — unlike
 * the sibling public forms (feedback, leads), which already rate-limit.
 */
...
describe('POST /api/invoices/public/[token]/checkout — rate limiting', () => {
  it('is rate-limited per-token and rejects (no Stripe session created) once the bucket is exhausted', async () => {
    ...
    expect(rateLimitDb).toHaveBeenCalledWith('invoice-checkout:tok-abc', 10, 10 * 60 * 1000)
  })
