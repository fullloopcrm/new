/**
 * POST /api/invoices/public/[token]/checkout is unauthenticated (token-auth)
 * and, on every call, creates a real Stripe Checkout Session against the
 * tenant's own connected account -- had no rate limiting at all. Fixed with
 * the same rateLimitDb bucket pattern used on the sibling quote
...
describe('POST /api/invoices/public/[token]/checkout — rate limiting', () => {
  it('rejects with 429 once the per-IP bucket is exhausted, before touching the DB/Stripe', async () => {
    ...
    expect(rateLimitDb).toHaveBeenCalledWith(
      expect.stringContaining('invoice-checkout:1.2.3.4'),
      10,
      60 * 1000
    )
  })
