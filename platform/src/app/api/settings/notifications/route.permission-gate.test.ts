...
/**
 * requirePermission gate probe — settings/notifications/route.ts GET.
 ...
 * settings.view) — it hands back the tenant's comms preferences/templates
 * and timing config. Any authenticated tenant member — including staff,
 * which has no settings.view per rbac.ts — could pull that configuration.
...
describe('GET /api/settings/notifications — settings.view permission gate', () => {
  it('allowed with settings.view, forbidden without', async () => {
    const ok = await notificationsGET()
    expect(ok.status).not.toBe(403)

    deny()
    const denied = await notificationsGET()
