...
/**
 * POST /api/portal/connect (client "Connect" channel, feeds connect_messages)
 * had no cap on `body` — same uncapped-conversational-input class as
 * team-portal/connect. Same MAX_MESSAGE_LENGTH=4000 convention.
 */
...
beforeEach(() => {
  DB.connect_channels = [{ id: 'chan-a', tenant_id: TENANT, type: 'client', client_id: CLIENT_ID }]
  DB.connect_messages = []
  DB.connect_read_cursors = []
  DB.clients = [{ id: CLIENT_ID, tenant_id: TENANT, name: 'A Own' }]
})

function req(body: unknown): NextRequest {
  const token = createToken(CLIENT_ID, TENANT)
  return new NextRequest('https://x/api/portal/connect', {
    method: 'POST',
...
describe('POST /api/portal/connect — body length cap', () => {
  ...
  it('allows a normal-sized body through to connect_messages', async () => {
    const res = await POST(req({ body: 'hello there' }))
