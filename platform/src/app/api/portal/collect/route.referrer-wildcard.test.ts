/**
 * REFERRAL-ATTRIBUTION WILDCARD INJECTION — same bug as client/collect: the
 * referrer-by-name lookup embedded the raw, unauthenticated `referrer_name`
 * field into an `ilike('name', '%'+name+'%')` pattern with no escaping. `%`
 * and `_` are ilike metacharacters: a submitter typing "%" as the referrer
 * name matched EVERY active referrer in the tenant (first row wins), silently
 * attributing a stranger's booking -- and the commission `team-portal/checkout`
 * later pays out on it -- to an arbitrary real referrer who never referred
 * anyone. This suite proves a `%`-only name can no longer resolve to a
 * referrer, while a genuine partial-name match still works exactly as before.
 * (`_` is the other ilike metacharacter in real Postgres, but the in-repo
 * Supabase fake used here doesn't model single-char wildcard matching, so it
 * can't be meaningfully red/green-tested against this harness -- escapeLike()
 * still covers it in production.)
 */
