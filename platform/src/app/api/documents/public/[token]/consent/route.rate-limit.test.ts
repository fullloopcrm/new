/**
 * POST /api/documents/public/[token]/consent is unauthenticated (per-signer
...
describe('POST /api/documents/public/[token]/consent — rate limiting', () => {
  it('rejects with 429 once the per-IP bucket is exhausted, before touching the DB', async () => {
    ...
    expect(rateLimitDb).toHaveBeenCalledWith(
      expect.stringContaining('document-consent:1.2.3.4'),
      15,
      60 * 1000
    )
  })
