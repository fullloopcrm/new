...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    // status='completed' + writes the signed PDF) can land in the gap.
    // Re-assert the pre-read status in THIS update's own WHERE so a void
    // click racing a signer's final signature can't silently revert an
