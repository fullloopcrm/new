 ...
 * referrer's own name/email/referral_code via the join. The live referrer
 * portal (src/app/site/referral/page.tsx) fetches its dashboard from the
 * Bearer-token-gated GET /api/referrers/[code] instead, which already scopes
 * commissions to the verified referrer server-side — nothing in-repo calls
 * this endpoint with referrer_id. Probe: an unauthenticated caller must be
 * rejected before any commission data is looked up or returned.
 */
