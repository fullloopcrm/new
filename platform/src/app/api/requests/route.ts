...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: fields, error: vError } = validate(body, {
      ...
      billing_address: { type: 'string', max: 200 },
      billing_city: { type: 'string', max: 100 },
      billing_state: { type: 'string', max: 2 },
      billing_zip: { type: 'string', max: 10 },
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
      ...
      <div style="text-align: center; margin-top: 30px;">
        <a href="https://homeservicecrm.ai/admin/requests" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 14px;">View in Admin Dashboard</a>
      </div>
