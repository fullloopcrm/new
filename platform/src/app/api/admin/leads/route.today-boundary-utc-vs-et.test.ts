 ...
 * visitor count built `startOfToday` via `new Date().getFullYear()/
 * getMonth()/getDate()` -- the SERVER's local calendar (UTC on Vercel), a
 * full day ahead of ET for ~4-5h every evening. During that window
 * `startOfToday` sat a full day LATER than the true ET-midnight boundary,
 * silently excluding a visit from earlier that ET evening out of "today"'s
 * count on this marketing analytics dashboard.
 *
...
describe('GET /api/admin/leads — "today" visitor count must use ET, not server-local', () => {
  beforeEach(() => {
    DB.website_visits = [
      {
        id: 'v-5pm-et',
  ...
  it('still counts a visit from earlier tonight ET toward "today"', async () => {
    ...
    expect(body.dashboard.today).toBe(1)
  })
})
