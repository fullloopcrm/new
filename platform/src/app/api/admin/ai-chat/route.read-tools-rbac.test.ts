...
/**
 * Sibling of ai/assistant/route.ts (fixed in 37cb395f) with the exact same
 * gap: the read-only tools (search_clients, search_team_members,
 * query_bookings, get_schedule_summary, get_client_details) had no entry in
 * TOOL_PERMISSIONS at all -- unlike update_bookings/cancel_bookings/
 * update_client/create_booking/get_revenue_stats. This route is live --
 * called from the wash-and-fold-nyc/wash-and-fold-hoboken/nyc-mobile-salon
 * tenant-clone dashboards' AiAssistant.tsx widget, which has no client-side
 * role gate -- so TOOL_PERMISSIONS is the only enforcement point: any
 * tenant member could ask the assistant to search clients/team members and
 * get full PII back, bypassing the same clients.view/team.view/
 * bookings.view RBAC overrides already enforced on the equivalent REST
 * endpoints (GET /api/clients, /api/team, /api/bookings).
 */
...
describe('POST /api/admin/ai-chat — read-tool RBAC gate', () => {
  beforeEach(() => {
    store.clients = [{ id: 'c1', tenant_id: TENANT, name: 'Jane Doe', email: 'jane@x.com' }]
    store.team_members = [{ id: 'tm1', tenant_id: TENANT, name: 'Bob', phone: '+15550001111', status: 'active' }]
  ...
  it('blocks a permission-less role from search_team_members (phone leak)', async () => {
    actorRole = 'nonexistent-role-with-no-perms'
    currentMock = mockAssistantFor('search_team_members', {})
    const res = await POST(req())
    expect(res.status).toBe(200)
    expect(capturedToolResult).toContain("don't have permission")
    expect(capturedToolResult).not.toContain('+15550001111')
  })
  ...
  it('allows staff (has team.view) to use search_team_members', async () => {
    ...
    const res = await POST(req())
    expect(res.status).toBe(200)
    expect(capturedToolResult).toContain('+15550001111')
  })
