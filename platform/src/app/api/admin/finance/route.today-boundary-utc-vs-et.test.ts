 ...
 * real (ET) current month out of this cross-tenant admin revenue widget.
 *
...
describe('GET /api/admin/finance — "month" boundary must use ET, not server-local', () => {
  beforeEach(() => {
    DB.bookings = [
      {
        id: 'bk-jan15',
  ...
  it('still counts a booking paid earlier in the ET-current month', async () => {
    const req = new NextRequest('https://x/api/admin/finance?period=month')
