...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  let query = supabaseAdmin.from('tenants').select('id, name, owner_email, email').neq('status', 'deleted')
  ...
  const recipients = (tenants || [])
    .map((t) => ({ name: t.name as string, email: (t.owner_email || t.email) as string | null }))
    .filter((t): t is { name: string; email: string } => !!t.email)
  ...
  for (const r of recipients) {
    ...
    try {
      ...
      console.error(`[company-campaigns] send failed for ${r.email}:`, e)
    }
  }
