...
export async function GET(request: Request) {
  ...
  // Verify the signed state (CSRF): only our own /auth init can mint a state
  // that binds a Google account to this tenant. A forged/expired state → reject.
  ...
  if (error) {
    return NextResponse.redirect(`${baseUrl}/admin/google-profile?error=${error}`)
  }

  if (!code) {
    return NextResponse.redirect(`${baseUrl}/admin/google-profile?error=no_code`)
  }

  if (!tenantId) {
    return NextResponse.redirect(`${baseUrl}/admin/google-profile?error=bad_state`)
  }
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(`${baseUrl}/admin/google-profile?error=token_exchange_failed`)
  }
  ...
  return NextResponse.redirect(`${baseUrl}/admin/businesses/${tenantId}?google=connected`)
}
