...
// Generic self-returning chainable stub -- these tests only exercise the
// auth gate, so the exact query shape doesn't matter.
...
describe('admin/selena/sms-status auth — URL query-param key is rejected', () => {
  it('falls through to the (mocked-401) session path when the key is ONLY a URL query param', async () => {
    const res = await GET(req(`https://x.test/api/admin/selena/sms-status?key=${MONITOR_KEY}&tenant_id=${TENANT_ID}`))
...
describe('admin/selena/sms-status auth — fails closed', () => {
  it('401s (via session fallback) with no credentials at all', async () => {
    const res = await GET(req('https://x.test/api/admin/selena/sms-status'))
    expect(res.status).toBe(401)
  })
})
