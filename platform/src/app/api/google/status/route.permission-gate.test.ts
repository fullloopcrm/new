...
/**
 * requirePermission gate probe — google/status/route.ts GET.
 ...
 * has no campaigns.view per rbac.ts — could read Google Business connection
 * status and review stats. Proves GET now requires campaigns.view and
...
describe('GET /api/google/status — campaigns.view permission gate', () => {
  it('allowed with campaigns.view, forbidden without', async () => {
    const ok = await statusGET()
    expect(ok.status).not.toBe(403)

    deny()
    const denied = await statusGET()
