...
import { checkAvailability, getSmartSuggestions, checkCleanerAvailability } from '@/app/site/nyc-mobile-salon/_lib/availability'
import { notify } from '@/app/site/nyc-mobile-salon/_lib/notify'
...
async function createOrLinkClient(name: string, conversationId: string): Promise<void> {
  try {
    ...
    const { data: client } = await supabaseAdmin
      .from('clients').insert({ name, phone, status: 'potential', pin: String(crypto.randomInt(100000, 1000000)) }).select('id').single()
...
async function handleCreateBooking(input: Record<string, unknown>, conversationId: string, result: SelenaResult): Promise<string> {
  try {
    ...
    // uses. Also normalize an explicit 'one_time' input (not just a missing
    // one) and bare 'monthly' (RecurringType has no bare 'monthly', only
    // monthly_date/monthly_weekday) -- same defense-in-depth core.ts's own
    // sibling handler applies, since an LLM isn't bound by this tool's
    // input_schema the way a form's <select> is.
