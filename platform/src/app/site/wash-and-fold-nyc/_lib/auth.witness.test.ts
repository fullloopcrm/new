...
beforeEach(() => {
  process.env.ADMIN_PASSWORD = 'wf-nyc-test-secret'
  process.env.CRON_SECRET = 'wf-nyc-cron-secret'
})

describe('wash-and-fold-nyc auth — session cookie signature (constant-time)', () => {
...
describe('wash-and-fold-nyc auth — client session signature (constant-time)', () => {
...
describe('wash-and-fold-nyc auth — protectCronAPI (constant-time)', () => {
  ...
  it('rejects a same-length wrong secret', async () => {
    ...
    const res = protectCronAPI(req('Bearer wf-nyc-cron-secreX'))
  ...
  it('control: the real CRON_SECRET bearer header passes', async () => {
    const { protectCronAPI } = await import('./auth')
    const res = protectCronAPI(req('Bearer wf-nyc-cron-secret'))
    expect(res).toBeNull()
  })
})
