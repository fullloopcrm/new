...
export default function ReferralSignupForm() {
  ...
  if (success) {
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
        <div className="w-16 h-16 bg-[#4BA3D4]/30 rounded-full flex items-center justify-center mx-auto mb-4">
        ...
        <div className="bg-[#F0F8FF] border border-[#4BA3D4]/30 rounded-xl p-6 mb-6">
          ...
          <p className="font-[family-name:var(--font-bebas)] text-4xl text-[#1a3a5c] tracking-wide">{refCode}</p>
        </div>

        <div className="bg-gray-50 rounded-xl p-6 mb-6">
          <p className="text-xs font-semibold text-gray-400 tracking-[0.2em] uppercase mb-2">Your Referral Link</p>
          <p className="text-sm font-mono text-gray-700 break-all mb-3">https://www.washandfoldnyc.com/book?ref={refCode}</p>
          <button
            onClick={copyLink}
            className="bg-[#4BA3D4] text-[#1a3a5c] px-6 py-2.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[#2B7BB0] transition-colors"
          >
            Copy Link
          </button>
        </div>
        ...
        <Link
          ...
          className="inline-block bg-[#1a3a5c] text-white px-8 py-3.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[#1a3a5c]/90 transition-colors"
        >
          Go to My Dashboard
        </Link>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-8">
      <h3 className="font-[family-name:var(--font-bebas)] text-2xl text-[#1a3a5c] tracking-wide mb-6">Sign Up to Start Earning</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 border border-gray-200 rounded-lg text-gray-900 focus:border-[#4BA3D4] focus:ring-1 focus:ring-[#4BA3D4] outline-none"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#4BA3D4] text-[#1a3a5c] py-3.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[#2B7BB0] transition-colors disabled:opacity-50"
        >
      ...
      <div className="mt-6 pt-6 border-t text-center">
        <p className="text-sm text-gray-500">
          Already a referrer?{' '}
          <Link href="/referral" className="text-[#1a3a5c] font-medium underline underline-offset-2">
