...
import { jobDates } from '@/app/site/long-island-maid/_lib/seo/job-dates'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `https://www.thelongislandmaid.com/careers/commission-sales-partner/${n.slug}` },
    openGraph: {
      title: `Sales Jobs in ${n.name} — Commission / 1099 | The Long Island Maid`,
      description,
      url: `https://www.thelongislandmaid.com/careers/commission-sales-partner/${n.slug}`,
    },
  }
}
...
export default async function NeighborhoodSalesJobPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jobSchema = {
    ...
    description: `<p>The Long Island Maid is hiring a Commission Sales Partner in ${n.name}, ${areaName}${landmarkPhrase}. Sign cleaning clients directly (homeowners, offices, property managers, Airbnb hosts) or sign referrers (doormen, real estate agents, concierges, movers) — and earn <strong>10% recurring commission on every completed cleaning</strong>, for as long as they stay a customer.</p><p>Paid daily via Stripe Connect. 1099 independent contractor, no cap. Tier upgrades to 12% at 50 clients and 15% at 100. Path to Sales Manager with override income. The whole payout system is already live for existing referrers.</p><p>Apply at thelongislandmaid.com/apply/commission-sales-partner or text (516) 202-5900.</p>`,
    identifier: {
      '@type': 'PropertyValue',
      name: 'The Long Island Maid',
      value: `long-island-maid-sales-${n.slug}`,
    },
    datePosted,
    validThrough,
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <section className="bg-[#A8F0DC] rounded-2xl p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="sms:5162025900" className="text-[#1E2A4A]/60 font-medium text-sm hover:text-[#1E2A4A] transition-colors underline underline-offset-4">
              Text us for questions
            </a>
          </div>
        </section>
