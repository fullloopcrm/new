...
export function homepageContent() {
  return {
    ...
    metaDescription: 'Long Island\'s top-rated maid service from $59/hr. House cleaning across Long Island & NJ. Licensed, insured. 5.0★ Rated. (516) 202-5900',
...
const areaIntros: Record<string, string[]> = {
  'nassau-county': [
    'Nassau County runs from village centers like Garden City and Mineola to the waterfront communities of Great Neck and Port Washington — our cleaning team handles every type of home with care and precision.',
    'From Rockville Centre colonials to Manhasset estates, our Nassau County cleaners deliver spotless results every time.',
  ],
}

export function areaContent(area: Area) {
  const intros = areaIntros[area.slug] || areaIntros['nassau-county']
  return {
...
const boroughFlavor: Record<string, (n: Neighborhood) => string> = {
  'nassau-county': (n) => `On Long Island, ${n.name} cleaners handle everything from village colonials to waterfront estates with formal entertaining rooms — the kind of work that's a half-day job, not a 90-minute studio clean.`,
}
...
export function serviceContent(service: Service) {
  return {
    ...
    metaDescription: `Professional ${service.name.toLowerCase()} across Long Island & NJ. ${service.features.slice(0, 2).join(', ')} & more. ${service.priceRange}. 5.0★ Rated. (516) 202-5900`,
...
export function commonServiceFAQs(service: Service): { question: string; answer: string }[] {
  return [
    ...
    { question: 'What areas do you serve?', answer: 'We serve Long Island (Nassau and western Suffolk), Long Island County, and northern New Jersey (Hudson and close-in Bergen). Same rates everywhere — no travel surcharges regardless of location.' },
