...
import AddressAutocomplete from '@/app/site/connecticut-maid/_components/AddressAutocomplete'
import { validateEmail } from '@/app/site/connecticut-maid/_lib/validate-email'
...
function trackBookingEvent(action: string, sessionId: string, extra: Record<string, unknown> = {}) {
  try {
    const body = JSON.stringify({
      domain: typeof window !== 'undefined' ? window.location.hostname : 'theconnecticutmaid.com',
      page: '/book/new',
      action,
...
function BookFormContent() {
  useEffect(() => { document.title = 'Book a Cleaning | The Connecticut Maid' }, [])
  ...
  useEffect(() => {
    ...
    fetch(`/api/client/availability?date=${form.date}&duration=${estimatedHours}`, { signal: ctrl.signal })
      ...
      .then((data: { slots?: { time: string; available: boolean }[]; message?: string; sameDay?: boolean } | null) => {
        ...
        setDayMessage(data.message || (data.sameDay ? 'Same-day bookings need a quick call to confirm — (203) 491-5600.' : ''))
  ...
  async function handleConfirmSubmit() {
    ...
    if (isFullyBooked) {
      try {
        ...
        setError('Network error. Please try again or text (203) 491-5600.')
        setSubmitting(false)
      }
      return
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center">
          ...
          {pin && (
            <div className="bg-[#A8F0DC]/30 border border-[#A8F0DC] rounded-lg p-4 mb-6">
              ...
              <p className="text-xs text-[#1E2A4A]/60 mt-2">Save this — log in at <Link href="/portal/login" className="underline">theconnecticutmaid.com/portal/login</Link></p>
            </div>
          )}
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1E2A4A] to-[#243352] py-10 md:py-16 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <div className="inline-block bg-[#A8F0DC] text-[#1E2A4A] text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-4">The Connecticut Maid Self-Booking System</div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-6 md:p-8 space-y-5">
          ...
          <label className="flex items-start gap-2 cursor-pointer -mt-1">
            ...
            <span className="text-[11px] text-gray-500 leading-relaxed">
              By providing your phone number and clicking &ldquo;Submit,&rdquo; you agree to receive SMS updates and marketing messages from The Connecticut Maid. Message frequency may vary. Standard Message and Data Rates may apply. Reply STOP to opt out. Reply HELP for help. Consent is not a condition of purchase.
            </span>
          </label>
                        ...
                        {waitlistState === 'error' && (
                          <p className="text-[11px] text-red-500 mt-1">
                            {(!form.name.trim() || !form.phone.trim())
                              ...
                              : 'Could not add you — please call (203) 491-5600.'}
                          </p>
                        )}
                      </div>
                    )}
          ...
          <p className="text-center text-xs text-gray-400 mt-3">
            No payment now &middot; You&apos;ll review &amp; confirm the recap on the next step &middot; Or text us at <a href="sms:2034915600" className="text-[#1E2A4A] underline">(203) 491-5600</a>
          </p>
        </form>
      </div>
    </div>
