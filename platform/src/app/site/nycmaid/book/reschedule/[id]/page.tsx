...
// Clients see a 2-hour arrival window, not the exact start. "1:00 PM" → "1:00 PM–3:00 PM".
...
// "1:00 PM" → "13:00" (24h), so it can be concatenated into a parseable datetime.
...
export default function ReschedulePage() {
  ...
  const [booking, setBooking] = useState<{ service_type?: string; recurring_type?: string | null; start_time: string } | null>(null)
  ...
  const handleReschedule = async () => {
    ...
    const startTime = new Date(`${selectedDate}T${to24h(selectedSlot.time)}:00`)
    if (isNaN(startTime.getTime())) {
      setSaving(false)
      alert('Could not read that time slot. Please pick the slot again.')
      return
    }
        ...
        {!selectedDate ? (
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {availableSlots.map((slot) => (
                  ...
                  >
                    <p className="font-semibold text-[#1E2A4A]">{slotWindow(slot.time)}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          <div>
            <div className="bg-white rounded-xl p-4 mb-6">
              ...
              <p className="text-gray-600">{slotWindow(selectedSlot.time)}</p>
            </div>
