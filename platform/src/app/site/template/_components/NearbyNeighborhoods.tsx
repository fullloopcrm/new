...
import { getNeighborhood } from '@/app/site/template/_lib/seo/locations'
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {neighborhoods.map(n => n && (
          <Link
            key={n.slug}
            href={`/${n.urlSlug}`}
            className="p-5 bg-white border border-gray-200 rounded-xl hover:border-[var(--accent)] hover:shadow-md transition-all group"
          >
            <p className="font-semibold text-[var(--brand)] group-hover:text-[var(--brand)]">{n.name}</p>
