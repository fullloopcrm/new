...
export const metadata: Metadata = {
  title: 'The 100 Best Cleaning Tips for NYC Apartments | Your Business',
  description: 'The definitive list — 100 expert cleaning tips for NYC apartments from professional cleaners. Kitchen, bathroom, floors, pets & more. From $59/hr. (555) 555-5555',
  alternates: { canonical: 'https://www.example.com/nyc-maid-and-cleaning-tips-and-advice-by-the-nyc-maid' },
  openGraph: {
    title: '100 Best Cleaning Tips | Your Business',
    description: 'The definitive cleaning tips guide for NYC apartments — 100 expert tips organized by room.',
    url: 'https://www.example.com/nyc-maid-and-cleaning-tips-and-advice-by-the-nyc-maid',
  },
}
    ...
    <>
      <JsonLd data={[
        ...
        breadcrumbSchema([
          ...
          { name: '100 Cleaning Tips', url: 'https://www.example.com/nyc-maid-and-cleaning-tips-and-advice-by-the-nyc-maid' },
        ]),
        faqSchema,
        howToSchema,
      ]} />
      ...
      <section className="bg-gradient-to-b from-[var(--brand)] to-[var(--brand-alt)] py-16 md:py-24">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <p className="text-xs font-semibold text-[var(--accent)] tracking-[0.25em] uppercase mb-4">From the Pros Who Clean NYC Every Day</p>
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <nav className="border border-gray-200 rounded-xl p-6 md:p-8 mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {categories.map(cat => (
              <a key={cat.id} href={`#${cat.id}`} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-[rgb(var(--accent-rgb)/0.2)] hover:text-[var(--brand)] transition-colors">
                <span>{cat.icon}</span>
                <span>{cat.name}</span>
              </a>
            ))}
        ...
        {categories.map(cat => (
          <section key={cat.id} id={cat.id} className="mb-16 scroll-mt-24">
            ...
            {(cat.id === 'bedroom' || cat.id === 'laundry' || cat.id === 'pets') && (
              <div className="bg-[rgb(var(--accent-rgb)/0.15)] border border-[rgb(var(--accent-rgb)/0.3)] rounded-xl p-6 mt-8 flex flex-col md:flex-row items-center justify-between gap-4">
                <p className="text-[var(--brand)] font-medium">Rather have a pro handle it? We start at $59/hr.</p>
                <a href="sms:5555555555" className="border-2 border-[var(--brand)] text-[var(--brand)] px-6 py-2.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[var(--brand)] hover:text-white transition-colors flex-shrink-0">
                  Text (555) 555-5555
                </a>
              </div>
            )}
