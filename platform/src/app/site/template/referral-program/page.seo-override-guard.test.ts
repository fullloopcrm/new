...
/**
 * generateMetadata() (site/template referral-program) — SIGNAL apply-layer
...
vi.mock('@/app/site/template/_lib/content/longform', () => ({
  referralContent: () => ({ title: 'Default Referral Title', metaDescription: 'Default referral description.' }),
}))
...
describe('site/template referral-program generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toBe('Default Referral Title')
    expect(meta.description).toBe('Default referral description.')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/referral-program`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's referral-program metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/referral-program`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
