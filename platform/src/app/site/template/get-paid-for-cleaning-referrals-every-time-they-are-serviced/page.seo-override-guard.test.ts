...
/**
 * generateMetadata() (site/template get-paid-for-cleaning-referrals...) —
...
describe('site/template get-paid-for-cleaning-referrals generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toContain('Get Paid for Cleaning Referrals')
    expect(meta.title).toContain('Test Tenant')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}${PATH}`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
  ...
  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's referral-page metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}${PATH}`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
