...
export function homepageContent(brand: BrandContext = DEFAULT_BRAND) {
  const p = industryProfile(brand.industry)
  if (!p.isCleaning) {
    return {
      title: `${brand.name} — Professional ${p.serviceLabel} in ${brand.city} | 5-Star Rated`,
      metaDescription: `${p.serviceLabel} you can count on in ${brand.city}. Licensed, insured, and trusted by local clients. 5.0★ Rated. Call ${brand.phone}.`,
      h1: `${brand.city}'s Trusted ${p.serviceLabel}`,
      subtitle: `Professional ${p.serviceNoun} across ${brand.city} and the surrounding area. Licensed, insured, and focused on doing the job right the first time.`,
    }
  }
  return {
...
const introTemplates = [
  (n: Neighborhood) => `Looking for a reliable cleaning service in ${n.name}? Your Business has been trusted by ${n.name} residents for years, delivering spotless results in ${n.housing_types[0]}, ${n.housing_types[1]}, and more.`,
...
const serviceIntroTemplates = [
  ...
  (n: Neighborhood, s: Service) => `${n.name} residents trust Your Business for expert ${s.name.toLowerCase()}. We understand the unique ${n.cleaning_challenges[0]} and ${n.cleaning_challenges[1]} that come with cleaning homes in this area.`,
...
const richContentMap: Record<string, ServiceRichContent> = {
  ...
  'same-day-cleaning': {
    ...
    nycTips: [
      { title: 'Text us early for best availability', detail: 'Same-day requests sent before 10am have the best chance of afternoon availability. After noon, same-day availability drops significantly. Text (555) 555-5555 as early as possible for the best shot at a same-day slot.' },
    ...
    faqs: [
      ...
      { question: 'How do I book same-day service?', answer: 'Text (555) 555-5555 as early in the day as possible. Text is fastest — we\'ll confirm availability and estimated arrival time within minutes. Morning requests before 10am have the best availability for afternoon service.' },
    ],
  },
