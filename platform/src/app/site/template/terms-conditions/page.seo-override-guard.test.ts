...
/**
 * generateMetadata() (site/template terms-conditions) — SIGNAL apply-layer
...
describe('site/template terms-conditions generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toBe('Terms & Conditions | Test Tenant')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/terms-conditions`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's terms-conditions metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/terms-conditions`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
