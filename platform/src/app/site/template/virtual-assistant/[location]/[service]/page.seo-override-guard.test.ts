...
/**
 * generateMetadata() (site/template virtual-assistant/[location]/[service]) —
 * SIGNAL apply-layer wiring bug-class probe, same shape as the
 * homepage/[slug] family fixes. Page is noindex (geo x service combo,
 * near-duplicate at national scale) but still consults seo_overrides so an
 * approved fix reaches it once/if a combo is promoted to indexed.
 */

const TENANT_A_URL = 'https://a-tenant.example.com'
const TENANT_B_URL = 'https://b-tenant.example.com'
const LOCATION_SLUG = 'california'
...
describe('site/template virtual-assistant/[location]/[service] generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata({ params: Promise.resolve({ location: LOCATION_SLUG, service: SERVICE_SLUG }) })
  ...
  it('an approved override for this exact url wins over the template default', async () => {
    const url = `${TENANT_A_URL}/virtual-assistant/${LOCATION_SLUG}/${SERVICE_SLUG}`
  ...
  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's VA-geo-service metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/virtual-assistant/${LOCATION_SLUG}/${SERVICE_SLUG}`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata({ params: Promise.resolve({ location: LOCATION_SLUG, service: SERVICE_SLUG }) })
    expect(meta.title).not.toBe('Tenant B Only Title')

    siteConfigUrl = TENANT_B_URL
    const metaB = await generateMetadata({ params: Promise.resolve({ location: LOCATION_SLUG, service: SERVICE_SLUG }) })
