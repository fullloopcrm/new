        ...
        <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && requestCode()} className="w-full px-4 py-3 border rounded-lg text-[var(--brand)]" placeholder="Enter your email" autoFocus />
            </div>
            <button onClick={requestCode} disabled={!email} className="w-full py-3 bg-[var(--brand)] text-white rounded-lg font-medium hover:bg-[rgb(var(--brand-rgb)/0.9)] disabled:opacity-50">Email Me a Login Code</button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-[var(--brand)]">Referrer Portal</h1>
            <p className="text-gray-500 mt-1">We sent a 6-digit code to <span className="font-medium text-[var(--brand)]">{email}</span></p>
          </div>
          {error && <div className="bg-red-50 text-red-600 p-3 rounded-lg mb-4 text-sm">{error}</div>}
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border rounded-lg text-[var(--brand)] text-center text-2xl tracking-[0.4em] font-mono"
                placeholder="000000"
                autoFocus
              />
            </div>
            <button onClick={verifyCode} disabled={code.length < 6} className="w-full py-3 bg-[var(--brand)] text-white rounded-lg font-medium hover:bg-[rgb(var(--brand-rgb)/0.9)] disabled:opacity-50">View My Earnings</button>
