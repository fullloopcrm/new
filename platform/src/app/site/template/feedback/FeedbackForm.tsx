...
function Form({ businessName }: { businessName: string }) {
  const searchParams = useSearchParams()
  const source = searchParams.get('from') || 'Feedback Link'
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 w-full max-w-md">
        <h1 className="text-2xl font-semibold text-[var(--brand)] mb-1">Leave Feedback for {businessName}</h1>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border border-gray-300 rounded-xl text-[var(--brand)] text-sm resize-none focus:outline-none focus:border-[var(--brand)] focus:ring-2 focus:ring-[var(--brand)]/10"
              rows={5}
              required
            />
            ...
            {!anonymous && phone.trim() && (
              <div className="mt-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
                <label className="flex items-start gap-3 cursor-pointer text-[13px] leading-relaxed text-gray-600">
                  ...
                  />
                  <span>
                    By checking this box, I consent to receive transactional text messages from <strong>{businessName}</strong> for follow-up on this feedback. Reply STOP to opt out. Reply HELP for help. Msg frequency may vary. Msg &amp; data rates may apply.
                  </span>
                </label>
              </div>
            )}
...
export default function FeedbackForm({ businessName }: { businessName: string }) {
  return (
    <Suspense>
