...
import { organizationSchema, webSiteSchema, webPageSchema, localBusinessSchema, howToBookSchema, breadcrumbSchema } from '@/app/site/template/_lib/seo/schema'
...
const careerFAQs = [
  ...
  { question: 'How do I apply?', questionEs: '¿Cómo aplico?', answer: 'Apply online at example.com/apply or text (555) 555-5555. We review applications within 24–48 hours and get you working fast.', answerEs: 'Aplica en example.com/apply o envía un texto al (555) 555-5555. Revisamos en 24–48 horas.' },
]


const pageUrl = 'https://www.example.com/available-nyc-maid-jobs'
const pageTitle = 'Cleaning Jobs NYC, Long Island, Westchester & NJ — Starting $30/hr + Bonuses, Open 24/7 | Trabajo de Limpieza'
const pageDescription = 'Hiring cleaners across Manhattan, Brooklyn, Queens, the Bronx, Staten Island, Long Island, Westchester & NJ! Starting $30/hr + bonus programs. Zelle in <30 min. 100% tips. Open 24/7. English & Spanish | Contratando — desde $30/hr + bonos, propinas 100% tuyas. (555) 555-5555'
...
function jobPostingSchema(region: string, locations: string) {
  ...
  return {
    ...
    description: `Now hiring experienced house cleaners in ${locations}. Starting at $30/hr paid via Zelle within 30 minutes of every completed job. Bonus programs available for retention, client satisfaction, and five-star reviews. Flexible schedule. Open 24/7. You provide your own cleaning supplies and equipment. Background check required. Apply at example.com/apply or text (555) 555-5555. Bilingual workplace (English/Spanish). 100% of tips are yours.`,
    identifier: {
      '@type': 'PropertyValue',
      name: 'Your Business',
      ...
      <section className="bg-gradient-to-b from-[var(--brand)] to-[var(--brand-alt)] py-20 md:py-28">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <h1 className="font-[family-name:var(--font-bebas)] text-4xl md:text-6xl lg:text-7xl text-white tracking-wide leading-[0.95] mb-6">
            Join Your Business — Starting at $30/hr, Bonus Programs Available
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5">
            <Link href="/apply" className="bg-[var(--accent)] text-[var(--brand)] px-10 py-4 rounded-lg font-bold text-sm tracking-widest uppercase hover:bg-[var(--accent-hover)] transition-colors">
              Apply Now / Aplica Ahora
            </Link>
            <a href="sms:5555555555" className="text-blue-200/70 font-medium text-lg py-4 hover:text-white transition-colors underline underline-offset-4">
        ...
        <div className="max-w-5xl mx-auto px-4">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-6 text-center">
            <div>
              ...
              <p className="text-[rgb(var(--brand-rgb)/0.6)] text-sm font-medium">Starting Per Hour</p>
              <p className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic">Desde por hora</p>
            </div>
            <div>
              <p className="font-[family-name:var(--font-bebas)] text-4xl text-[var(--brand)] tracking-wide">30 Min</p>
              <p className="text-[rgb(var(--brand-rgb)/0.6)] text-sm font-medium">Pay After Every Job</p>
              <p className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic">Pago después de cada trabajo</p>
            </div>
            <div>
              <p className="font-[family-name:var(--font-bebas)] text-4xl text-[var(--brand)] tracking-wide">Zelle</p>
              <p className="text-[rgb(var(--brand-rgb)/0.6)] text-sm font-medium">Direct to Your Bank</p>
              <p className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic">Directo a tu banco</p>
            </div>
            <div>
              <p className="font-[family-name:var(--font-bebas)] text-4xl text-[var(--brand)] tracking-wide">Bonos</p>
              <p className="text-[rgb(var(--brand-rgb)/0.6)] text-sm font-medium">Performance Programs</p>
              <p className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic">Programas de rendimiento</p>
            </div>
            <div>
              <p className="font-[family-name:var(--font-bebas)] text-4xl text-[var(--brand)] tracking-wide">24/7</p>
              <p className="text-[rgb(var(--brand-rgb)/0.6)] text-sm font-medium">NYC, LI &amp; NJ</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {openings.map(opening => {
              ...
              return (
                <div key={opening.id} className="border border-gray-200 rounded-2xl overflow-hidden hover:border-[var(--accent)] transition-all">
                  <div className="bg-gradient-to-b from-[var(--brand)] to-[var(--brand-alt)] p-6">
                    <p className="text-[var(--accent)] text-xs font-semibold tracking-[0.2em] uppercase mb-1">Now Hiring · {neighborhoods.length} Neighborhoods</p>
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="bg-[rgb(var(--accent-rgb)/0.2)] text-[var(--brand)] text-xs font-semibold px-3 py-1 rounded-full">Pago el Mismo Día</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {neighborhoods.map(n => (
                        <Link key={n.slug} href={`/available-nyc-maid-jobs/${n.slug}`} className="bg-gray-50 text-gray-600 text-xs px-2.5 py-1 rounded-full hover:bg-[rgb(var(--accent-rgb)/0.2)] hover:text-[var(--brand)] transition-colors">{n.name}</Link>
                      ))}
                    </div>
                    <Link href="/apply" className="block text-center bg-[var(--brand)] text-white py-3 rounded-lg font-bold text-sm tracking-widest uppercase hover:bg-[rgb(var(--brand-rgb)/0.9)] transition-colors">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-20">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-gradient-to-b from-[var(--brand)] to-[var(--brand-alt)] rounded-2xl p-8">
              <p className="text-[var(--accent)] text-xs font-semibold tracking-[0.2em] uppercase mb-4">What You Get / Lo Que Recibes</p>
            ...
            <div className="bg-[var(--accent)] rounded-2xl p-8">
              ...
              <p className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic mb-2">Ganancias Semanales Ejemplo</p>
              <p className="text-[rgb(var(--brand-rgb)/0.5)] text-xs mb-3">Avg job: 2.5 hrs &times; $30/hr = $75/job</p>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <div>
                    <span className="text-[rgb(var(--brand-rgb)/0.7)] text-sm">5 jobs/week (part-time)</span>
                    <span className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic block">5 trabajos/semana (medio tiempo)</span>
                  </div>
                  <span className="font-bold text-[var(--brand)]">$375/wk</span>
                </div>
                <div className="flex justify-between items-center">
                  <div>
                    <span className="text-[rgb(var(--brand-rgb)/0.7)] text-sm">10 jobs/week</span>
                    <span className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic block">10 trabajos/semana</span>
                  </div>
                  <span className="font-bold text-[var(--brand)]">$750/wk</span>
                </div>
                <div className="flex justify-between items-center border-t border-[rgb(var(--brand-rgb)/0.1)] pt-3">
                  <div>
                    <span className="text-[rgb(var(--brand-rgb)/0.7)] text-sm">18–20 jobs/week (full-time)</span>
                    <span className="text-[rgb(var(--brand-rgb)/0.4)] text-xs italic block">18–20 trabajos/semana (tiempo completo)</span>
                  </div>
        ...
        {/* Bonus Programs */}
        <section className="mb-20">
          <p className="text-xs font-semibold text-[var(--accent)] tracking-[0.2em] uppercase mb-2">Earn More / Gana Más</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="border-2 border-[var(--accent)] rounded-2xl p-6">
              <div className="w-12 h-12 bg-[rgb(var(--accent-rgb)/0.2)] rounded-full flex items-center justify-center mb-4">
          ...
          {/* Review Tiers */}
          <div className="bg-gradient-to-r from-[var(--brand)] to-[var(--brand-alt)] rounded-2xl p-8 md:p-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div className="bg-white/10 rounded-xl p-5">
                <p className="text-[var(--accent)] text-3xl mb-1">⭐</p>
                ...
                <p className="text-[var(--accent)] text-sm font-bold mt-2">Tier 1 Bonus</p>
              </div>
              <div className="bg-white/10 rounded-xl p-5 ring-1 ring-[rgb(var(--accent-rgb)/0.3)]">
                <p className="text-[var(--accent)] text-3xl mb-1">⭐⭐</p>
                ...
                <p className="text-[var(--accent)] text-sm font-bold mt-2">Tier 2 Bonus</p>
              </div>
              <div className="bg-white/10 rounded-xl p-5 ring-1 ring-[rgb(var(--accent-rgb)/0.5)]">
                <p className="text-[var(--accent)] text-3xl mb-1">⭐⭐⭐</p>
                ...
                <p className="text-[var(--accent)] text-sm font-bold mt-2">Tier 3 Bonus</p>
              </div>
              <div className="bg-white/15 rounded-xl p-5 ring-2 ring-[var(--accent)]">
                <p className="text-[var(--accent)] text-3xl mb-1">👑</p>
                ...
                <p className="text-[var(--accent)] text-sm font-bold mt-2">Max Tier Bonus</p>
              </div>
            </div>
        ...
        <section className="bg-gradient-to-b from-[var(--brand)] to-[var(--brand-alt)] rounded-2xl p-8 md:p-14 mb-20">
          <p className="text-[var(--accent)] text-xs font-semibold tracking-[0.2em] uppercase mb-2">Getting Started / Cómo Empezar</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={item.step}>
                <div className="w-10 h-10 bg-[var(--accent)] text-[var(--brand)] rounded-full flex items-center justify-center font-bold mb-4">{item.step}</div>
        ...
        <section className="mb-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              <p className="text-xs font-semibold text-[var(--accent)] tracking-[0.2em] uppercase mb-2">Your Tools / Tus Herramientas</p>
            ...
            <div className="space-y-6">
              <div className="bg-[var(--accent)] rounded-2xl p-8">
                ...
                <p className="text-[rgb(var(--brand-rgb)/0.7)] text-sm mb-2">We never take a cut of your tips. When a client tips you — cash, Zelle, Venmo, whatever — it&apos;s 100% yours. Always.</p>
                <p className="text-[rgb(var(--brand-rgb)/0.5)] text-xs italic">El 100% de las propinas son tuyas. Nunca tomamos nada de tus propinas. Siempre.</p>
              </div>
              ...
              <div key={area.slug} className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {neighborhoods.map(nb => (
                    <Link key={nb.slug} href={`/available-nyc-maid-jobs/${nb.slug}`} className="px-3 py-1.5 bg-gray-100 rounded-full text-sm text-gray-700 hover:bg-[rgb(var(--accent-rgb)/0.2)] hover:text-[var(--brand)] transition-colors">
                      {nb.name}
                    </Link>
                  ))}
                </div>
        ...
        <section className="mb-16">
          <div className="border-2 border-[var(--brand)] rounded-2xl p-8 md:p-10 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
              <p className="text-[var(--accent)] text-xs font-semibold tracking-[0.2em] uppercase mb-1">Also Hiring</p>
              <p className="font-[family-name:var(--font-bebas)] text-2xl md:text-3xl text-[var(--brand)] tracking-wide">Operations Coordinator — Part-Time, 10% Per Job, $40+/hr Effective</p>
            ...
            <Link href="/careers/operations-coordinator" className="bg-[var(--brand)] text-white px-8 py-3.5 rounded-lg font-bold text-sm tracking-widest uppercase hover:bg-[rgb(var(--brand-rgb)/0.9)] transition-colors whitespace-nowrap flex-shrink-0">
              View Role
            </Link>
          </div>
        </section>
        ...
        <section className="bg-[var(--accent)] rounded-2xl p-8 md:p-12 text-center mb-16">
          ...
          <p className="text-[rgb(var(--brand-rgb)/0.6)] max-w-xl mx-auto mb-2">
            Apply in 2 minutes. We review applications within 24–48 hours. Get working and get paid — starting $30/hr, same day, every job.
          </p>
          <p className="text-[rgb(var(--brand-rgb)/0.4)] text-sm italic max-w-xl mx-auto mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
            <Link href="/apply" className="bg-[var(--brand)] text-white px-10 py-4 rounded-lg font-bold text-sm tracking-widest uppercase hover:bg-[rgb(var(--brand-rgb)/0.9)] transition-colors">
              Apply Now / Aplica Ahora
            </Link>
