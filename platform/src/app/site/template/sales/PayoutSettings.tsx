      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex gap-3 flex-wrap">
            <button onClick={() => setMethod('stripe_connect')} className={`px-4 py-2 rounded-lg text-sm font-medium ${method === 'stripe_connect' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600'}`}>Stripe (instant)</button>
            <button onClick={() => setMethod('zelle')} className={`px-4 py-2 rounded-lg text-sm font-medium ${method === 'zelle' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600'}`}>Zelle</button>
            <button onClick={() => setMethod('apple_cash')} className={`px-4 py-2 rounded-lg text-sm font-medium ${method === 'apple_cash' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600'}`}>Apple Cash</button>
          </div>
        </div>
        ...
        {method === 'stripe_connect' ? (
          <div>
            {stripeConnectAccountId ? (
              ...
              <div>
                <button onClick={connectStripe} disabled={connecting} className="px-4 py-2.5 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 disabled:opacity-50">
          ...
          <div>
            ...
            <input type="text" value={zelle} onChange={(e) => setZelle(e.target.value)} placeholder="you@example.com or (212) 555-1234" className="w-full px-4 py-2.5 border rounded-lg text-gray-900" />
          </div>
        ) : (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Apple Cash phone</label>
            <input type="text" value={appleCash} onChange={(e) => setAppleCash(e.target.value)} placeholder="(212) 555-1234" className="w-full px-4 py-2.5 border rounded-lg text-gray-900" />
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="number" value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="1000" className="w-full px-4 py-2.5 border rounded-lg text-gray-900" />
          </div>
        </div>

        <button onClick={save} disabled={saving} className="px-5 py-2.5 bg-gray-900 text-white rounded-lg font-medium hover:bg-gray-800 disabled:opacity-50">
