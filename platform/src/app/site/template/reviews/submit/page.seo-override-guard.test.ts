...
/**
 * generateMetadata() (site/template reviews/submit) — SIGNAL apply-layer
...
describe('site/template reviews/submit generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toBe('Leave a Review | Test Tenant')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/reviews/submit`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
  ...
  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's reviews/submit metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/reviews/submit`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
