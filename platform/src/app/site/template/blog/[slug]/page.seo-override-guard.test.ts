...
/**
 * generateMetadata() (site/template blog/[slug]) — SIGNAL apply-layer wiring
...
vi.mock('@/app/site/template/_lib/content/longform', () => ({
  blogPosts: () => [
    { slug: SLUG, page: { title: 'Default Post Title', metaDescription: 'Default post description.', h1: 'Post', body: [] } },
  ],
}))
...
describe('site/template blog/[slug] generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    ...
    expect(meta.title).toBe('Default Post Title')
    expect(meta.description).toBe('Default post description.')
  })

  it('an approved override for this exact post url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/blog/${SLUG}`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata({ params: Promise.resolve({ slug: SLUG }) })
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's blog post metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/blog/${SLUG}`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata({ params: Promise.resolve({ slug: SLUG }) })
    expect(meta.title).not.toBe('Tenant B Only Title')

    siteConfigUrl = TENANT_B_URL
    const metaB = await generateMetadata({ params: Promise.resolve({ slug: SLUG }) })
