...
/**
 * generateMetadata() (site/template blog index) — SIGNAL apply-layer wiring
...
describe('site/template blog index generateMetadata() — seo_overrides apply-layer probe', () => {
  ...
  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/blog`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's blog index metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/blog`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
