...
/**
 * generateMetadata() (site/template faq) — SIGNAL apply-layer wiring
...
vi.mock('@/app/site/template/_lib/content/longform', () => ({
  faqContent: () => ({ title: 'Default FAQ Title', metaDescription: 'Default faq description.' }),
}))
...
describe('site/template faq generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toBe('Default FAQ Title')
    expect(meta.description).toBe('Default faq description.')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/faq`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's faq metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/faq`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
