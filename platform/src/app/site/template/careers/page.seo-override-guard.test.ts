...
/**
 * generateMetadata() (site/template careers) — SIGNAL apply-layer wiring
...
vi.mock('@/app/site/template/_lib/content/longform', () => ({
  careersContent: () => ({ title: 'Default Careers Title', metaDescription: 'Default careers description.' }),
}))
...
describe('site/template careers generateMetadata() — seo_overrides apply-layer probe', () => {
  it('uses the template default when no override exists for this url', async () => {
    const meta = await generateMetadata()
    expect(meta.title).toBe('Default Careers Title')
    expect(meta.description).toBe('Default careers description.')
  })

  it('an approved override for this exact url wins over the template default', async () => {
    overridesByUrl[`${TENANT_A_URL}/careers`] = { title: 'Approved Title', description: 'Approved description.' }
    const meta = await generateMetadata()
    expect(meta.title).toBe('Approved Title')
    expect(meta.description).toBe('Approved description.')
  })

  it("wrong-tenant probe: tenant B's applied override never leaks into tenant A's careers metadata", async () => {
    overridesByUrl[`${TENANT_B_URL}/careers`] = { title: 'Tenant B Only Title', description: 'Tenant B only description.' }
    siteConfigUrl = TENANT_A_URL
    const meta = await generateMetadata()
