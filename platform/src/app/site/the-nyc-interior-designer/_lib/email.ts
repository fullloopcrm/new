...
const ADMIN_DOMAINS = ['thenycinteriordesigner.com', 'consortiumnyc.com']

function isAdminEmail(email: string): boolean {
  return ADMIN_DOMAINS.some(d => email.toLowerCase().endsWith(`@${d}`) || email.toLowerCase().includes('thenycinteriordesigner'))
}
...
export async function sendEmail(to: string, subject: string, html: string, attachments?: any[], options?: { bcc?: string | string[]; skipOwnerBcc?: boolean }) {
  ...
  // Auto-BCC owner on all outbound emails to clients (not admin-to-admin)
  let bcc = options?.bcc
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const { data, error } = await resend.emails.send({
        from: `The NYC Interior Designer <${process.env.FROM_EMAIL || 'hello@thenycinteriordesigner.com'}>`,
        to,
        subject,
        html,
        attachments,
