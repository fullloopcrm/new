...
/**
 * verifySessionCookie/protectCronAPI compared an HMAC signature (or the
 * CRON_SECRET bearer header) with a naive `!==`/`===`, which leaks the
 * expected value byte-by-byte via timing — the same class already fixed for
 * CRON_SECRET across cron/admin routes (de510a4e) and the global ADMIN_PIN
 * in /api/admin-auth (413adc6f), just missed on this per-tenant site's own
 * admin session + cron auth. Fixed with the shared `safeEqual()`
 * constant-time compare.
 */

beforeEach(() => {
  process.env.ADMIN_PASSWORD = 'nid-test-secret'
  process.env.CRON_SECRET = 'nid-cron-secret'
})

describe('the-nyc-interior-designer auth — session cookie signature (constant-time)', () => {
...
describe('the-nyc-interior-designer auth — protectCronAPI (constant-time)', () => {
  ...
  it('rejects a same-length wrong secret', async () => {
    ...
    const res = protectCronAPI(req('Bearer nid-cron-secreX'))
  ...
  it('control: the real CRON_SECRET bearer header passes', async () => {
    const { protectCronAPI } = await import('./auth')
    const res = protectCronAPI(req('Bearer nid-cron-secret'))
    expect(res).toBeNull()
  })
})
