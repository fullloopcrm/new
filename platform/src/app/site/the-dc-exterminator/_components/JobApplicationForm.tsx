...
import { track as localTrack, getSessionId } from "@/app/site/the-dc-exterminator/_lib/tracker";
...
export default function JobApplicationForm({
  ...
  useEffect(() => {
    if (!openedRef.current) {
      ...
      localTrack.formOpen("job-application");
    }
    ...
    const onBeforeUnload = () => {
      if (openedRef.current && !submittedRef.current) {
        localTrack.formAbandon("job-application", lastFieldRef.current);
      }
    };
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localTrack.formSuccess("job-application", {
        ...
        setStatus("error");
        localTrack.formError("job-application", { status: res.status });
      }
    } catch {
      setStatus("error");
      localTrack.formError("job-application", { reason: "network" });
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <select id="app-license" name="license" required className={selectClass}>
            ...
            <option value="nys-dec-7a">Yes — DOEE Category 7A</option>
            <option value="nys-dec-other">Yes — other DOEE certification</option>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input type="text" id="app-location" name="location" required defaultValue={neighborhood || ""} className={inputClass} placeholder="e.g. Shaw, Arlington" />
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Your pest control experience, certifications, what kind of role you're looking for, and why you want to join The DC Exterminator..."
        />
      </div>
