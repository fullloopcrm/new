...
import { STATES } from "@/app/site/toll-trucks-near-me/_data/cities";
import { CtaButtons } from "@/app/site/toll-trucks-near-me/_components/CtaButtons";
import { customerTypeContent } from "@/app/site/toll-trucks-near-me/_data/customer-content";
...
export async function generateMetadata({ params }: { params: Promise<{ type: string }> }): Promise<Metadata> {
  ...
  if (!ct) return {};
  return {
    title: `Tow Truck Dispatch for ${ct.name} — Toll Trucks Near Me for Your Stuff`,
    description: `${ct.description} Starting at $95 hookup, flat upfront pricing, flat upfront pricing. ${CITY_COUNT}+ cities.`,
    ...
    <>
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">Tow Truck Dispatch Built for {ct.name}</p>
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
            Tow Truck Dispatch for <span className="gradient-text">{ct.name}</span>
          </h1>
      ...
      <section className="bg-section-white py-16">
        <div className="mx-auto max-w-5xl px-6">
          <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">What {ct.name} Earn in Upfront Quotes</p>
      ...
      <section className="bg-section-white py-16">
        <div className="mx-auto max-w-5xl px-6">
          <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">Find {ct.name} Tow Truck Dispatch in Your State</p>
          <h2 className="mt-3 text-center text-3xl font-bold text-slate-900 font-heading">Tow Truck Dispatch for {ct.name} by State</h2>
          <p className="mx-auto mt-4 max-w-2xl text-center text-base text-slate-600">
            Select your state to find local {ct.shortName.toLowerCase()} tow truck service with <Link href="/pricing" className="text-teal-700 font-semibold hover:underline">our pricing</Link> and credits.
          </p>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
        <div className="absolute inset-0 grid-bg opacity-30" />
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">Book Tow Truck Dispatch for {ct.name} Today</p>
