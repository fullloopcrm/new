...
import { customerCityContent } from "@/app/site/toll-trucks-near-me/_data/customer-content";
...
export async function generateMetadata({ params }: { params: Promise<{ type: string; state: string; city: string }> }): Promise<Metadata> {
  ...
  if (!ct || !result) return {};
  return {
    title: `Tow Truck Dispatch for ${ct.name} in ${result.city.name}, ${result.state.abbreviation}`,
    description: `${ct.name} tow truck service in ${result.city.name}, ${result.state.abbreviation}. ${ct.description} Starting at $95 hookup.`,
    ...
    <>
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">{ct.name} Tow Truck Dispatch in {city.name}</p>
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
            <span className="gradient-text">{ct.name}</span> Tow Truck Dispatch<br />in {city.name}, {state.abbreviation}
          </h1>
      ...
      {nearbyCities.length > 0 && (
        <section className="bg-section-white py-16">
          <div className="mx-auto max-w-5xl px-6">
            <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">{ct.name} Tow Truck Dispatch Near {city.name}</p>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
        <div className="absolute inset-0 grid-bg opacity-30" />
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <h2 className="mt-3 text-center text-3xl font-bold text-white sm:text-4xl font-heading">Book {ct.name} Tow Truck Dispatch in {city.name}</h2>
          <CtaButtons variant="dark" />
        </div>
      </section>
    </>
