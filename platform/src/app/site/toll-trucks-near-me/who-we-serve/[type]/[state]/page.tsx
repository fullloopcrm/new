...
import { CUSTOMER_TYPES } from "@/app/site/toll-trucks-near-me/_data/customer-types";
...
import { customerStateContent } from "@/app/site/toll-trucks-near-me/_data/customer-content";
...
export async function generateMetadata({ params }: { params: Promise<{ type: string; state: string }> }): Promise<Metadata> {
  ...
  if (isServiceSlug(stateOrService)) {
    ...
    return {
      title: `${svc.title} for ${ct.name} — Toll Trucks Near Me`,
      description: `${svc.title} specifically for ${ct.name.toLowerCase()}. ${svc.description} Starting at $95 hookup, flat upfront pricing.`,
  ...
  if (!state) return {};
  return {
    title: `Tow Truck Dispatch for ${ct.name} in ${state.name} — Toll Trucks Near Me`,
    description: `${ct.name} tow truck service in ${state.cities.length} ${state.abbreviation} cities. ${ct.description} Starting at $95 hookup, flat upfront pricing.`,
    alternates: { canonical: `/who-we-serve/${type}/${stateOrService}` },
  };
}
        ...
        <section className="bg-section-white py-16">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <div className="mx-auto mt-8 max-w-3xl space-y-5 text-center text-base leading-relaxed text-slate-700">
              ...
              <p>For {ct.name.toLowerCase()}, {svc.title.toLowerCase()} addresses specific needs: {ct.painPoints.slice(0, 3).join(", ")}. Our crew understands these challenges and delivers accordingly — with the same starting at $95 hookup rate, flat upfront pricing with no surcharge games, and flat upfront pricing on every item worth something.</p>
            </div>
          </div>
        </section>
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
        <div className="absolute inset-0 grid-bg opacity-30" />
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">{ct.name} Tow Truck Dispatch in {state.abbreviation}</p>
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
            Tow Truck Dispatch for {ct.name}<br />in <span className="gradient-text">{state.name}</span>
          </h1>
      ...
      <section className="bg-section-white py-16">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <h2 className="mt-3 text-center text-3xl font-bold text-slate-900 font-heading">{ct.name} Tow Truck Dispatch in {state.name}</h2>
          <p className="mx-auto mt-4 max-w-2xl text-center text-base text-slate-600">
            {state.cities.length} cities, same <Link href="/pricing" className="text-teal-700 font-semibold hover:underline">pricing</Link>, local crews. Browse <Link href={`/who-we-serve/${type}`} className="text-teal-700 font-semibold hover:underline">all {ct.shortName.toLowerCase()} services</Link> or <Link href="/book-tow-truck-now" className="text-teal-700 font-semibold hover:underline">book now</Link>.
          </p>
          <div className="mx-auto mt-8 max-w-3xl space-y-5 text-center text-base leading-relaxed text-slate-700">
      ...
      <section className="bg-section-teal py-16">
        <div className="mx-auto max-w-5xl px-6">
          <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">{ct.name} Tow Truck Dispatch Cities in {state.abbreviation}</p>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
        <div className="absolute inset-0 grid-bg opacity-30" />
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <h2 className="mt-3 text-center text-3xl font-bold text-white sm:text-4xl font-heading">Book {ct.name} Tow Truck Dispatch in {state.name}</h2>
          <CtaButtons variant="dark" />
        </div>
      </section>
    </>
