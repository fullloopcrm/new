...
export function BookingForm({ variant = "default" }: { variant?: "default" | "hero" | "dark" }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <label className={`block text-sm font-semibold mb-1 font-cta ${isDark ? "text-white/80" : "text-slate-700"}`}>Vehicle & situation</label>
        <textarea
          name="details"
          rows={3}
          placeholder="e.g. 2019 Honda Civic, dead battery in driveway, need tow to Firestone on Main St"
          className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500"
        />
      </div>
      {error && (
      ...
      >
        {submitting ? "Sending..." : "Dispatch a Tow — We'll Call You"}
      </button>
      <p className={`mt-2 text-center text-xs ${isDark ? "text-white/40" : "text-slate-400"}`}>
        24/7 live dispatch &bull; Flat upfront pricing &bull; 30-min arrival option &bull; Licensed & insured
      </p>
    </form>
  );
}
