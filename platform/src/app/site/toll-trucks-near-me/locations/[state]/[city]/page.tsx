...
import { STATES, getTopCitiesPerState, getCityBySlug, getStateBySlug } from "@/app/site/toll-trucks-near-me/_data/cities";
import { SERVICES } from "@/app/site/toll-trucks-near-me/_data/services";
import { cityPageContent } from "@/app/site/toll-trucks-near-me/_data/content-templates";
import { getOfficeByState } from "@/app/site/toll-trucks-near-me/_data/offices";
import { OfficeBlock } from "@/app/site/toll-trucks-near-me/_components/OfficeBlock";

function isStateGuide(slug: string) {
  return slug.includes("tow-truck-in-") && slug.endsWith("-guide-rates-arrival");
}
...
export async function generateMetadata({ params }: { params: Promise<{ state: string; city: string }> }): Promise<Metadata> {
  ...
  if (isStateGuide(citySlug)) {
    ...
    if (!state) return {};
    return {
      title: `Tow Truck Dispatch in ${state.name} — Complete Guide, Tips & Costs`,
      description: `Everything ${state.name} residents need to know about tow truck service. Local tips, pricing, what items earn you credit, and how to save in ${state.abbreviation}.`,
      ...
      <>
        <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
          ...
          <div className="relative mx-auto max-w-5xl px-6 text-center">
            <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">Expert Tow Truck Dispatch Guide for {state.abbreviation} Residents</p>
            <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
              Tow Truck Dispatch in {state.name}<br /><span className="gradient-text">Guide, Tips &amp; Costs</span>
            </h1>
        ...
        <section className="bg-section-white py-16">
          <div className="mx-auto max-w-5xl px-6">
            <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">What {state.name} Residents Need to Know About Tow Truck Dispatch</p>
            <h2 className="mt-3 text-center text-3xl font-bold text-slate-900 font-heading">The Complete Guide to Tow Truck Dispatch in {state.name}</h2>
            ...
            <div className="mx-auto mt-8 max-w-3xl space-y-5 text-center text-base leading-relaxed text-slate-700">
              <p>Here&apos;s the industry secret every {state.name} resident should know: <strong>every tow truck company in {state.abbreviation} has been charging you for the service AND keeping 35%+ of your items to resell for profit.</strong> Toll Trucks Near Me is the only company in {state.name} that credits you 50% (when applicable) of resale value. Starting at $95 hookup with flat upfront pricing with no surcharge games.</p>
        ...
        <section className="bg-section-teal py-16">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <div className="mx-auto mt-8 max-w-3xl space-y-5 text-center text-base leading-relaxed text-slate-700">
              <p><strong>Furniture ($50–$2,000+):</strong> Solid wood dining sets, dressers, desks, leather sofas. <Link href="/services/flatbed-towing" className="text-teal-700 font-semibold hover:underline">Furniture removal</Link> is our most-requested service in {state.abbreviation}.</p>
              <p><strong>Appliances ($75–$600+):</strong> Working refrigerators, washers, dryers, ovens. <Link href="/services/flatbed-towing" className="text-teal-700 font-semibold hover:underline">Appliance removal</Link> frequently covers the entire bill.</p>
        ...
        <section className="bg-section-white py-16">
          <div className="mx-auto max-w-5xl px-6">
            <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">City-Specific Tow Truck Dispatch Guides in {state.abbreviation}</p>
            ...
            <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
              {topCities.map((c) => (
                <Link key={c.slug} href={`/locations/${stateSlug}/${c.slug}/tow-truck-in-${c.slug}-guide-rates-arrival`}
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
          <div className="absolute inset-0 grid-bg opacity-30" />
          <div className="relative mx-auto max-w-5xl px-6 text-center">
            <h2 className="mt-3 text-center text-3xl font-bold text-white sm:text-4xl font-heading">Book Tow Truck Dispatch in {state.name} Today</h2>
            <CtaButtons variant="dark" />
          </div>
        </section>
      </>
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          ...
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
            Tow Truck Dispatch in <span className="gradient-text">{city.name}, {state.abbreviation}</span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-white/80">
            The only tow truck company in {city.name} that pays you. starting at $95 hookup, flat upfront pricing, flat upfront pricing.
          </p>
          <CtaButtons variant="dark" />
        </div>
      </section>
      ...
      <section className="bg-section-white py-16">
        <div className="mx-auto max-w-5xl px-6">
          <h2 className="text-center text-3xl font-bold text-slate-900 font-heading">
            {SERVICES.length} Tow Truck Services in {city.name}, {state.abbreviation}
          </h2>
      ...
      {content.nearbyCities.length > 0 && (
        <section className="bg-section-teal py-16">
          <div className="mx-auto max-w-5xl px-6">
            <h2 className="text-center text-3xl font-bold text-slate-900 font-heading">
              Tow Truck Dispatch Near {city.name}
            </h2>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <h2 className="text-center text-3xl font-bold text-white sm:text-4xl font-heading">
            Book Tow Truck Dispatch in {city.name} Today
          </h2>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-white/80">
            Same-day available. starting at $95 hookup. Flat upfront pricing with no surcharge games. flat upfront pricing.
          </p>
          <div className="mt-8">
            <a href={PHONE_HREF}>
