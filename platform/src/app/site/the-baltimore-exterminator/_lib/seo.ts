...
export function getServiceHubMeta(service: Service) {
  const title = `${service.name} Baltimore | $199/hr | Self-Book & Save $20`;
  const description = `${service.name} across the entire Baltimore metro area — at a flat $199/hr, 1-hour minimum (fully inclusive — no hidden fees). Self-book online & save $20 — the fastest way to get service. Pay only when the job is done. No contracts. No deposits. No catches. Licensed & insured.`;
