...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const blogPhotoUrl = `https://www.thenycmaid.com${blogPhoto.src}`
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://www.thenycmaid.com/nyc-maid-service-blog/${post.slug}`,
    },
    url: `https://www.thenycmaid.com/nyc-maid-service-blog/${post.slug}`,
  }
      ...
      <article className="max-w-3xl mx-auto px-4 py-12">
          ...
          {post.sections.map((section, i) => (
            <div key={i} className="mb-8">
              ...
              {/* SAFE: `p` is static, operator-authored blog copy from a local module
                  (BLOG_POSTS in @/lib/seo/blog-data) — no DB, no user/tenant input. Raw
                  HTML is intentional: paragraphs carry inline <a> links (hence [&_a] styles).
        ...
        <div className="border border-gray-200 rounded-xl p-6 my-12">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/reviews/submit" className="text-[#1E2A4A] text-sm font-medium underline underline-offset-2">Write a review</Link>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#1E2A4A] rounded-xl p-8 my-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="sms:2122029030" className="bg-[#A8F0DC] text-[#1E2A4A] px-8 py-3.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[#8DE8CC] transition-colors">
              Text (212) 202-9030
            </a>
          </div>
        </div>
