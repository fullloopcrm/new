...
export default function ApplyPage() {
  const [form, setForm] = useState({
    ...
    max_travel_minutes: '',
    sms_consent: false,
    // Florida has no statewide ban-the-box law for private employers, so
    // this can be asked directly on the application (unlike the NY/NJ/CT
    // tenants, which ask post-offer in the team portal instead — see
    // src/app/team/disclosure/page.tsx). Framed as job-relevant disclosure,
    // not a blanket bar, to stay clear of Title VII disparate-impact
    // exposure that applies regardless of state law.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.criminal_history) {
      ...
      trackApply('apply_submit_error', { reason: 'missing_criminal_history' })
      return
    }
    setLoading(true)
    setError('')
    ...
    try {
      ...
      const res = await fetch('/api/cleaner-applications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, criminal_history_response, photo_url, attribution, ...getSpamGuardFields() })
      })
      ...
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center max-w-sm">
            ...
            <p className="text-gray-500 text-sm mt-4">Questions? / ¿Preguntas? (954) 710-3636</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="max-w-lg mx-auto p-4 pt-6">
        <form onSubmit={handleSubmit} onFocusCapture={markStart} onBlurCapture={handleFieldBlur} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
          ...
          <p className="text-xs text-gray-400 text-center">
            Questions? / ¿Preguntas? (954) 710-3636
          </p>
        </form>
      </div>
    </div>
