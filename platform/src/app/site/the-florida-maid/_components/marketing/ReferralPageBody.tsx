import ReferralSignupForm from '@/app/site/the-florida-maid/_components/marketing/ReferralSignupForm'

const en = {
  heroTitle: 'Get Paid for Cleaning Referrals',
  heroBody: 'Earn 10% commission every time someone you refer books a cleaning with The Florida Maid. No limit on referrals. Recurring income for as long as they stay a customer.',
  benefits: [
  ...
  steps: [
    ...
    { title: 'Share With Friends & Family', body: 'Send your link to anyone who needs cleaning anywhere in Florida. They book using your link.' },
...
const es = {
  ...
  heroBody: 'Gana 10% de comisión cada vez que alguien que refieras reserve una limpieza con The Florida Maid. Sin límite de referidos. Ingresos recurrentes mientras sigan siendo clientes.',
  benefits: [
  ...
  steps: [
    ...
    { title: 'Comparte con Amigos y Familia', body: 'Envía tu enlace a quien necesite limpieza en cualquier parte de Florida. Reservan usando tu enlace.' },
    ...
    <>
      ...
      <section className="bg-[#CC6222] py-20 md:py-28">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <p className="text-gray-200 text-base max-w-2xl leading-relaxed mt-3 italic">
            {es.heroBody}
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {en.benefits.map((b, i) => (
            <div key={b.label} className="bg-[#F5FBF8] border border-[#34D399]/30 rounded-xl p-8 text-center">
              <p className="font-[family-name:var(--font-bebas)] text-5xl text-[#CC6222] tracking-wide mb-2">{b.big}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-16">
          <div>
            ...
            <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#CC6222] tracking-wide mb-1">{en.howItWorksTitle}</p>
            <p className="font-[family-name:var(--font-bebas)] text-xl text-[#CC6222]/60 tracking-wide mb-8">{es.howItWorksTitle}</p>

            <div className="space-y-8">
              {en.steps.map((s, i) => (
                <div key={s.title} className="flex gap-5">
                  <div className="w-10 h-10 bg-[#CC6222] text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold">{i + 1}</div>
                  <div>
                    <h3 className="font-semibold text-[#CC6222] text-lg mb-0.5">{s.title}</h3>
                    <h3 className="font-semibold text-[#CC6222]/60 text-base mb-1">{es.steps[i].title}</h3>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-10 mb-16 text-center">
          ...
          <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#CC6222] tracking-wide mb-1">{en.earningsTitle}</p>
          <p className="font-[family-name:var(--font-bebas)] text-lg text-[#CC6222]/60 tracking-wide mb-6">{es.earningsTitle}</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
            {en.earnings.map((e, i) => (
              <div key={e.label} className={`p-5 rounded-xl ${i === 2 ? 'bg-[#F5FBF8] border border-[#34D399]/30' : 'bg-gray-50'}`}>
                <p className="text-sm text-gray-500">{e.label}</p>
                <p className="text-sm text-gray-400 italic mb-1">{es.earnings[i].label}</p>
                <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#CC6222] tracking-wide">{e.value}</p>
              </div>
            ))}
          </div>
        </div>
