...
import { PHONE, PHONE_HREF, CITY_COUNT } from "@/app/site/nycroadsideemergencyassistance/_data/content";
import { SERVICES, SERVICE_CATEGORIES, getExtendedContent } from "@/app/site/nycroadsideemergencyassistance/_data/services";
import { getPhoto } from "@/app/site/nycroadsideemergencyassistance/_data/photos";
import { getLinkifiedExtendedContent } from "@/app/site/nycroadsideemergencyassistance/_data/services-links";
import { getServiceTips } from "@/app/site/nycroadsideemergencyassistance/_data/service-tips";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return {};
  return {
    title: `${service.title} NYC — $149/hr, $25 Off Online, No Catches. 24/7 All Boroughs.`,
    description: `${service.title} in NYC at one rate: $149/hour, $25 off online ($124 first hour). ${service.description} No NYC surcharge, no after-hours markup. 24/7 dispatch, 20–40 min arrival, all 5 boroughs.`,
    ...
    <>
      ...
      <section className="bg-section-white py-16">
        <div className="mx-auto max-w-3xl px-6">
          {(() => { const photo = getPhoto(service.slug); return (
          <div className="relative aspect-[16/9] overflow-hidden rounded-2xl shadow-lg">
            <Image src={photo.url} alt={photo.alt || `${service.title} in NYC`} fill sizes="(max-width: 768px) 100vw, 768px" className="object-cover" priority={false} />
          </div>
          ); })()}
        ...
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { step: "2", title: "$149/hr — $25 Off Online", desc: `One rate for every service: $149/hour, 1-hour minimum. Book online and your first hour is $124. Live ETA, truck number, and driver name before we hang up.` },
