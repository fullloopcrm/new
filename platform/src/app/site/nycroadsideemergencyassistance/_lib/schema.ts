/**
 * JSON-LD schema builders for NYC Roadside Emergency Assistance.
 *
...
export const SITE_URL = "https://www.nycroadsideemergencyassistance.com";
export const BRAND_NAME = "NYC Roadside Emergency Assistance";
...
/** Canonical hourly rate for schema purposes — same $149/hr for every service.
 *  The $25 online-booking discount is a channel promo, not the rate itself, so schema
 *  always emits the standard hourly rate (PRICING.standard). */
...
export function organizationSchema() {
  return {
    ...
    image: LOGO_URL,
    description:
      "24/7 towing and roadside assistance across all five NYC boroughs. Light-duty, heavy-duty, flatbed, battery, tire, lockout, gas, and winch-out. One rate for every service: $149/hour, 1-hour minimum, $25 off when you book online ($124 first hour). No NYC surcharge, no after-hours markup. 20–40 minute arrival, licensed and insured.",
    telephone: PHONE,
    email: EMAIL,
    priceRange: "$$",
...
export interface ServiceSchemaOptions {
  ...
  /** Optional roadway override — beats `location` when both are set. */
  roadwayName?: string;
}
...
export function serviceSchema(service: Service, opts: ServiceSchemaOptions = {}) {
  ...
  const areaServed = opts.roadwayName
    ? {
        "@type": "Place",
        name: opts.roadwayName,
      }
    : opts.location?.cityName
    ? {
        "@type": "Place",
...
/* -------------------------------------------------------------------------- */
/*                            EmergencyService per Roadway                     */
/* -------------------------------------------------------------------------- */

export interface RoadwayLocalBusinessInput {
  name: string;
  url: string;
  geo?: { lat: number; lon: number };
  /** Borough slug for geo fallback. */
  boroughSlug?: string;
}

/**
 * EmergencyService LocalBusiness for a specific roadway corridor.
 * Associates the org with the geographic feature in Google's knowledge graph.
 */
export function emergencyServiceForRoadway(input: RoadwayLocalBusinessInput) {
  const geo = input.geo ?? (input.boroughSlug ? boroughGeo(input.boroughSlug) : undefined);
  return {
    "@context": "https://schema.org",
    "@type": ["EmergencyService", "AutomotiveBusiness"],
    "@id": `${SITE_URL}${input.url}#localbusiness`,
    name: `${BRAND_NAME} — ${input.name}`,
...
export function faqPageSchema(faqs: FaqItem[]) {
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    // Speakable hints assistants (Google, Alexa) which parts to read aloud.
    speakable: {
      "@type": "SpeakableSpecification",
      cssSelector: ["h1", "h2", "h3", "[itemprop=name]", "[itemprop=text]"],
    },
...
export function articleSchema(input: ArticleSchemaInput) {
  ...
  if (input.image) {
    out.image = { "@type": "ImageObject", url: input.image, width: 1600, height: 900 };
  }
