...
import { getTopCitiesPerState, getCityBySlug } from "@/app/site/theroadsidehelper/_data/cities";
...
import { customerCityContent } from "@/app/site/theroadsidehelper/_data/customer-content";
...
export async function generateMetadata({
  ...
  return pageSeo({
    ...
    description: `${ct.name} roadside assistance in ${result.city.name}, ${result.state.abbreviation}. ${ct.description} $149/hr flat.`,
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 pt-36 pb-16 sm:pt-44 sm:pb-24">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-teal-200 font-cta">{ct.name} Roadside Assistance in {city.name}</p>
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl font-heading">
            <span className="gradient-text">{ct.name}</span> Roadside Assistance<br />in {city.name}, {state.abbreviation}
          </h1>
      ...
      {nearbyCities.length > 0 && (
        <section className="bg-section-white py-16">
          <div className="mx-auto max-w-5xl px-6">
            <p className="text-center text-sm font-semibold uppercase tracking-widest text-teal-600 font-cta">{ct.name} Roadside Assistance Near {city.name}</p>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-teal-700 via-teal-600 to-teal-800 py-16">
        <div className="absolute inset-0 grid-bg opacity-30" />
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <h2 className="mt-3 text-center text-3xl font-bold text-white sm:text-4xl font-heading">Book {ct.name} Roadside Assistance in {city.name}</h2>
          <CtaButtons variant="dark" />
        </div>
      </section>
    </>
