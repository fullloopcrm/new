...
import { SERVICES, getService } from '@/app/site/pennsylvania-maid/_lib/seo/services'
import { neighborhoodServiceContent, neighborhoodFAQs, serviceFAQs, commonServiceFAQs } from '@/app/site/pennsylvania-maid/_lib/seo/content'
import { neighborhoodServicePageSchemas, faqSchema } from '@/app/site/pennsylvania-maid/_lib/seo/schema'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://www.thepennsylvaniamaid.com/${slug}/${serviceSlug}`
  const title = `${service.name} in ${neighborhood.name}, ${area.name} From $59/hr | The Pennsylvania Maid`
  const description = `Professional ${service.name.toLowerCase()} in ${neighborhood.name}, ${area.name}. ${service.features.slice(0, 3).join(', ')} & more. ${service.priceRange}. 5.0★ Rated. (215) 398-4500`
      ...
      <section className="py-20 bg-[#1E2A4A]">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { n: '01', t: 'Text Us', d: `Reach us at (215) 398-4500 with your ${neighborhood.name} address and tell us you need ${service.name.toLowerCase()}.` },
