...
import { ALL_NEIGHBORHOODS, getNeighborhoodsByArea, type Neighborhood } from '@/app/site/pennsylvania-maid/_lib/seo/locations'
...
import { pickTeamPhoto } from '@/app/site/pennsylvania-maid/_lib/seo/photos'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = `💰 Now hiring cleaners in ${n.name}! Starting $30/hr + bonus programs. Stripe in <30 min. 100% tips. Open 24/7 🇺🇸🇪🇸 | Contratando en ${n.name} — desde $30/hr + bonos. (215) 398-4500`

  return {
    title,
    description,
    alternates: { canonical: `https://www.thepennsylvaniamaid.com/available-pennsylvania-maid-jobs/${n.slug}` },
    openGraph: {
      title: `Cleaning Jobs in ${n.name} | The Pennsylvania Maid`,
      description,
      url: `https://www.thepennsylvaniamaid.com/available-pennsylvania-maid-jobs/${n.slug}`,
    },
  }
}
...
export default async function NeighborhoodJobPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const photoUrl = `https://www.thepennsylvaniamaid.com${photo.src}`
    ...
    <>
      <JsonLd data={[
        ...
        breadcrumbSchema([
          ...
          { name: `Jobs in ${n.name}`, url: `https://www.thepennsylvaniamaid.com/available-pennsylvania-maid-jobs/${n.slug}` },
        ]),
        jobSchema,
      ]} />
      ...
      <section className="bg-gradient-to-b from-[#1E2A4A] to-[#243352] py-16 md:py-24">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-white/60 text-xs">Pennsylvania, Long Island &amp; NJ</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5 mt-8">
            ...
            <a href="sms:2153984500" className="text-white/70 font-medium text-lg py-4 hover:text-white transition-colors underline underline-offset-4">
      ...
      {/* Team photo — real Pennsylvania Maid cleaning crew */}
      <section className="relative bg-[#1E2A4A]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <Breadcrumbs items={[
          ...
          { name: `Jobs in ${n.name}`, href: `/available-pennsylvania-maid-jobs/${n.slug}` },
        ]} />
        ...
        {nearby.length > 0 && (
          <section className="mb-16">
            ...
            <div className="flex flex-wrap gap-2">
              {nearby.map(nb => (
                <Link key={nb.slug} href={`/available-pennsylvania-maid-jobs/${nb.slug}`} className="px-4 py-2 bg-gray-100 rounded-full text-sm text-gray-700 hover:bg-[#A8F0DC]/20 hover:text-[#1E2A4A] transition-colors">
                  {nb.name}
                </Link>
              ))}
            </div>
        ...
        {sameArea.length > 0 && (
          <section className="mb-16">
            ...
            <div className="flex flex-wrap gap-2">
              {sameArea.map(nb => (
                <Link key={nb.slug} href={`/available-pennsylvania-maid-jobs/${nb.slug}`} className="px-3 py-1.5 bg-gray-100 rounded-full text-sm text-gray-700 hover:bg-[#A8F0DC]/20 hover:text-[#1E2A4A] transition-colors">
                  {nb.name}
                </Link>
              ))}
            </div>
        ...
        <div className="text-center">
          <Link href="/available-pennsylvania-maid-jobs" className="text-[#1E2A4A] font-semibold hover:underline">&larr; View All Positions / Ver Todas las Posiciones</Link>
        </div>
      </div>
    </>
  )
