...
export const metadata: Metadata = {
  ...
  openGraph: { title: pageTitle, description: pageDescription, url: pageUrl, type: 'article', siteName: 'The Pennsylvania Maid' },
}
...
function administratorJobPostingSchema(config: NonNullable<ReturnType<typeof getPosition>>) {
  ...
  return {
    ...
    datePosted,
    validThrough,
    description: `<h2>${config.title} — The Pennsylvania Maid — Remote, Anywhere in the US</h2><p>${config.tagline}</p><p><strong>Compensation:</strong> ${config.compSummary}</p><p><strong>Schedule:</strong> ${config.schedule}</p><p><strong>Employment type:</strong> ${config.employmentType}</p>${config.responsibilitiesNote ? `<p>${config.responsibilitiesNote}</p>` : ''}<p>How to apply: <a href="${pageUrl.replace('/careers/', '/apply/')}">thepennsylvaniamaid.com/apply/administrator</a>. Includes a short video introduction.</p>`,
    hiringOrganization: {
      '@type': 'Organization',
    ...
    directApply: true,
    url: pageUrl,
    identifier: { '@type': 'PropertyValue', name: 'The Pennsylvania Maid', value: 'nycmaid-administrator-2026' },
...
const faqs = [
  ...
  { question: 'How do I apply?', answer: 'Submit the application at thepennsylvaniamaid.com/apply/administrator. It includes background questions, a few scenario questions, and a short required video introduction.' },
]
