...
import { ALL_NEIGHBORHOODS, getNeighborhoodsByArea, type Neighborhood } from '@/app/site/new-jersey-maid/_lib/seo/locations'
...
import { pickTeamPhoto } from '@/app/site/new-jersey-maid/_lib/seo/photos'
import { jobDates } from '@/app/site/new-jersey-maid/_lib/seo/job-dates'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = `Commission sales jobs in ${n.name}, New Jersey. 1099 sales rep role — sign clients or referrers and earn 10% recurring on every cleaning, paid daily via Stripe. No cap. Apply: (973) 370-8800`
  return {
    title,
    description,
    alternates: { canonical: `https://www.thenewjerseymaid.com/careers/commission-sales-partner/${n.slug}` },
    openGraph: {
      title: `Sales Jobs in ${n.name} — Commission / 1099 | The New Jersey Maid`,
      description,
      url: `https://www.thenewjerseymaid.com/careers/commission-sales-partner/${n.slug}`,
    },
  }
}
...
export default async function NeighborhoodSalesJobPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const photoUrl = `https://www.thenewjerseymaid.com${photo.src}`
  ...
  const jobSchema = {
    ...
    hiringOrganization: {
      ...
      logo: { '@type': 'ImageObject', url: 'https://www.thenewjerseymaid.com/icon-512.png', width: 512, height: 512 },
    ...
    qualifications: `New Jersey-area network access in or around ${n.name}. Relationship builder. Sales background helpful, not required. English required, English/Spanish a plus.`,
    ...
    <>
      <JsonLd data={[
        ...
        breadcrumbSchema([
          ...
          { name: 'Sales Partner', url: 'https://www.thenewjerseymaid.com/careers/commission-sales-partner' },
          { name: `Sales Partner in ${n.name}`, url: `https://www.thenewjerseymaid.com/careers/commission-sales-partner/${n.slug}` },
        ]),
        jobSchema,
      ]} />
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <section className="bg-[#A8F0DC] rounded-2xl p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="sms:9733708800" className="text-[#1E2A4A]/60 font-medium text-sm hover:text-[#1E2A4A] transition-colors underline underline-offset-4">
              Text us for questions
            </a>
          </div>
        </section>
