...
const pageUrl = 'https://www.thenewjerseymaid.com/careers/administrator'
const pageTitle = 'Administrator — Your Path to CEO of a Multi-Million-Dollar Company | The New Jersey Maid'
const pageDescription = 'The New Jersey Maid is a brand-new market launch backed by the same team and platform behind The New Jersey Maid. We’re hiring an Administrator with a real long-term path to becoming CEO. Compensation based on experience. Remote, anywhere in the US.'
...
export const metadata: Metadata = {
  ...
  openGraph: { title: pageTitle, description: pageDescription, url: pageUrl, type: 'article', siteName: 'The New Jersey Maid' },
}
...
function administratorJobPostingSchema(config: NonNullable<ReturnType<typeof getPosition>>) {
  ...
  return {
    ...
    datePosted,
    validThrough,
    description: `<h2>${config.title} — The New Jersey Maid — Remote, Anywhere in the US</h2><p>${config.tagline}</p><p><strong>Compensation:</strong> ${config.compSummary}</p><p><strong>Schedule:</strong> ${config.schedule}</p><p><strong>Employment type:</strong> ${config.employmentType}</p>${config.responsibilitiesNote ? `<p>${config.responsibilitiesNote}</p>` : ''}<p>How to apply: <a href="${pageUrl.replace('/careers/', '/apply/')}">thenewjerseymaid.com/apply/administrator</a>. Includes a short video introduction.</p>`,
    hiringOrganization: {
      '@type': 'Organization',
    ...
    directApply: true,
    url: pageUrl,
    identifier: { '@type': 'PropertyValue', name: 'The New Jersey Maid', value: 'new-jersey-maid-administrator-2026' },
...
const faqs = [
  ...
  { question: 'Is this a real, backed opportunity?', answer: 'The New Jersey Maid is a brand-new market launch from the same team and platform behind The New Jersey Maid, one of New Jersey’s highest-rated cleaning companies, backed by someone with 20 years of experience in home services marketing. This is a genuine ground-floor role in a new market, not an established operation with a track record yet — that’s exactly the opportunity: you’d be building it.' },
  { question: 'Do I need to live in New Jersey?', answer: 'No. This is fully remote and open to candidates anywhere in the United States.' },
  ...
  { question: 'How do I apply?', answer: 'Submit the application at thenewjerseymaid.com/apply/administrator. It includes background questions, a few scenario questions, and a short required video introduction.' },
]

export default function AdministratorCareersPage() {
  const config = getPosition('new-jersey-maid', 'administrator')
