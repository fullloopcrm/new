...
export function homepageContent() {
  return {
    ...
    metaDescription: 'New Jersey\'s top-rated maid service from $59/hr. House cleaning across New Jersey & NJ. Licensed, insured. 5.0★ Rated. (973) 370-8800',
...
const areaIntros: Record<string, string[]> = {
  'central-jersey': [
    'Central Jersey runs from the Rutgers-area energy of New Brunswick to the quiet suburban streets of East Brunswick and Somerset — our cleaning team handles every type of home with care and precision.',
    'From Highland Park rowhouses to Piscataway and North Brunswick colonials, our Central Jersey cleaners deliver spotless results every time.',
  ],
}

export function areaContent(area: Area) {
  const intros = areaIntros[area.slug] || areaIntros['central-jersey']
  return {
    title: `House Cleaning Services in ${area.name}`,
    metaDescription: `Professional cleaning in ${area.name} from $59/hr. Deep cleaning, weekly service, move-in/out & more. 5.0★ Rated. (973) 370-8800`,
...
const introTemplates = [
  (n: Neighborhood) => `Looking for a reliable cleaning service in ${n.name}? The New Jersey Maid has been trusted by ${n.name} residents for years, delivering spotless results in ${n.housing_types[0]}, ${n.housing_types[1]}, and more.`,
...
export function neighborhoodContent(neighborhood: Neighborhood, area: Area) {
  ...
  return {
    title: `${h1} | ${area.name}`,
    metaDescription: `Professional cleaning in ${neighborhood.name}, ${area.name} from $59/hr. Serving ${neighborhood.housing_types.slice(0, 2).join(', ')} near ${neighborhood.landmarks[0]}. 5.0★ Rated. (973) 370-8800`,
    h1,
    intro,
  }
}
...
const knownForTemplates = [
  (n: Neighborhood) => [`Home to ${n.landmarks[0]}`, `Known for its beautiful ${n.housing_types[0]}`, `A neighborhood of ${n.housing_types[1]} and local character`, `Walking distance to ${n.landmarks[1] || n.landmarks[0]}`, `One of New Jersey's most sought-after areas for families and professionals`],
...
const boroughFlavor: Record<string, (n: Neighborhood) => string> = {
  'central-jersey': (n) => `Across Central Jersey, ${n.name} cleaners handle everything from campus-adjacent apartments to suburban colonials — the same insured, background-checked team on every visit.`,
}
...
const serviceIntroTemplates = [
  ...
  (n: Neighborhood, s: Service) => `${n.name} residents trust The New Jersey Maid for expert ${s.name.toLowerCase()}. We understand the unique ${n.cleaning_challenges[0]} and ${n.cleaning_challenges[1]} that come with cleaning homes in this area.`,
...
export function neighborhoodServiceContent(neighborhood: Neighborhood, service: Service, area: Area) {
  ...
  return {
    title: `${h1} | ${area.name}`,
    metaDescription: `${service.name} in ${neighborhood.name}, ${area.name}. ${service.features.slice(0, 2).join(', ')} & more. ${service.priceRange}. 5.0★ Rated. (973) 370-8800`,
    h1,
    intro,
    whyUs: [
...
export function serviceContent(service: Service) {
  return {
    ...
    metaDescription: `Professional ${service.name.toLowerCase()} across New Jersey & NJ. ${service.features.slice(0, 2).join(', ')} & more. ${service.priceRange}. 5.0★ Rated. (973) 370-8800`,
...
const richContentMap: Record<string, ServiceRichContent> = {
  ...
  'same-day-cleaning': {
    ...
    nycTips: [
      { title: 'Text us early for best availability', detail: 'Same-day requests sent before 10am have the best chance of afternoon availability. After noon, same-day availability drops significantly. Text (973) 370-8800 as early as possible for the best shot at a same-day slot.' },
    ...
    faqs: [
      ...
      { question: 'How do I book same-day service?', answer: 'Text (973) 370-8800 as early in the day as possible. Text is fastest — we\'ll confirm availability and estimated arrival time within minutes. Morning requests before 10am have the best availability for afternoon service.' },
    ],
  },
...
export function neighborhoodFAQs(neighborhood: Neighborhood, area: Area): { question: string; answer: string }[] {
  return [
    ...
    },
    {
      ...
      answer: `You can book online through our website, text us at (973) 370-8800. We typically can schedule within 24-48 hours, with same-day availability for urgent requests.`,
    },
    {
...
export function commonServiceFAQs(service: Service): { question: string; answer: string }[] {
  return [
    ...
    { question: 'What areas do you serve?', answer: 'We serve New Jersey (Nassau and western Suffolk), New Jersey County, and northern New Jersey (Hudson and close-in Bergen). Same rates everywhere — no travel surcharges regardless of location.' },
    ...
    { question: 'Are you licensed and insured?', answer: 'Yes. The New Jersey Maid is a fully licensed cleaning company with general liability insurance and bonding. Every cleaner is covered while working in your home. We can provide proof of insurance upon request.' },
