/**
 * This forked assistant (wash-and-fold-hoboken has its own standalone
 * _lib/selena.ts, not the shared src/lib/selena-legacy.ts) had the SAME gap
 * selena-legacy.ts's handleCreateBooking had before its P11.16/17 fix:
 * handleCheckAvailability already tells the AI "Same-day booking. Rate is
 * $100/hr emergency." but handleCreateBooking trusted the LLM's hourly_rate
 * argument verbatim and never set is_emergency on the row it inserted. A
 * model that misread or forgot the $100 rate could underbill same-day
 * bookings with zero server-side guardrail.
 *
...
vi.mock('@/app/site/wash-and-fold-hoboken/_lib/notify', () => ({ notify: vi.fn().mockResolvedValue(undefined) }))
...
describe('wash-and-fold-hoboken handleCreateBooking — server-side $100 emergency rate + is_emergency', () => {
