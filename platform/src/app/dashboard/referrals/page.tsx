    ...
    <div>
      ...
      <div className="flex items-center justify-between bg-slate-800 border border-slate-700 rounded-xl px-5 py-3 mb-6">
        <div className="flex items-center gap-2 text-sm">
          ...
          <code className="text-blue-400 font-mono text-xs bg-slate-700 px-2 py-0.5 rounded">{typeof window !== 'undefined' ? `${window.location.origin}/referral/signup` : '/referral/signup'}</code>
        </div>
        <button onClick={() => navigator.clipboard.writeText(`${window.location.origin}/referral/signup`)} className="text-xs text-slate-400 hover:text-white transition-colors">Copy Link</button>
      </div>
        ...
        {({ config, updateConfig }) => (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between max-w-sm">
              <label className="text-sm text-slate-300">Auto-generate referral codes for new clients</label>
              <button
      ...
      {/* CREATE FORM */}
      {showCreate && (
        <form onSubmit={createReferral} className="bg-slate-800 border border-slate-700 rounded-xl p-6 mb-6">
      ...
      <input
        ...
        className="w-full md:w-64 bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm mb-4 placeholder-gray-500"
      />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyCode(r.referral_code)}
                        className={`text-[10px] px-1.5 py-0.5 rounded transition-colors ${
                          copied === r.referral_code ? 'bg-green-500/20 text-green-400' : 'bg-slate-700 text-slate-400 hover:text-slate-400'
                        }`}>
      ...
      {activeTab === 'payouts' && (
        <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
          ...
          {paidOut.length > 0 && (
            <>
              <div className="px-5 py-3 border-t border-slate-700 bg-slate-700/30">
                ...
                <div key={entry.name} className="flex items-center justify-between px-5 py-3">
                  <div className="flex items-center gap-3">
                    <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                      ...
                      i === 1 ? 'bg-slate-600 text-slate-400' :
