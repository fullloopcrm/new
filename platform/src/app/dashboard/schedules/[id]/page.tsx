...
export default function ScheduleDetailPage() {
  ...
  return (
    <div>
      <Link href="/dashboard/schedules" className="text-sm text-gray-400 hover:text-white mb-4 inline-block">
        &larr; All Schedules
      </Link>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={startEditing} className="px-4 py-2 text-sm text-gray-400 border border-gray-700 rounded-lg hover:text-white hover:border-gray-600">
            Edit
          </button>
      ...
      {editing && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          <h3 className="font-semibold text-white mb-4">Edit Schedule</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wide mb-1 block">Recurring Type</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wide mb-1 block">Preferred Time</label>
              <input type="time" value={editForm.preferred_time} onChange={e => setEditForm({...editForm, preferred_time: e.target.value})}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wide mb-1 block">Day of Week</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wide mb-1 block">Duration (hours)</label>
          ...
          <div className="flex gap-2">
            <button onClick={saveEdit} className="bg-white text-gray-900 px-5 py-2 rounded-lg text-sm font-medium">Save</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h3 className="font-semibold text-white mb-4">Schedule Details</h3>
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between"><dt className="text-gray-400">Frequency</dt><dd className="capitalize">{schedule.recurring_type.replace('_', ' ')}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-400">Day</dt><dd>{schedule.day_of_week != null ? DAYS[schedule.day_of_week] : '—'}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-400">Time</dt><dd>{schedule.preferred_time || '—'}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-400">Duration</dt><dd>{schedule.duration_hours ? `${schedule.duration_hours} hours` : '—'}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-400">Status</dt><dd className="capitalize font-medium">{schedule.status}</dd></div>
              {schedule.paused_until && <div className="flex justify-between"><dt className="text-gray-400">Paused Until</dt><dd>{new Date(schedule.paused_until).toLocaleDateString()}</dd></div>}
              {schedule.notes && <div><dt className="text-gray-400 mb-1">Notes</dt><dd className="bg-gray-800/50 rounded p-2">{schedule.notes}</dd></div>}
            </dl>
          </div>

          {schedule.status === 'active' && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="font-semibold text-white mb-4">Pause Schedule</h3>
              <div className="flex gap-2">
                <input type="date" value={pauseDate} onChange={(e) => setPauseDate(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm" />
                <button onClick={pause} className="bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm font-medium">Pause Until</button>
              </div>
            </div>
          )}
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h3 className="font-semibold text-white mb-4">Generated Bookings ({bookings.length})</h3>
            <div className="space-y-2">
              {bookings.map((b) => (
                <Link key={b.id} href={`/dashboard/bookings/${b.id}`} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-800/30 border border-gray-800">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{new Date(b.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
              ...
              {bookings.length === 0 && <p className="text-sm text-gray-400">No bookings generated yet</p>}
            </div>
          </div>
        </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            {schedule.clients ? (
              <dl className="space-y-2 text-sm">
                ...
                {schedule.clients.phone && <dd className="text-gray-500">{schedule.clients.phone}</dd>}
                {schedule.clients.address && <dd className="text-gray-400">{schedule.clients.address}</dd>}
              </dl>
            ) : (
