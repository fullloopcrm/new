...
export default function CampaignsPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">{campaigns.length} total &middot; {draftCount} drafts &middot; {sentCount} sent</p>
          </div>
      ...
      >
        {({ config, updateConfig }) => (
          <div className="space-y-5">
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wide mb-2 block">Default Campaign Type</label>
              <select
            ...
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wide mb-2 block">Default Sender Name</label>
              <input
                type="text"
            ...
            <div className="flex items-center justify-between max-w-xs">
              <label className="text-sm text-gray-300">Auto-add unsubscribe link</label>
              <button
                onClick={() => updateConfig('auto_unsubscribe', config.auto_unsubscribe === false ? true : !config.auto_unsubscribe)}
                className={`relative w-10 h-5 rounded-full transition-colors ${config.auto_unsubscribe !== false ? 'bg-blue-500' : 'bg-gray-700'}`}
              >
            ...
            <div className="flex items-center justify-between max-w-xs">
              <label className="text-sm text-gray-300">Campaign approval required</label>
              <button
                onClick={() => updateConfig('approval_required', !config.approval_required)}
                className={`relative w-10 h-5 rounded-full transition-colors ${config.approval_required ? 'bg-blue-500' : 'bg-gray-700'}`}
              >
      ...
      {/* CREATE FORM */}
      {showCreate && (
        <form onSubmit={create} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          <h3 className="font-semibold text-white mb-4">Create Campaign</h3>

          {/* Step 1: Basics */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
            <div>
              <label className="text-xs text-gray-500 uppercase mb-1 block">Campaign Name *</label>
            ...
            <div>
              <label className="text-xs text-gray-500 uppercase mb-1 block">Schedule (optional)</label>
          ...
          {/* Step 2: Recipients */}
          <div className="mb-5">
            <label className="text-xs text-gray-500 uppercase mb-2 block">Recipients</label>
          ...
          {(form.type === 'email' || form.type === 'both') && (
            <div className="mb-4">
              <label className="text-xs text-gray-500 uppercase mb-1 block">Subject Line {form.type === 'both' && <span className="text-gray-600 normal-case">(for email)</span>}</label>
            ...
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs text-gray-500 uppercase">
                {form.type === 'sms' ? 'Message' : form.type === 'both' ? 'Campaign Body' : 'Email Body'} {(form.type === 'sms' || form.type === 'both') && form.body && (
                  <span className={`ml-1 ${form.body.length > 160 ? 'text-red-500' : 'text-gray-400'}`}>
                    ({form.body.length}/160{form.type === 'both' ? ' SMS chars' : ''})
                  </span>
                )}
              </label>
              <span className="text-[10px] text-gray-400">Use {'{name}'} for client name, {'{business}'} for your business</span>
            </div>
          ...
          {/* AI Writing Assist */}
          <div className="bg-gray-800/50 border border-gray-800 rounded-lg p-4 mb-5">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm">✧</span>
              <h4 className="text-xs font-semibold text-gray-500 uppercase">Selenas AI — Write Assist</h4>
            </div>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={aiWrite} disabled={aiLoading || !aiPrompt.trim()}
                className="bg-white text-gray-900 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 disabled:opacity-50 whitespace-nowrap">
            ...
            <div className="flex gap-1.5 mt-2 flex-wrap">
              {['Promo discount', 'Seasonal reminder', 'Win-back offer', 'Thank you', 'Holiday greeting'].map((tag) => (
                <button key={tag} type="button"
                  ...
                  className="text-[10px] px-2 py-0.5 rounded-full bg-gray-700 text-gray-400 hover:bg-gray-600">
                  {tag}
                </button>
              ))}
            </div>
              ...
              {form.type === 'both' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-300 whitespace-pre-wrap bg-green-500/20 rounded-lg p-3 max-w-xs">
                      {form.body.replace(/\{name\}/g, 'Sarah').replace(/\{business\}/g, 'My Business')}
                    </div>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div className={`text-sm text-gray-300 whitespace-pre-wrap ${form.type === 'sms' ? 'bg-green-500/20 rounded-lg p-3 max-w-xs' : ''}`}>
                    {form.body.replace(/\{name\}/g, 'Sarah').replace(/\{business\}/g, 'My Business')}
                  </div>
                </>
              )}
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowCreate(false)}
              className="px-4 py-2 text-sm text-gray-500 hover:text-white">Cancel</button>
          </div>
        </form>
      )}
            ...
            {filtered.map((c) => (
              <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                <td className="px-4 py-3">
                  <Link href={`/dashboard/campaigns/${c.id}`} className="font-medium text-white hover:text-blue-400">
                    {c.name}
                  </Link>
                  {c.subject && <p className="text-xs text-gray-400 mt-0.5 truncate max-w-xs">{c.subject}</p>}
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-gray-500">
                  {c.open_count != null ? (
                    <span>{c.open_count} {c.recipient_count ? <span className="text-gray-400 text-xs">({Math.round((c.open_count / c.recipient_count) * 100)}%)</span> : ''}</span>
                  ) : '—'}
                </td>
                <td className="px-4 py-3 text-gray-500">{c.click_count ?? '—'}</td>
