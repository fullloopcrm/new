  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Selenas AI</h2>
          <p className="text-sm text-slate-400">Your AI assistant for campaigns, copy, and business strategy</p>
        </div>
        {messages.length > 0 && (
          <button onClick={() => setMessages([])}
            className="text-xs text-slate-400 hover:text-slate-400 px-3 py-1.5 border border-slate-200 rounded-lg">
            Clear Chat
          </button>
        )}
      </div>
      ...
      {/* Chat area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto border border-slate-200 rounded-lg mb-4">
        {messages.length === 0 ? (
          <div className="p-8">
            <div className="text-center mb-8">
              ...
              <h3 className="text-lg font-semibold text-slate-900">Hey! I&apos;m Selenas.</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-w-2xl mx-auto">
              {quickActions.map((qa) => (
                <button key={qa.label} onClick={() => send(qa.prompt)}
                  className="text-left bg-slate-50 hover:bg-slate-50 border border-slate-200 rounded-lg p-3 transition-colors">
          ...
          <div className="p-4 space-y-4">
              ...
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] ${
                  m.role === 'user'
                    ...
                    : 'bg-slate-50 text-slate-900 rounded-2xl rounded-bl-md px-4 py-2.5 border border-slate-200'
                }`}>
                  {m.role === 'assistant' ? (
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-slate-50 text-slate-400 rounded-2xl rounded-bl-md px-4 py-2.5 border border-slate-200 text-sm">
                  Thinking...
                </div>
              </div>
            )}
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent"
          disabled={loading}
        />
        <button onClick={() => send()} disabled={loading || !input.trim()}
          className="bg-teal-600 text-white px-5 py-3 rounded-lg text-sm font-medium hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed">
          Send
        </button>
      </div>
    </div>
