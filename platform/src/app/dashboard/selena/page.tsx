...
export default function SelenaPage() {
  ...
  const [agentName, setAgentName] = useState('Selena')
  const [settingsLoaded, setSettingsLoaded] = useState(false)
  const [settingsSaving, setSettingsSaving] = useState(false)
  const [settingsMsg, setSettingsMsg] = useState('')

  useEffect(() => { document.title = `${agentName} — AI Concierge` }, [agentName])
  ...
  useEffect(() => {
    fetch('/api/settings')
      ...
      .then(data => {
        ...
        setAgentName(data.tenant?.agent_name || data.tenant?.selena_config?.ai_name || 'Selena')
        setSettingsLoaded(true)
      })
      .catch(() => setSettingsLoaded(true))
  }, [])

  async function saveAgentName(name: string) {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">{agentName} — AI Concierge</h1>
        <button
      ...
      {showSettings && settingsLoaded && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h3 className="font-semibold text-slate-900">{agentName} Quick Settings</h3>
            <div className="flex items-center gap-3">
              {settingsMsg && <span className={`text-xs ${settingsMsg === 'Saved!' ? 'text-green-600' : 'text-red-600'}`}>{settingsMsg}</span>}
              {settingsSaving && <span className="text-xs text-slate-400">Saving...</span>}
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between max-w-xs">
              ...
              <button
                ...
                className={`relative w-10 h-5 rounded-full transition-colors ${selenaConfig.ai_enabled ? 'bg-teal-600' : 'bg-slate-300'}`}
              >
            ...
            <a href="/dashboard/settings" className="inline-flex items-center gap-2 text-sm text-teal-600 hover:text-teal-700 transition-colors">
              Full {agentName} Settings
