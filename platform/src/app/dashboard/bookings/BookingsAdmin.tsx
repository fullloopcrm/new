    ...
    <div className={`mb-2 p-2 rounded-lg border ${variant === 'full' ? 'bg-amber-50 border-amber-200' : 'bg-indigo-50/60 border-indigo-200'}`}>
      ...
      <div className="flex flex-col gap-1.5">
        {suggestions.map((s) => (
          ...
          >
            <span className="text-sm font-semibold text-[#1C1C1C]">{s.label}</span>
  ...
  return (
    <>
      <main className="p-3 md:p-6 max-w-[1400px] mx-auto">
        {/* Stat Cards */}
        {!loading && (
          <div className="sched-outlook" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: 20 }}>
        ...
        <div className="bk-toolbar">
          <div className="bk-ai-search" style={{ fontStyle: 'normal' }}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="var(--bk-muted)" style={{ flexShrink: 0 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input type="text" placeholder={`Search client, ${worker.singular.toLowerCase()}, address...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
          <div className="bk-toolbar-right">
            <button onClick={() => setShowFilters(!showFilters)} className={'bk-btn ' + (showFilters || activeFilterCount > 0 ? 'bk-btn-primary' : 'bk-btn-ghost')}>
              Filters{activeFilterCount > 0 ? ` (${activeFilterCount})` : ''}
            </button>
            <button onClick={() => { setShowWaitlist(!showWaitlist); if (!showWaitlist) loadWaitlist() }} className={'bk-btn ' + (showWaitlist ? 'bk-btn-primary' : 'bk-btn-ghost')}>
              Waitlist
            </button>
            <button onClick={() => setShowCloseOut(!showCloseOut)} className={'bk-btn ' + (showCloseOut ? 'bk-btn-primary' : 'bk-btn-ghost')}>
            ...
            <button onClick={openCreate} className="bk-btn bk-btn-primary">+ New Booking</button>
          </div>
        </div>

        {/* Quick Links */}
        <div className="text-xs text-gray-400 mb-4 hidden md:flex items-center gap-1 flex-wrap">
          <a href="https://www.thenycmaid.com/book" target="_blank" className="text-gray-500 hover:text-[#1C1C1C] hover:underline">Client Portal</a>
          <span className="text-gray-300 mx-1">/</span>
          <a href="https://www.thenycmaid.com/book/new" target="_blank" className="text-gray-500 hover:text-[#1C1C1C] hover:underline">New Booking</a>
          <span className="text-gray-300 mx-1">/</span>
          <a href="https://www.thenycmaid.com/book/collect" target="_blank" className="text-gray-500 hover:text-[#1C1C1C] hover:underline">Collect Info</a>
          <span className="text-gray-300 mx-1">/</span>
          <a href="https://www.thenycmaid.com/team" target="_blank" className="text-gray-500 hover:text-[#1C1C1C] hover:underline">Team Portal</a>
        </div>

        {/* Status Filter Pills */}
        <div className="bk-filter-row" style={{ marginBottom: 16, flexWrap: 'nowrap', overflowX: 'auto' }}>
          <span className="bk-filter-label">Status</span>
          <span className={`bk-chip ${!filters.status ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: '' })}>
            All <span className="bk-chip-count">{statusCounts.all}</span>
          </span>
          {statusCounts.pending > 0 && (
            <span className={`bk-chip ${filters.status === 'pending' ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: 'pending' })}>
              <span className="bk-chip-dot warn" />Pending <span className="bk-chip-count">{statusCounts.pending}</span>
            </span>
          )}
          <span className={`bk-chip ${filters.status === 'scheduled' ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: 'scheduled' })}>
            <span className="bk-chip-dot scheduled" />Scheduled <span className="bk-chip-count">{statusCounts.scheduled}</span>
          </span>
          <span className={`bk-chip ${filters.status === 'in_progress' ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: 'in_progress' })}>
            <span className="bk-chip-dot progress" />In Progress <span className="bk-chip-count">{statusCounts.in_progress}</span>
          </span>
          <span className={`bk-chip ${filters.status === 'completed' ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: 'completed' })}>
            <span className="bk-chip-dot completed" />Completed <span className="bk-chip-count">{statusCounts.completed}</span>
          </span>
          <span className={`bk-chip ${filters.status === 'cancelled' ? 'active' : ''}`} onClick={() => setFilters({ ...filters, status: 'cancelled' })}>
            <span className="bk-chip-dot cancelled" />Cancelled <span className="bk-chip-count">{statusCounts.cancelled}</span>
          </span>
        </div>

        {/* Advanced Filters Panel */}
        {showFilters && (
          <div className="bg-gray-50/80 backdrop-blur-sm rounded p-4 mb-4 space-y-4 border border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Service</label>
                <select value={filters.service_type} onChange={(e) => setFilters({ ...filters, service_type: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded text-[#1C1C1C] text-sm bg-white focus:outline-none focus:border-[#1C1C1C]">
              ...
              <div>
                ...
                <select value={filters.cleaner_id} onChange={(e) => setFilters({ ...filters, cleaner_id: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded text-[#1C1C1C] text-sm bg-white focus:outline-none focus:border-[#1C1C1C]">
                  <option value="">All</option>
              ...
              <div>
                ...
                <select value={filters.client_id} onChange={(e) => setFilters({ ...filters, client_id: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded text-[#1C1C1C] text-sm bg-white focus:outline-none focus:border-[#1C1C1C]">
                  <option value="">All</option>
              ...
              <div>
                ...
                <input type="date" value={filters.date_from} onChange={(e) => setFilters({ ...filters, date_from: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded text-[#1C1C1C] text-sm bg-white focus:outline-none focus:border-[#1C1C1C]" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">To</label>
                <input type="date" value={filters.date_to} onChange={(e) => setFilters({ ...filters, date_to: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded text-[#1C1C1C] text-sm bg-white focus:outline-none focus:border-[#1C1C1C]" />
              </div>
            </div>
            <div className="flex justify-between items-center pt-2">
              <p className="text-sm text-gray-500">{filteredBookings.length} booking{filteredBookings.length !== 1 ? 's' : ''} found</p>
              <button onClick={clearFilters} className="text-sm text-gray-400 hover:text-[#1C1C1C] transition-colors">Clear All</button>
            </div>
          </div>
        )}
        ...
        {!loading && bookings.filter(b => b.status === 'pending').length > 0 && (
          <div className="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200/60 rounded p-4 mb-5">
            ...
            <div className="space-y-2">
              {bookings.filter(b => b.status === 'pending').map((b) => (
                <div key={b.id} onClick={() => openEdit(b)} className="flex items-center justify-between bg-white/80 backdrop-blur-sm border border-red-200/40 rounded p-3.5 cursor-pointer hover:bg-white hover:shadow-sm transition-all">
                  <div>
                    <p className="text-[#1C1C1C] font-semibold text-sm">{b.clients?.name || '-'}</p>
                  ...
                  <div className="text-right flex flex-col items-end gap-1.5">
                    ...
                    <p className="text-[#1C1C1C] text-sm font-semibold">~${(b.price / 100).toFixed(0)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {showWaitlist && (
          <div className="mb-5">
            <div className="bg-gradient-to-r from-purple-50 to-violet-50 border border-purple-200/60 rounded p-4 mb-4">
              ...
              {waitlistLoading ? (
                ...
                <div className="space-y-3">
                  {waitlistEntries.map((entry) => (
                    <div key={entry.id} className="bg-white rounded border border-gray-200 p-4 transition-all">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-[#1C1C1C] font-semibold text-sm">{entry.name || 'Unknown'}</p>
        ...
        {showCloseOut && (
          <div className="mb-5">
            <div className="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200/60 rounded p-4 mb-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
              ...
              {closeOutJobs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {closeOutJobs.map((b) => {
                    ...
                    return (
                      <div key={b.id} className={'bg-white rounded border p-4 transition-all ' + (isSaving ? 'opacity-60 border-emerald-200' : 'border-gray-200')}>
                        {/* Job header */}
                        <div className="flex items-start justify-between mb-3">
                          <button onClick={toggleExpanded} className="flex-1 text-left hover:opacity-80 transition-opacity">
                            <p className="text-[#1C1C1C] font-semibold text-sm flex items-center gap-1.5">
                          ...
                          <div className="text-right">
                            <p className="text-[#1C1C1C] font-bold text-lg">${(b.price / 100).toFixed(0)}</p>
            ...
            {recentlyClosedJobs.length > 0 && (
              <div className="bg-gray-50/80 border border-gray-200/60 rounded p-4">
                  ...
                  {recentlyClosedJobs.map((b) => (
                    <div key={b.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-white/60 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-[#1C1C1C] font-medium">{b.clients?.name || '-'}</p>
                      ...
                      <div className="text-right flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-[#1C1C1C]">${(b.price / 100).toFixed(0)}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* Desktop Table */}
        <div className="bg-white rounded border border-gray-200/60 overflow-hidden shadow-sm hidden md:block">
          {loading ? (
                ...
                {paginatedBookings.map((b) => (
                  ...
                  >
                    <td className="px-4 py-3.5">
                      <div>
                        <p className={'text-sm font-medium ' + (b.status === 'cancelled' ? 'text-gray-400' : 'text-[#1C1C1C]')}>{b.clients?.name || '-'}</p>
                    ...
                    <td className="px-4 py-3.5">
                      <span className={'text-sm ' + (b.status === 'cancelled' ? 'text-gray-400' : 'text-[#1C1C1C]')}>{formatDate(b.start_time)}</span>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={'text-sm font-semibold ' + (b.status === 'cancelled' ? 'text-gray-400 line-through' : 'text-[#1C1C1C]')}>~${(b.price / 100).toFixed(0)}</span>
                    </td>
                    <td className="px-4 py-3.5 hidden lg:table-cell">
                          ...
                          <div className="relative">
                            ...
                            {resendMenuId === b.id && (
                              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1 min-w-[100px]">
                                <button onClick={() => handleResend(b.id, 'email')} className="w-full text-left px-3 py-1.5 text-sm text-[#1C1C1C] hover:bg-gray-50 transition-colors">Email</button>
                                <button onClick={() => handleResend(b.id, 'sms')} className="w-full text-left px-3 py-1.5 text-sm text-[#1C1C1C] hover:bg-gray-50 transition-colors">Text</button>
                              </div>
                            )}
                          </div>
                        )}
                        <button onClick={() => openEdit(b)} className="p-1.5 rounded-lg text-gray-400 hover:text-[#1C1C1C] hover:bg-gray-100 transition-colors" title="Edit">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                        {b.status === 'cancelled' ? (
                          <button onClick={async () => { if (confirm(`Permanently delete this cancelled booking for ${b.clients?.name || 'this client'}?`)) { const res = await fetch('/api/bookings/' + b.id + '?hard_delete=true', { method: 'DELETE' }); if (!res.ok) { const err = await res.json().catch(() => ({ error: res.statusText })); alert(`Failed to delete: ${err.error || 'Unknown error'}`); } await loadBookings() } }} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete">
                      ...
                      typeof p === 'string' ? (
                        ...
                        <button
                          ...
                          className={
                            ...
                            (currentPage === p ? 'bg-[#1C1C1C] text-white' : 'text-gray-500 hover:bg-gray-100')
                          }
                        >
                          {p}
                        </button>
            ...
            <>
              {paginatedBookings.map((b) => (
                <div
                  ...
                  onClick={() => openEdit(b)}
                  className={
                    'bg-white rounded border border-gray-200/60 p-4 cursor-pointer transition-all active:scale-[0.99] ' +
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      <p className={'font-semibold text-sm ' + (b.status === 'cancelled' ? 'text-gray-400' : 'text-[#1C1C1C]')}>{b.clients?.name || '-'}</p>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-gray-100">
                    ...
                    <span className={'text-sm font-bold ' + (b.status === 'cancelled' ? 'text-gray-400 line-through' : 'text-[#1C1C1C]')}>~${(b.price / 100).toFixed(0)}</span>
                  </div>
                </div>
              ))}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between pt-2 pb-4">
                  <button
                    onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                    disabled={currentPage === 1}
                    className="px-4 py-2 rounded text-sm font-medium text-gray-500 bg-white border border-gray-200 disabled:opacity-30 transition-colors"
                  >
                    Prev
                  </button>
        ...
        <SidePanel open={showModal} onClose={() => { setShowModal(false); setEditingBooking(null) }} title={editingBooking.clients?.name || 'Booking'} width="max-w-lg">
          <form onSubmit={handleSubmit}>
              ...
              <div>
                ...
                {editingBooking.clients?.phone && (
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm font-medium text-[#1C1C1C]">{formatPhone(editingBooking.clients.phone)}</span>
            ...
            {editingBooking.cleaner_token && (
              <button type="button" onClick={copyTeamLink} className="text-xs text-[#1C1C1C]/50 hover:text-[#1C1C1C] mb-2 block">{copied ? 'Copied!' : 'Copy team link'}</button>
            )}
            ...
            {editingBooking.status === 'scheduled' && !editingBooking.check_in_time && (
              <button type="button" onClick={async () => { setSaving(true); const now = new Date().toISOString(); await fetch('/api/bookings/' + editingBooking.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'in_progress', check_in_time: now, cleaner_id: form.cleaner_id || null, skip_email: true }) }); setEditingBooking({ ...editingBooking, status: 'in_progress', check_in_time: now }); setForm({ ...form, status: 'in_progress' }); loadBookings(); setSaving(false) }} className="w-full mb-3 py-2 bg-[#1C1C1C] text-white rounded-lg text-sm font-medium">Check In (Admin)</button>
            )}
                  ...
                  <div className="flex gap-2">
                    ...
                    {!confirmCheckout ? (
                      ...
                      <div className="flex-1 flex gap-1.5">
                        ...
                        <button type="button" onClick={async () => { setConfirmCheckout(false); setSaving(true); const now = new Date(); const ciStr = editingBooking.check_in_time!; const checkIn = new Date(ciStr.endsWith('Z') || ciStr.includes('+') ? ciStr : ciStr + 'Z'); const totalMin = (now.getTime() - checkIn.getTime()) / 60000; const halfHrs = Math.floor(totalMin / 30); const rem = totalMin - halfHrs * 30; const actualHours = Math.max(0.5, rem >= 5 ? (halfHrs + 1) * 0.5 : halfHrs * 0.5); const cap = (editingBooking as any).max_hours; const billableHours = (typeof cap === 'number' && cap > 0) ? Math.min(actualHours, cap) : actualHours; const teamSize = Math.max(1, (editingBooking as any).team_size || 1); const clientRate = editingBooking.hourly_rate || 69; const updatedPrice = Math.round(billableHours * clientRate * teamSize * 100); const cleanerHourlyPay = clientRate <= 60 ? 25 : 30; const cleanerPay = Math.round(billableHours * cleanerHourlyPay * 100); await fetch('/api/bookings/' + editingBooking.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'completed', check_out_time: now.toISOString(), actual_hours: actualHours, price: updatedPrice, team_member_pay: cleanerPay, cleaner_id: form.cleaner_id || null, skip_email: true }) }); setEditingBooking({ ...editingBooking, status: 'completed', check_out_time: now.toISOString(), actual_hours: actualHours, price: updatedPrice, team_member_pay: cleanerPay }); setForm({ ...form, status: 'completed', actual_hours: actualHours, team_member_pay: cleanerPay }); loadBookings(); setSaving(false) }} className="flex-1 py-2 bg-red-600 text-white rounded-lg text-xs font-bold">Confirm Check Out</button>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="bg-gray-50 rounded p-3 mb-3 space-y-2">
              <div className="grid grid-cols-4 gap-2">
                <div>
                  ...
                  <input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-[#1C1C1C] bg-white" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={form.hours} onChange={(e) => setForm({ ...form, hours: parseInt(e.target.value) })} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-[#1C1C1C] bg-white">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select value={form.service_type} onChange={(e) => setForm({ ...form, service_type: e.target.value })} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-[#1C1C1C] bg-white">
                    {serviceTypes.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div className="flex items-end">
                  <div className="flex items-center justify-between w-full px-2 py-1.5 border border-gray-200 rounded-lg bg-white">
                    <span className="text-sm text-[#1C1C1C]">Discount</span>
              ...
              <div className="bg-green-50 border border-green-200 rounded p-3 mb-3">
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input type="number" step="0.5" min="0" value={form.actual_hours ?? ''} onChange={(e) => { const hrs = e.target.value ? parseFloat(e.target.value) : null; const cr = cleaners.find(c => c.id === form.cleaner_id)?.hourly_rate || 25; setForm({ ...form, actual_hours: hrs, team_member_pay: hrs ? Math.round(hrs * cr * 100) : null }) }} placeholder="—" className="w-full px-2 py-1.5 border border-green-300 rounded-lg text-sm text-[#1C1C1C] bg-white" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-green-600 uppercase">Team Pay</label>
                    <input type="number" step="0.01" min="0" value={form.team_member_pay != null ? (form.team_member_pay / 100).toFixed(2) : ''} onChange={(e) => setForm({ ...form, team_member_pay: e.target.value ? Math.round(parseFloat(e.target.value) * 100) : null })} placeholder="auto" className="w-full px-2 py-1.5 border border-green-300 rounded-lg text-sm text-[#1C1C1C] bg-white" />
                  </div>
                  <div>
                    <label className="block text-[10px] text-green-600 uppercase">Team Paid</label>
                    <select value={form.team_member_paid ? 'paid' : 'not_paid'} onChange={(e) => setForm({ ...form, team_member_paid: e.target.value === 'paid' })} className={'w-full px-2 py-1.5 border rounded-lg text-sm ' + (form.team_member_paid ? 'border-green-300 text-green-700 bg-green-50' : 'border-green-300 text-[#1C1C1C] bg-white')}>
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <div>
                ...
                <select value={form.payment_status} onChange={(e) => setForm({ ...form, payment_status: e.target.value })} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-[#1C1C1C]">
                  <option value="pending">Pending</option><option value="paid">Paid</option>
                </select>
              </div>
              <div>
                <label className="block text-[10px] text-gray-400 uppercase">Method</label>
                <select value={form.payment_method} onChange={(e) => setForm({ ...form, payment_method: e.target.value })} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-[#1C1C1C]">
            ...
            <div className="mb-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="px-2 py-0.5 border border-gray-300 rounded text-xs text-[#1C1C1C] bg-white"
                  >
                  ...
                  {form.team_size > 1 && (
                    <button
                      ...
                      className="text-[10px] px-2 py-0.5 bg-[#A8F0DC] text-[#1C1C1C] rounded font-semibold hover:bg-[#90E5CC]"
                    >
                    ...
                    {[form.cleaner_id, ...form.extra_cleaner_ids].filter(Boolean).map((cid, idx, arr) => {
                      ...
                      return (
                        <div
                          ...
                          className="flex items-center justify-between bg-white border border-indigo-200 rounded px-2.5 py-1.5 text-sm cursor-move hover:border-indigo-400"
                        >
                          <span className="flex items-center gap-2">
                            <span className="text-gray-400 text-base leading-none">⋮⋮</span>
                            <span className="font-medium text-[#1C1C1C]">{c?.name || cid}</span>
              ...
              <div className="space-y-1 max-h-36 overflow-y-auto">
                {form.team_size <= 1 && (
                  <button type="button" onClick={() => setForm({ ...form, cleaner_id: '' })} className={`w-full flex items-center px-3 py-1.5 rounded-lg border text-sm ${!form.cleaner_id ? 'border-indigo-500 bg-indigo-50 font-medium' : 'border-gray-200 hover:border-gray-300'} text-[#1C1C1C]`}>Unassigned</button>
                )}
                {cleaners
                  ...
                  return (
                    <button key={c.id} type="button" onClick={onClickPick} className={`w-full text-left px-3 py-1.5 rounded-lg border text-sm ${
                      ...
                      <div className="flex items-center justify-between">
                        <span className={selected ? 'font-medium text-[#1C1C1C]' : ''}>
            ...
            <div className="flex gap-2 pt-3 border-t border-gray-100">
              ...
              <button type="button" onClick={() => { setShowModal(false); setEditingBooking(null) }} className="px-4 py-2 border border-gray-300 rounded-lg text-[#1C1C1C] text-sm">Close</button>
              <button type="submit" disabled={saving} className="px-6 py-2 bg-[#1C1C1C] text-white rounded-lg text-sm font-medium">{saving ? '...' : 'Save'}</button>
            </div>
          </form>
        </SidePanel>
      )}
      ...
      {showUpdateChoice && (
        <div className="fixed inset-0 bg-[#1C1C1C]/50 flex items-center justify-center z-[10001]" onClick={() => setShowUpdateChoice(false)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-[#1C1C1C] mb-4">Update Recurring Booking</h3>
            <p className="text-gray-600 mb-6">Apply changes to:</p>
            <div className="space-y-3">
              <button onClick={() => saveBooking('single')} className="w-full py-3 px-4 border border-gray-300 rounded-lg text-[#1C1C1C] hover:bg-gray-50 text-left">
                <p className="font-medium">This booking only</p>
                <p className="text-sm text-gray-500">Only update this appointment</p>
              </button>
              <button onClick={() => saveBooking('all')} className="w-full py-3 px-4 border border-gray-300 rounded-lg text-[#1C1C1C] hover:bg-gray-50 text-left">
            ...
            <button onClick={() => setShowUpdateChoice(false)} className="w-full mt-4 py-2 text-gray-500 hover:text-[#1C1C1C]">Cancel</button>
          </div>
        </div>
      )}
        ...
        <SidePanel open={showCreateModal} onClose={() => { setShowCreateModal(false); setShowClientDropdown(false) }} title="Create Booking" width="max-w-lg">
            <form onSubmit={handleCreate}>
              <div className="space-y-4">
                <div className="relative">
                  <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Client *</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]"
                  />
                  
                  {showClientDropdown && (
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                      <button type="button" onClick={handleNewClientClick} className="w-full px-3 py-2 text-left hover:bg-gray-50 border-b border-gray-200 font-medium text-[#1C1C1C]">+ New Client</button>
                ...
                {createForm.client_id && clientProperties.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Address{clientProperties.length > 1 ? ' *' : ''}</label>
                    <select
                ...
                <div>
                  <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Service</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Date *</label>
                    <input type="date" required value={createForm.start_date} onChange={(e) => setCreateForm({ ...createForm, start_date: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]" />
                  </div>
                  <div>
                ...
                {createForm.is_emergency ? (
                  <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
                    ...
                    <div className="flex items-center">
                      <span className="text-[#1C1C1C] text-lg mr-1">$</span>
                      <input
                        type="number"
                        step="1"
                        min="25"
                        ...
                        className="w-24 px-3 py-2 border border-red-300 rounded-lg text-[#1C1C1C] text-center font-mono bg-white"
                      />
                      <span className="text-[#1C1C1C] ml-1">/hr</span>
                    </div>
                  </div>
                ) : (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="block text-sm font-medium text-[#1C1C1C]">{createForm.team_size > 1 ? worker.plural : worker.singular} *</label>
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          className="px-2 py-1 border border-gray-300 rounded text-sm text-[#1C1C1C] bg-white"
                        >
                          {[1, 2, 3, 4, 5, 6].map(n => <option key={n} value={n}>{n}</option>)}
                        </select>
                        {createForm.team_size > 1 && (
                          <button
                            ...
                            className="text-xs px-2 py-1 bg-[#A8F0DC] text-[#1C1C1C] rounded font-semibold hover:bg-[#90E5CC]"
                          >
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-colors ${
                              isLead
                                ? 'border-indigo-500 bg-indigo-50 text-[#1C1C1C]'
                                : isExtra
                                  ? 'border-indigo-500 bg-indigo-50 text-[#1C1C1C]'
                                  : topPick
                                    ? 'border-green-400 bg-green-50 text-[#1C1C1C]'
                                    : isZoneMatch
                                      ? 'border-green-200 bg-green-50/40 text-[#1C1C1C]'
                                      : 'border-gray-200 hover:border-gray-300 text-[#1C1C1C]'
                            }`}
                          >
                            <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Hours</label>
                    <select value={createForm.hours} onChange={(e) => setCreateForm({ ...createForm, hours: parseInt(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]">
                      {[1,2,3,4,5,6,7,8].map(h => <option key={h} value={h}>{h}hr</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#1C1C1C] mb-1">Rate</label>
                    <input
                      type="number"
                      min={1}
                      step={1}
                ...
                <div>
                  ...
                  <select value={createForm.status} onChange={(e) => setCreateForm({ ...createForm, status: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]">
                    <option value="pending">Pending</option>
                    <option value="scheduled">Scheduled</option>
                    <option value="completed">Completed</option>
                    <option value="cancelled">Cancelled</option>
                ...
                <div>
                  ...
                  <textarea value={createForm.notes} onChange={(e) => setCreateForm({ ...createForm, notes: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]" rows={2} placeholder="Access codes..." />
                </div>
              </div>
              <div className="flex gap-3 mt-6">
                <button type="button" onClick={() => { setShowCreateModal(false); setShowClientDropdown(false) }} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]">Cancel</button>
                <button type="submit" disabled={saving || !createForm.client_id} className="flex-1 px-4 py-2 bg-[#1C1C1C] text-white rounded-lg disabled:bg-gray-300">
      ...
      {showNewClientModal && (
        <div className="fixed inset-0 bg-[#1C1C1C]/50 flex items-center justify-center z-[60]" onClick={() => setShowNewClientModal(false)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-[#1C1C1C] mb-4">New Client</h3>
            <form onSubmit={handleNewClientSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Name *</label>
                <input type="text" required value={newClientForm.name} onChange={(e) => setNewClientForm({ ...newClientForm, name: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]" placeholder="John Smith" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="email" value={newClientForm.email} onChange={(e) => setNewClientForm({ ...newClientForm, email: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]" placeholder="john@email.com" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Phone *</label>
                <input type="tel" required value={newClientForm.phone} onChange={(e) => setNewClientForm({ ...newClientForm, phone: formatPhone(e.target.value) })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]" placeholder="212-555-1234" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={newClientForm.unit} onChange={(e) => setNewClientForm({ ...newClientForm, unit: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]" placeholder="Apt 4B" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={newClientForm.referrer_id} onChange={(e) => setNewClientForm({ ...newClientForm, referrer_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]">
                  <option value="">None</option>
              ...
              <div>
                ...
                <textarea value={newClientForm.notes} onChange={(e) => setNewClientForm({ ...newClientForm, notes: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1C1C1C]" rows={3} placeholder="Any special instructions..." />
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowNewClientModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-[#1C1C1C]">Cancel</button>
                <button type="submit" disabled={saving} className="flex-1 px-4 py-2 bg-[#1C1C1C] text-white rounded-lg">{saving ? '...' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
