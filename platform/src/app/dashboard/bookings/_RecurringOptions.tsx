  ...
  return (
    <div className="border-t pt-4 mt-4">
      <div className="flex justify-between items-center mb-3">
        <h4 className="font-medium text-[var(--color-loop-ink)]">Repeat</h4>
        <div 
          onClick={() => onEnabledChange(!enabled)}
          className={`w-10 h-6 rounded-full transition-colors ${enabled ? 'bg-[var(--color-loop-ink)]' : 'bg-gray-300'} relative cursor-pointer`}
        >
      ...
      {enabled && (
        <div className="space-y-4 bg-gray-50 border border-gray-200 rounded-lg p-4">
          {/* Repeat Type */}
          <div>
            <label className="block text-sm font-medium text-[var(--color-loop-ink)] mb-2">Repeats</label>
            <select 
              value={repeatType} 
              onChange={(e) => onRepeatTypeChange(e.target.value)} 
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[var(--color-loop-ink)] bg-white focus:ring-2 focus:ring-[var(--color-loop-ink)] outline-none"
            >
          ...
          {repeatType === 'custom' && (
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center border border-gray-300 rounded-lg bg-white">
                ...
                >−</button>
                <span className="px-4 py-2 text-[var(--color-loop-ink)] font-medium min-w-[40px] text-center">{customInterval}</span>
                <button
                  type="button"
          ...
          {/* End Options */}
          <div>
            <label className="block text-sm font-medium text-[var(--color-loop-ink)] mb-2">End repeating</label>
            <select 
              value={repeatEnd} 
          ...
          {repeatEnd === 'after' && (
            <div className="flex items-center gap-3">
              ...
              <input 
                ...
                className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-[var(--color-loop-ink)] text-center"
              />
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-3">
            <p className="text-sm text-gray-600 mb-2">
              Recurring schedule — first <span className="font-semibold text-[var(--color-loop-ink)]">{Math.min(previewDates.length, 4)}</span> bookings created, then auto-generated weekly:
            </p>
