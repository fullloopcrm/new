...
// Same detail/manage view for a single booking, usable both as the standalone
// /dashboard/bookings/[id] page (embedded=false, real back-link + full width)
// and inside a SidePanel from the Calendar view (embedded=true, no back-link —
// the panel's own title bar + close X cover that). Extracted so Calendar no
// longer has to router.push() away to a full page to show the same booking
// BookingsAdmin.tsx already shows in-place via a side panel.
export default function BookingDetailContent({
  ...
  const teamPortalLink = typeof window !== 'undefined' ? `${window.location.origin}/team/${bookingId}` : `/team/${bookingId}`
