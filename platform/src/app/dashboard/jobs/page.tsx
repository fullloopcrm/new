        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-xs text-slate-500 uppercase">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">Est. completion</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {jobs.map(j => (
              <tr key={j.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => router.push(`/dashboard/jobs/${j.id}`)}>
                ...
                <td className="px-4 py-3 text-right text-slate-500">{j.session_count > 0 ? `${j.pct_complete}%` : '—'}</td>
                <td className="px-4 py-3 text-slate-500">{j.ends_on ? dateOnlyLabel(j.ends_on) : '—'}</td>
            ...
            {jobs.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400 text-sm">No jobs yet. Convert an accepted quote to a project job.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
