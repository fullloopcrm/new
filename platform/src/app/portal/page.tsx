...
export default function PortalHomePage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-400 mb-6">{auth.tenant.name}</p>

      <Link href="/portal/book" className="block w-full bg-slate-800 text-white text-center py-3 rounded-xl font-medium mb-6">
        Book an Appointment
      </Link>
              ...
              <Link key={b.id} href={`/portal/bookings/${b.id}`} className="block bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {b.team_members && <p className="text-xs text-slate-400">{b.team_members.name}</p>}
                  </div>
      ...
      {past.length > 0 && (
        <div className="mb-6">
          <h2 className="font-semibold text-slate-800 mb-3">Past Bookings</h2>
          <div className="space-y-2">
      ...
      <Link href="/portal/feedback" className="block text-center text-sm text-slate-400 hover:text-slate-500 mt-8">
        Leave Feedback
      </Link>
    </div>
  )
