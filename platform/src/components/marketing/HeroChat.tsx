  ...
  async function send(text?: string) {
    ...
    if (waitingForPhone) {
      ...
      try {
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: 'Sorry, something went wrong. Text us at (212) 202-9030.' }])
        setQuickReplies([])
      } finally {
        setLoading(false)
  ...
  return (
    <div className="w-full">
      ...
      {!open && quickReplies.length > 0 && (
        <>
          <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-white tracking-wide mb-1">Chat with <span className="text-[#A8F0DC]">Selena</span></p>
          <p className="text-blue-200/70 text-base mb-4 max-w-[85%]">Ask a question, get a quote, book an appointment, reschedule, or manage your account — all in one chat.</p>
