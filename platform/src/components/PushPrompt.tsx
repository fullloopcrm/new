...
export default function PushPrompt({ role, userId }: PushPromptProps) {
    ...
    try {
      ...
      await fetch('/api/push/subscribe', {
        ...
        body: JSON.stringify({
          ...
          role,
          team_member_id: role === 'team_member' ? userId : undefined,
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
      {isIOS && !isStandalone ? (
        <>
          <p className="font-medium text-sm text-slate-900 mb-1">Get push notifications</p>
        ...
        <>
          <p className="font-medium text-sm text-slate-900 mb-2">Get push notifications on this device</p>
          <button
            onClick={enablePush}
            disabled={pushLoading}
            className="w-full py-2 px-4 bg-teal-600 text-white text-sm font-medium rounded-lg hover:bg-teal-700 transition disabled:opacity-50"
          >
        ...
        <>
          <p className="font-medium text-sm text-slate-900 mb-1">Push notifications unavailable</p>
