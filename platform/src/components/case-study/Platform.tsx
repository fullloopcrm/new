  ...
  return (
    <Band id="platform" tone="cream">
      <Column className="py-20 sm:py-28">
        ...
        <H3>The dashboard an operator actually gets</H3>
        <P>The nav an operator logs into is organized around the actual shape of running the business, not around software modules invented for a demo: The Loop as the executive home; Clients with per-client SMS threads and feedback; ComHub, which folds every conversation surface — including Loop Connect&apos;s Slack-style channels for the team, each client, and each crew member — into one inbox; Sales, which is where the Catalog (the price book quotes and invoices actually pull from) lives alongside Sales Partners and the client-referral program; Production, which is Bookings, multi-visit Projects, the Schedule, Crews, staffing lookup, and company announcements in one place; Finance, with the ledger, reconciliation, and payroll underneath it; a new HR module for team documents and onboarding paperwork, which is genuinely the newest and least-built-out surface in the platform — it exists, it is not yet as deep as the rest; and Marketing, split into Campaigns, Reviews, Social, Google Business Profile sync, the tenant&apos;s own Websites, and Analytics. A separate Platform tray underneath all of it holds Onboarding, Settings, Users, the AI agent&apos;s own admin surface, and Legal.</P>
        <P>None of that is aspirational copy — it&apos;s a direct description of the nav a real operator sees on login, the same one running The NYC Maid today.</P>
