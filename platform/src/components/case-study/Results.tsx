...
export default function Results({ live }: { live: CaseStudyStats | null }) {
  ...
  const clientsDisplay = live ? live.clients.toLocaleString() : "852";
  const conversationsDisplay = live ? live.conversations.toLocaleString() : "1,966";
  ...
  return (
    <Band id="results" tone="canvas">
      <Column wide className="py-20 sm:py-28">
        ...
        <P>Hold those two numbers next to each other — {clientsDisplay} clients, one operator — because their ratio is the whole argument. Conventional wisdom in the trade puts a single person&apos;s practical ceiling at a few dozen active clients before the coordination overwhelms them; past that, you hire. The NYC Maid is operating at an order of magnitude beyond that ceiling with no back-office headcount at all. That isn&apos;t a marginal efficiency gain to be celebrated with a percentage. It&apos;s a different relationship between the size of a business and the human cost of running it.</P>

        {live && (
          <>
            <H3>This month vs. last month</H3>
            <P>Booked, not just completed — because the pipeline for the current month is the leading indicator, and this page shows it live rather than waiting for the month to close to report it.</P>
            <StatGrid cols={3} items={[
              { v: live.lastMonthCompleted.toLocaleString(), l: "Jobs completed", s: "last calendar month" },
              { v: live.thisMonthBooked.toLocaleString(), l: "Jobs booked", s: "this month, not cancelled" },
              { v: String(Math.max(0, live.daysLeftInMonth)), l: "Days left", s: "in the current month" },
            ]} />
          </>
        )}
        ...
        {live?.avgTicketPrice && (
          <>
            <H3>The average job</H3>
            <StatGrid cols={2} items={[
              { v: live.avgTicketPrice, l: "Average ticket", s: "per completed, paid job" },
              { v: live.revenueRangeYtd, l: "Revenue", s: "since launch (Feb 2026)" },
            ]} />
          </>
        )}
        ...
        <P>The result is a cost structure that inverts the usual trap: revenue scales with jobs, while the overhead that normally scales right alongside it — the people you hire to handle growth — simply doesn&apos;t. The marginal cost of the {clientsDisplay}th client is close to the marginal cost of the 70th. That is the whole game, and it&apos;s why this is worth proving rather than asserting.</P>

        {live?.cleanerPayoutMedianSeconds != null && (
          <Callout title="Cleaner payouts, measured — not asserted">
            Median time from a cleaner tapping &ldquo;checkout&rdquo; to their Stripe payout landing, measured across the most recent paid-out jobs: <strong>{live.cleanerPayoutMedianSeconds} seconds</strong>. That&apos;s a recent change — payouts used to run in batches, not per job — and it&apos;s computed live from the same production data as everything else on this page, not a number we typed in once.
          </Callout>
        )}
