...
export default function ClientTranscript({ clientId }: { clientId: string }) {
  ...
  if (loading) return <div className="text-sm text-slate-400 py-2">Loading transcript...</div>
  if (messages.length === 0) return <div className="text-sm text-slate-400 py-2">No SMS history</div>
    ...
    <div>
      ...
      <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
        {Object.entries(shownGrouped).map(([date, msgs]) => (
          <div key={date}>
            <p className="text-[10px] text-slate-400 text-center mb-1.5">{date}</p>
