...
// Can't be forged without knowing the password — signWithSecret() throws if
// ADMIN_PASSWORD is unset rather than silently signing with a '' key, which
// would be a publicly-computable signature (zero-config session forgery).
function signToken(token: string): string {
  return signWithSecret(token, process.env.ADMIN_PASSWORD)
}

// Hash password with HMAC-SHA256. No 'fallback' default — see signToken().
