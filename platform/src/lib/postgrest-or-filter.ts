 ...
 * term — it becomes additional filter *syntax*. See
 * `deploy-prep/or-filter-injection-determination.md` for the full exploitability
 * analysis and the tenant-isolation boundary reasoning.
 *
 * THE FIX PRIMITIVE
 * -----------------
 ...
 * param `or=(filters)` (the whole string, URL-encoded — verified in
 * node_modules/@supabase/postgrest-js). Injection is therefore confined *inside*
 * that one OR group and can never add a sibling top-level query param, so a
 * separate `.eq('tenant_id', …)` filter is structurally AND-ed and unreachable.
 * This helper closes the in-group manipulation; the separate tenant `.eq()` is
 * what enforces cross-tenant isolation.
 */
