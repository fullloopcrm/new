/**
 * `createBookingFromQuote` — email-case client dedup.
 *
...
describe('createBookingFromQuote — email-case client dedup', () => {
  it('matches an existing client whose stored email differs only in case from contact_email', async () => {
    const result = await createBookingFromQuote(TENANT_ID, QUOTE_ID)
    expect(result.already_converted).toBe(false)

    expect(fake._all('clients').length).toBe(1)
    const booking = fake._all('bookings').find((b) => b.id === result.booking_id)
