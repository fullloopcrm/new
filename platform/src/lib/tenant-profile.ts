...
export const PROFILE_FIELDS: FieldDef[] = [
  ...
  { key: 'ein', label: 'EIN / Tax ID', section: 'identity', store: 'entity', col: 'ein', tier: 'critical', dependsOn: { key: 'einOptOut', value: false }, validation: { kind: 'regex', pattern: '^\\d{2}-?\\d{7}$', message: 'Enter your real 9-digit EIN (XX-XXXXXXX) — this has to match what\'s on file with the IRS for Stripe and Telnyx to accept it.' }, read: (x) => e(x, 'ein'), help: 'Your business\'s federal tax ID (like a Social Security number, but for the business) — the 9-digit number on your IRS confirmation letter. Not the same as your Social Security number. Required to register your business texting number with Telnyx (federal SMS compliance) and to verify your Stripe account — without it, payments and texting can\'t go live.' },
  // einOptOut is the single shared gate for EIN + Stripe + Telnyx -- not three separate decisions. Neither
  // Stripe nor Telnyx can actually go live without an EIN (see ein's help text above), so there's no
  // meaningful "opt out of Stripe but keep Telnyx" state to model. tier stays 'optional' -- the toggle
  // itself never blocks launch, it just changes whether `ein` (above) and `paymentMethods` (payments
  // section, below) are still required. See computeReadiness()'s dependsOn handling in tenant-readiness.ts.
  { key: 'einOptOut', label: 'I don\'t have an EIN yet', section: 'identity', store: 'selena', col: 'ein_opt_out', kind: 'bool', input: 'toggle', tier: 'optional', read: (x) => s(x, 'ein_opt_out'), help: 'Turn this on to skip your EIN for now. Since Stripe and Telnyx both require it, this also means no card payments through Full Loop and no business texting number yet — you can add your EIN and turn both on later without losing any other setup.' },
  ...
  { key: 'ownerName', label: 'Owner name', section: 'contact', store: 'tenant', col: 'owner_name', tier: 'recommended', help: 'The business owner\'s name — used for owner-facing notifications and the admin\'s Owner Contact quick-view.', read: (x) => t(x, 'owner_name') },
  ...
  { key: 'ownerPhone', label: 'Owner phone', section: 'contact', store: 'tenant', col: 'owner_phone', tier: 'recommended', help: 'The owner\'s direct phone number — used for owner-facing notifications, separate from the public business phone above.', validation: { kind: 'minLength', minLength: 10, digitsOnly: true, message: 'Enter a real 10-digit phone number.' }, read: (x) => t(x, 'owner_phone') },
  ...
  // dependsOn einOptOut: PAYMENT_OPTIONS is Stripe-only today (see its definition below), and Stripe can't
  // go live without an EIN -- so once a tenant's opted out of EIN there's nothing on this list they could
  // actually pick yet. Non-critical + hidden in that state instead of demanding an unanswerable question.
  // Revisit once PAYMENT_OPTIONS grows non-Stripe methods (Zelle/Venmo/etc, queued separately).
  { key: 'paymentMethods', label: 'Payment methods', section: 'payments', store: 'tenant', col: 'payment_methods', kind: 'array', input: 'array', options: PAYMENT_OPTIONS, tier: 'critical', dependsOn: { key: 'einOptOut', value: false }, funnels: ['booking', 'pipeline'], help: 'We process everything through Stripe — card, Apple Pay, and Cash App are all handled through the same Stripe checkout, nothing separate to set up for each.', read: (x) => t(x, 'payment_methods') },
  ...
  // noInsurance never blocks launch (insurance fields above were already 'recommended'/'optional', never
  // 'critical') -- this is purely so a tenant can say "I don't have insurance" explicitly instead of
  // leaving these blank ambiguously. Deliberately does NOT change runLegalOverlookCheck (legal-overlook.ts):
  // its insurance_missing trigger already fires off !compliance.insurance_carrier regardless of any flag,
  // so a tenant who opts out here still gets the same legal reminders/tips as one who just never filled it in.
  { key: 'noInsurance', label: 'I don\'t currently have insurance', section: 'compliance', store: 'compliance', col: 'no_insurance', kind: 'bool', input: 'toggle', tier: 'optional', read: (x) => c(x, 'no_insurance'), help: 'Turn this on if you don\'t carry general liability insurance yet. You\'ll still get reminders about getting covered — some states require it and customers trust it — but nothing here blocks you from using Full Loop.' },
  { key: 'insuranceCarrier', label: 'Insurance carrier', section: 'compliance', store: 'compliance', col: 'insurance_carrier', input: 'custom', tier: 'recommended', dependsOn: { key: 'noInsurance', value: false }, help: 'The company your business (general liability, not personal) insurance is through.', read: (x) => c(x, 'insurance_carrier') },
  { key: 'insurancePolicy', label: 'Insurance policy #', section: 'compliance', store: 'compliance', col: 'insurance_policy', tier: 'optional', dependsOn: { key: 'noInsurance', value: false }, help: 'The policy number for your general liability insurance (the policy that covers accidents/damage on the job — not your license number above).', read: (x) => c(x, 'insurance_policy') },
  { key: 'insuranceCoverage', label: 'Coverage amount', section: 'compliance', store: 'compliance', col: 'insurance_coverage', input: 'custom', tier: 'optional', dependsOn: { key: 'noInsurance', value: false }, help: 'How much your general liability policy covers — from your insurance declarations page.', read: (x) => c(x, 'insurance_coverage') },
  ...
  { key: 'insuranceCertUrl', label: 'Certificate of insurance', section: 'compliance', store: 'compliance', col: 'insurance_cert_url', input: 'custom', tier: 'recommended', dependsOn: { key: 'noInsurance', value: false }, help: 'The actual certificate document from your insurer proving the coverage above is real.', read: (x) => c(x, 'insurance_cert_url') },
