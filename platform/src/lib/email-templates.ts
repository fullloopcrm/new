...
© ${year} ${escapeHtml(data.tenantName)} · powered by <a href="https://homeservicecrm.ai/" style="text-decoration:none"><span style="font-family:${DISPLAY};font-weight:600;color:#1C1C1C">Full&nbsp;Loop</span><span style="font-family:${MONO};font-size:8px;letter-spacing:0.18em;color:#807B70">&nbsp;CRM</span></a><br>Autonomous Home Service Business CRM Systems${data.businessAddress ? `<br>${escapeHtml(data.businessAddress)}` : ''}${data.unsubscribeUrl ? `<br><a href="${safeUrl(data.unsubscribeUrl)}" style="color:#807B70;text-decoration:underline">Unsubscribe from these emails</a>` : ''}
</td></tr>
