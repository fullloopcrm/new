...
export function bookingReminderEmail(data: TemplateData & {
  ...
  return baseTemplate(`
    ...
    <tr><td style="padding:8px 16px;">
      ...
      <!-- dateTime is intentionally NOT escaped: bookings/broadcast/route.ts passes a
           pre-built HTML fragment through this field (its own inputs are escaped there). -->
...
export function bookingConfirmationEmail(data: TemplateData & {
  ...
  return baseTemplate(`
    ...
    ${data.portalUrl ? `<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
      <a href="${escapeHtml(data.portalUrl)}" style="display:inline-block;background:${data.primaryColor || '#111827'};color:#ffffff;padding:12px 32px;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;">
        View Booking
      </a>
    </td></tr></table>` : ''}
...
export function teamApplicationApprovedEmail(data: TemplateData & {
  ...
  return baseTemplate(`
    ...
    <div style="background:#f5f5f5;border-radius:8px;padding:24px;margin:0 0 24px;text-align:center;">
      ...
      <p style="margin:0;font-size:36px;font-weight:700;color:#111827;letter-spacing:6px;">${data.pin}</p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;"><tr><td align="center">
      <a href="${data.portalUrl}" style="display:inline-block;background:${color};color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:14px 28px;border-radius:8px;">Open Team Portal / Abrir Portal</a>
    </td></tr></table>
...
// Wraps caller-built HTML in the shared branded shell. For sends whose body
// is generated by the caller (marketing/campaign copy, e.g. renurture win-back
// touches) rather than one of the typed template functions above — bodyHtml
// is trusted, already-safe HTML the caller is responsible for escaping any
// interpolated user data into (see lib/renurture.ts's use of escapeHtml).
export function campaignEmail(data: TemplateData & { bodyHtml: string }): string {
  return baseTemplate(data.bodyHtml, data)
}
