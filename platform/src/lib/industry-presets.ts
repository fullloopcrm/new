...
export const SERVICE_PRESETS: Record<IndustryKey, DefaultService[]> = {
  ...
  dumpster: [
    ...
    svc('15-Yard Dumpster', 'Small-to-mid cleanout or remodel', 1, 400, 2),
    svc('20-Yard Dumpster', 'Mid-size renovation / cleanout', 1, 450, 3),
    svc('30-Yard Dumpster', 'Large construction / demolition', 1, 550, 4),
    svc('40-Yard Dumpster', 'Commercial / heavy debris', 1, 650, 5),
    svc('Same-Day / Rush Delivery', 'Expedited drop-off within hours', 1, 495, 6),
    svc('Extended Rental (per week)', 'Additional week beyond the standard term', 1, 75, 7),
    svc('Heavy Debris Dumpster (Dirt/Concrete/Brick)', 'Weight-capped bin for dense material', 1, 495, 8),
    svc('Driveway Protection Boards', 'Plywood placement to prevent driveway damage', 1, 45, 9),
    svc('Dumpster Swap / Exchange', 'Pull full bin and drop an empty mid-project', 1, 350, 10),
  ],
  towing: [
  ...
  landscaping: [
    ...
    svc('Sod Installation', 'Remove old lawn, grade, and lay new sod', 6, 90, 5),
    svc('Retaining Wall Build', 'Small block or timber retaining wall', 8, 95, 6),
    svc('Tree & Shrub Planting', 'Selection, planting, and initial care', 3, 85, 7),
  ],
  remodeling: [
    ...
    svc('Whole-Home Renovation', 'Multi-room full-house renovation', 8, 100, 5),
    svc('Attic Conversion', 'Convert attic to livable space', 8, 95, 6),
    svc('Garage Conversion', 'Convert garage to living space', 8, 90, 7),
    svc('Sunroom / Patio Enclosure', 'Enclose or build out a sunroom/patio', 8, 95, 8),
    svc('Aging-in-Place Modification', 'Accessibility-focused remodel (widened doors, curbless shower)', 8, 95, 9),
    svc('Design-Build Consultation', 'On-site scope walkthrough + written estimate', 2, 95, 10),
  ],
  roofing: [
    ...
    svc('Gutter & Flashing Repair', 'Flashing reseal + gutter reattachment', 3, 110, 5),
    svc('Skylight Install', 'New skylight install with flashing', 4, 135, 6),
    svc('Roof Maintenance Plan', 'Annual inspection + minor repair package', 2, 95, 7),
  ],
  siding: [
    ...
    svc('Vinyl Siding Install', 'New vinyl siding, full exterior', 8, 100, 5),
    svc('Fiber Cement Siding Install', 'Fiber cement plank install', 8, 120, 6),
    svc('Siding Cleaning & Inspection', 'Wash + condition assessment', 2, 75, 7),
  ],
  painting: [
    ...
    svc('Drywall Patch & Repair', 'Pre-paint holes, cracks, and dents', 2, 65, 5),
    svc('Wallpaper Removal', 'Strip old wallpaper + wall prep', 4, 70, 6),
    svc('Popcorn Ceiling Removal', 'Scrape + smooth + repaint ceiling', 6, 75, 7),
    svc('Trim, Doors & Baseboards', 'Detail painting of trim and doors', 4, 65, 8),
    svc('Commercial / Office Painting', 'After-hours commercial repaint', 8, 80, 9),
    svc('Color Consultation', 'On-site color matching and selection', 1, 60, 10),
  ],
  flooring: [
    ...
    svc('Carpet Installation', 'New carpet, padding, and install', 6, 75, 5),
    svc('Subfloor Repair', 'Repair or replace damaged subfloor', 4, 90, 6),
    svc('Baseboard & Trim', 'Matching trim install after flooring', 3, 65, 7),
  ],
  concrete: [
    ...
    svc('Stamped / Decorative Concrete', 'Stamped patterns and coloring', 8, 105, 5),
    svc('Concrete Steps', 'New or replacement concrete steps', 6, 95, 6),
    svc('Sidewalk Install', 'New concrete sidewalk section', 6, 90, 7),
  ],
  deck: [
    ...
    svc('Composite Deck Build', 'Low-maintenance composite decking', 8, 105, 5),
    svc('Deck Demolition', 'Tear-out and haul-away of old deck', 4, 85, 6),
    svc('Pergola / Shade Structure', 'Attached or freestanding pergola build', 8, 95, 7),
  ],
  fencing: [
    ...
    svc('Gate Install / Repair', 'New gate or hardware repair', 2, 80, 5),
    svc('Fence Staining & Sealing', 'Clean, stain, and seal wood fence', 4, 70, 6),
    svc('Aluminum / Ornamental Fence', 'Decorative aluminum fence install', 8, 100, 7),
  ],
  demolition: [
    ...
    svc('Shed / Outbuilding Demo', 'Tear down and haul small structure', 4, 95, 5),
    svc('Deck / Patio Demo', 'Remove existing deck or patio', 4, 90, 6),
    svc('Selective Strip-Out', 'Cabinets, fixtures, flooring removal', 4, 85, 7),
  ],
  drywall: [
    ...
    svc('New Construction Hang & Finish', 'Full-home hang, tape, and finish', 8, 90, 5),
    svc('Ceiling Repair', 'Water stain or crack ceiling repair', 3, 85, 6),
    svc('Skim Coating', 'Smooth-finish skim coat over wall', 4, 90, 7),
  ],
  epoxy: [
    ...
    svc('Metallic Epoxy Finish', 'Decorative metallic epoxy floor', 8, 125, 5),
    svc('Polyaspartic Coating', 'Fast-cure polyaspartic garage coating', 6, 110, 6),
    svc('Concrete Grinding & Prep', 'Surface prep before coating', 4, 85, 7),
  ],
  foundation: [
    ...
    svc('Pier / Piling Install', 'Foundation leveling with piers', 8, 175, 5),
    svc('Egress Window Install', 'Basement egress window + well', 6, 150, 6),
    svc('French Drain Install', 'Exterior drainage system', 6, 125, 7),
  ],
  insulation: [
    ...
    svc('Crawl Space Insulation', 'Encapsulation + insulation', 4, 95, 5),
    svc('Insulation Removal', 'Remove old/contaminated insulation', 4, 90, 6),
    svc('Rim Joist Sealing', 'Seal + insulate rim joist band', 2, 80, 7),
  ],
  moving: [
    ...
    svc('Furniture Disassembly / Reassembly', 'Disassemble + reassemble large furniture', 2, 85, 5),
    svc('Piano / Specialty Item Move', 'Specialty equipment for heavy items', 3, 150, 6),
    svc('Storage / Junk Haul-Away', 'Load to storage or dispose unwanted items', 3, 100, 7),
  ],
  paving: [
    ...
    svc('Asphalt Patch & Crack Fill', 'Pothole and crack patching', 3, 85, 5),
    svc('Curb & Gutter Install', 'Concrete curb and gutter section', 6, 95, 6),
    svc('Line Striping', 'Parking lot line striping', 2, 75, 7),
  ],
  windows_doors: [
    ...
    svc('Interior Door Install', 'Interior door + hardware', 2, 90, 5),
    svc('Garage Man-Door Install', 'Side entry door install', 3, 100, 6),
    svc('Window Screen Repair', 'Screen re-mesh and reframe', 1, 55, 7),
  ],
  stucco: [
    ...
    svc('Stucco Color Coat', 'Fresh finish/color coat over existing', 6, 90, 5),
    svc('EIFS Repair', 'Synthetic stucco (EIFS) repair', 4, 100, 6),
    svc('Stucco Inspection', 'Moisture + crack assessment report', 1, 85, 7),
  ],
  solar: [
    ...
    svc('Permit & Utility Interconnection', 'Permitting and utility paperwork', 2, 110, 5),
    svc('Solar Panel Cleaning', 'Panel wash for output efficiency', 2, 75, 6),
    svc('EV Charger + Solar Integration', 'Tie EV charger into solar system', 3, 150, 7),
  ],
  smart_home: [
    ...
    svc('Smart Thermostat Install', 'Thermostat setup + app config', 1, 85, 5),
    svc('Home Theater / AV Setup', 'Speaker, TV, and AV wiring', 4, 110, 6),
    svc('Network / Wi-Fi Optimization', 'Mesh Wi-Fi install for whole-home coverage', 2, 95, 7),
  ],
  accessibility: [
    ...
    svc('Stairlift Install', 'Straight or curved stairlift install', 6, 150, 5),
    svc('Widened Doorway', 'Widen doorway for wheelchair access', 4, 110, 6),
    svc('Curbless Shower Conversion', 'Zero-threshold shower conversion', 8, 125, 7),
  ],
  restoration: [
    ...
    svc('Sewage Cleanup', 'Biohazard sewage backup cleanup', 4, 175, 5),
    svc('Odor Removal Treatment', 'Ozone/hydroxyl odor treatment', 2, 110, 6),
    svc('Content Pack-Out & Cleaning', 'Pack out, clean, and store belongings', 4, 95, 7),
  ],
  interior_design: [
    ...
    svc('Furniture Selection & Sourcing', 'Curated furniture sourcing plan', 2, 110, 5),
