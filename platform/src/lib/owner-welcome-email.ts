    ...
    await sendEmail({
      ...
      html: `
        <div style="font-family: -apple-system, sans-serif; max-width: 500px;">
          <h2 style="color: #333;">${tenantName} is live on Full Loop</h2>
          <p style="color: #555;">Sign in with your PIN — no password to remember.</p>
          <a href="${loginUrl}" style="display: inline-block; background: #0d9488; color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; margin: 12px 0;">${loginUrl}</a>
          <p style="color: #333; font-size: 20px; font-weight: 700; letter-spacing: 2px; margin: 16px 0;">${ownerPin}</p>
