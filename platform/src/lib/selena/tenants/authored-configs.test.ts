...
describe('consortium-nyc — full-service marketing agency (sibling brand), quote-first persona', () => {
  it('registry resolves the consortium slug to the authored config', () => {
    expect(getAuthoredConfig(CONSORTIUM_NYC_SLUG)).toBe(consortiumNycConfig)
  })

  it('resolves to its OWN Consortium persona, not the generic professional default', () => {
    const cfg = getAuthoredConfig(CONSORTIUM_NYC_SLUG)!
    expect(cfg.identity.business_name).toBe('Consortium NYC')
    expect(cfg.voice.persona).toContain('marketing partner')
    expect(cfg.voice.persona).not.toContain(GENERIC_PERSONA)
  })
