...
describe('create_manual_booking — FK tenant-ownership', () => {
  ...
  it("REJECTS a client_id from another tenant (no booking inserted)", async () => {
    ...
    expect(insertCalls.filter((c) => c.table === 'bookings')).toHaveLength(0)
  })
  ...
  it('ALLOWS own-tenant client + cleaner (booking inserted, tenant-scoped)', async () => {
    ...
    const bookingInserts = insertCalls.filter((c) => c.table === 'bookings')
    expect(bookingInserts).toHaveLength(1)
    const values = bookingInserts[0].values as Record<string, unknown>
...
describe('create_deal — FK tenant-ownership', () => {
  it("REJECTS a client_id from another tenant (no deal inserted)", async () => {
    ...
    expect(JSON.parse(out).error).toBe('client not found')
    expect(insertCalls.filter((c) => c.table === 'deals')).toHaveLength(0)
  })
  ...
  it('ALLOWS an own-tenant client (deal inserted, tenant-scoped)', async () => {
    ...
    expect(JSON.parse(out).ok).toBe(true)
    const dealInserts = insertCalls.filter((c) => c.table === 'deals')
    expect(dealInserts).toHaveLength(1)
    expect((dealInserts[0].values as Record<string, unknown>).tenant_id).toBe(TENANT_A)
  })
})
