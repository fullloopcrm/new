...
async function handleResendConfirmation(input: Record<string, unknown>, conversationId: string): Promise<string> {
  try {
    ...
    const { data: booking } = await supabaseAdmin.from('bookings')
      .select('start_time, service_type, hourly_rate, clients(name, email, pin), team_members(name)')
      .eq('id', bookingId).eq('tenant_id', tid).eq('client_id', convo.client_id).single()
...
export async function handleRescheduleBooking(input: Record<string, unknown>, conversationId: string): Promise<string> {
  try {
    ...
    const { data: booking } = await supabaseAdmin.from('bookings').select('id, start_time, recurring_type, client_id, tenant_id')
      .eq('id', bookingId).eq('tenant_id', tid).eq('client_id', convo.client_id).single()
    if (!booking) return JSON.stringify({ error: 'Booking not found' })
...
async function handleCancelBooking(input: Record<string, unknown>, conversationId: string): Promise<string> {
  try {
    ...
    const { data: booking } = await supabaseAdmin.from('bookings').select('id, start_time, recurring_type, clients(name), tenant_id')
      .eq('id', bookingId).eq('tenant_id', tid).eq('client_id', convo.client_id).single()
    if (!booking) return JSON.stringify({ error: 'Booking not found' })
...
export async function handleBookingDetails(input: Record<string, unknown>, conversationId: string): Promise<string> {
  try {
    ...
    const { data: booking } = await supabaseAdmin.from('bookings')
      .select('id, start_time, end_time, check_in_time, check_out_time, check_in_location, check_out_location, actual_hours, hourly_rate, price, team_member_pay, payment_status, payment_method, status, service_type, team_members(name), clients(name, address), client_properties(address)')
      .eq('id', bookingId).eq('tenant_id', tid).eq('client_id', convo.client_id).single()
