...
beforeEach(() => {
  ...
  // This branch's isOwnerOfTenant() (not part of p1-w3's base) resolves
  // owner status per-tenant via tenants.owner_phone for any non-nycmaid
  // tenant id -- seed it so the owner() helper's calls pass the gate.
...
describe('create_manual_booking writes team_member_id/suggested_team_member_id', () => {
  it('leaves team_member_id null and stores the suggested tech under suggested_team_member_id', async () => {
    // This branch's handleCreateManualBooking (not part of p1-w3's base) verifies
    // client_id/cleaner_id belong to this tenant before insert (FK-injection guard) --
    // seed both so that check passes and the schema-mapping behavior under test runs.
    fake._seed('clients', [{ id: 'client-1', tenant_id: TENANT_ID }])
    fake._seed('team_members', [{ id: 'member-1', tenant_id: TENANT_ID }])
