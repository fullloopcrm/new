...
-- on it -- has been dead code since inception. Confirmed via static grep
-- (three columns, all three referenced only by this one webhook, none
-- defined anywhere in the tracked migration history, resend_email_id never
-- written anywhere either) -- no live Supabase env in this worktree to
-- confirm against information_schema directly.
--
-- Additive-only: adding these columns alone does not make delivered/opened
-- tracking start working -- the send path (campaigns/send/route.ts,
-- campaigns/[id]/send/route.ts, via notify()'s email branch) also never
-- captures/stores the Resend email id returned by sendEmail() at send time,
-- so campaign_recipients.resend_email_id would stay NULL on every row even
