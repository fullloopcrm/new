...
function classify(r: Rollup, tenant_id: string | null): Detected | null {
  ...
  const base = { property: r.property, tenant_id, target_url: r.page, intent, detail, status: 'open' }

  // Page 2 with real demand → one push from page 1. Highest-leverage.
  if (pos >= 11 && pos <= 20 && r.impressions >= 10) {
    return { ...base, type: 'striking_distance', recipe: 'onpage_push', tier: 1, severity: r.impressions >= 100 ? 'high' : 'medium' }
  }
  // Ranking top-10 but barely clicked → title/meta problem, not a ranking one.
  if (pos <= 10 && r.ctr < 0.03 && r.impressions >= 20) {
    return { ...base, type: 'low_ctr', recipe: 'title_meta', tier: 1, severity: 'medium' }
  }
  // Real demand but ranking deep → the page is too thin to compete → enrich.
  if (pos > 20 && r.impressions >= 15) {
    return { ...base, type: 'deep_underperformer', recipe: 'enrich', tier: 2, severity: r.impressions >= 100 ? 'high' : 'low' }
  }
  return null
}
...
async function fetchRollup(): Promise<Rollup[]> {
  ...
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await supabaseAdmin
      ...
      .select('property,page,impressions,clicks,ctr,avg_position,has_applicant')
