...
export async function checkFleetHealth(): Promise<SiteHealth[]> {
  ...
  // Union both sources as independent check targets rather than picking one
  // winner per tenant: tenant_domains is best-effort at registration time
  // (activate-tenant.ts's upsert never blocks activation on failure), so a
  // tenant can be live only in tenants.domain. Checking both means coverage
  // can never silently drop a tenant's real domain — the tradeoff is a stale
  // tenants.domain (post-migration) also gets checked, which is noise, not blindness.
  const rows: Array<{ domain: unknown; tenant_id: string | null }> = [
    ...(data ?? []),
    ...(legacy ?? []).map((t) => ({ domain: t.domain, tenant_id: t.id as string | null })),
  ]
  const targets = rows
    .map((r) => ({ domain: norm(String(r.domain)), tenant_id: r.tenant_id }))
    .filter((t) => {
