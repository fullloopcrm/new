/**
 * NYC Tow (thenyctowingservice.com) — tenant SEO descriptor.
 * Was on the 11-URL fallback; enumerates the real route tree so its location,
 * service, and who-we-serve pages get listed. getCityBySlug / SERVICES.find /
 * CUSTOMER_TYPES validate against these same sources → every URL renders 200.
 */
...
registerTenantSeo({
  ...
  buildUrls(): UrlSpec[] {
    ...
    for (const state of STATES) {
      ...
      out.push({ loc: `${BASE}/careers/${state.slug}`, priority: 0.5, changeFrequency: 'monthly' })
