/**
 * `createRecurringSeriesFromQuote` — email-case client dedup.
 *
...
describe('createRecurringSeriesFromQuote — email-case client dedup', () => {
  it('matches an existing client whose stored email differs only in case from contact_email', async () => {
    const result = await createRecurringSeriesFromQuote(TENANT_ID, QUOTE_ID)
    expect(result.already_converted).toBe(false)

    expect(fake._all('clients').length).toBe(1)
    const schedule = fake._all('recurring_schedules').find((s) => s.id === result.schedule_id)
    expect(schedule?.client_id).toBe(EXISTING_CLIENT_ID)
  })
})
