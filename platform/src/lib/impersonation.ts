import crypto from 'node:crypto'

/**
 * Signed impersonation cookie helpers.
 * Cookie value format: `<tenantId>:<exp>.<hmac>` where hmac is HMAC-SHA256 of
 * the `<tenantId>:<exp>` payload under ADMIN_TOKEN_SECRET, and exp is a Unix
 * ms timestamp embedded in (and covered by) the signature.
 *
 * Why: before signing, a client could forge `fl_impersonate=<any-tenant-id>`
 * and, combined with a stolen/leaked admin_token, impersonate arbitrary tenants.
 * The signature proves the cookie was minted by our server. exp must be part
 * of the signed payload (not just the cookie's Max-Age) — matching
 * createAdminToken/createTenantAdminToken in admin-auth/route.ts — otherwise a
 * captured cookie value stays validly-signed forever, replayable for as long
 * as a paired admin_token remains valid (up to 24h, far past the intended
 * 1-hour impersonation window).
 */
export const IMPERSONATE_COOKIE = 'fl_impersonate'
const MAX_AGE_MS = 3600 * 1000 // 1 hour, matches admin/impersonate route's cookie Max-Age

export function signImpersonation(tenantId: string): string {
  const secret = process.env.ADMIN_TOKEN_SECRET
  if (!secret) throw new Error('ADMIN_TOKEN_SECRET not configured')
  const payload = `${tenantId}:${Date.now() + MAX_AGE_MS}`
  const hmac = crypto.createHmac('sha256', secret).update(payload).digest('hex')
  return `${payload}.${hmac}`
}

/**
 * Returns the validated tenantId if the cookie is genuine and unexpired, else
 * null. Accepts legacy unsigned values too (raw UUID) when
 * IMPERSONATION_ALLOW_UNSIGNED=1 — useful during rolling cutover; remove once
 * all in-flight sessions have rotated. Previously-issued signed cookies in the
 * old `<tenantId>.<hmac>` (no exp) format are no longer accepted — that's the
 * point of this fix, not a bug.
 */
export function verifyImpersonationCookie(raw: string | undefined): string | null {
  if (!raw) return null
  const secret = process.env.ADMIN_TOKEN_SECRET
  if (!secret) return null

  // Signed form: "<uuid>:<exp>.<hex>"
  const dotIdx = raw.indexOf('.')
  if (dotIdx > 0) {
    const payload = raw.slice(0, dotIdx)
    const sig = raw.slice(dotIdx + 1)
    const expected = crypto.createHmac('sha256', secret).update(payload).digest('hex')
    const a = Buffer.from(sig, 'hex')
    const b = Buffer.from(expected, 'hex')
    if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) return null

    const sepIdx = payload.indexOf(':')
    if (sepIdx <= 0) return null // no embedded exp — reject (old format or forged)
    const exp = Number(payload.slice(sepIdx + 1))
    if (!Number.isFinite(exp) || exp <= Date.now()) return null
    return payload.slice(0, sepIdx)
  }

  // Legacy unsigned form — only accept if explicitly allowed.
  if (process.env.IMPERSONATION_ALLOW_UNSIGNED === '1') return raw
  return null
}
