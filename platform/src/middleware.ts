...
function rewriteToSite(req: NextRequest, tenantId: string, tenantSlug: string): NextResponse {
  ...
  // /quote, /invoice, /sign are public, token-authed proposal/invoice/
  // document-sign pages (src/app/quote/[token] etc.) — also root-level, not
  // under /site/<slug>. Without this, a proposal/invoice/agreement link sent
  // to a tenant's own domain rewrote to a nonexistent /site/<slug>/quote/...
  // page and 404d for every tenant.
